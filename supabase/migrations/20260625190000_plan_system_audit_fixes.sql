-- My Plan audit fixes: active-plan uniqueness and transactional operations.

with ranked_active_itineraries as (
  select
    id,
    row_number() over (
...
set status = 'archived'
from ranked_active_itineraries ranked
...
create or replace function public.get_or_create_active_itinerary(
...
begin
  if p_owner_user_id is distinct from auth.uid() then
    raise exception 'Cannot create itinerary for a different user';
  end if;
  ...
  begin
    insert into public.itineraries (owner_user_id)
...
begin
  ...
  from public.processed_plan_operations
  where operation_id = p_operation_id
    and owner_user_id = p_owner_user_id;

  select id
    into v_itinerary_id
  from public.get_or_create_active_itinerary(p_owner_user_id)
  limit 1;
...
begin
  ...
  if exists (
    select 1 from public.processed_plan_operations
    where operation_id = p_operation_id
      and owner_user_id = p_owner_user_id
  ) then
    select *
...
create or replace function public.delete_itinerary_operation(
  p_operation_id uuid,
  p_owner_user_id uuid
)
returns void
...
begin
  ...
  if not found then
    ...
    values (p_operation_id, p_owner_user_id, 'delete_plan')
    on conflict (operation_id) do nothing;
    return;
  end if;

  update public.itinerary_shares s
  set status = 'revoked', revoked_at = v_now
  where s.itinerary_id = v_itinerary.id
    and s.status = 'active';
...
create or replace function public.create_live_share_operation(
  p_owner_user_id uuid,
  p_rotate boolean,
  p_token_hash text,
  p_max_rotations_per_day integer
)
returns table (
  reused boolean,
  share_id uuid
...
declare
  v_itinerary public.itineraries%rowtype;
  v_share public.itinerary_shares%rowtype;
  v_rotations integer;
  v_today_start timestamptz;
begin
  if p_owner_user_id is distinct from auth.uid() then
    raise exception 'Cannot share another user plan';
  end if;
  ...
  if not found then
    raise exception 'No plan to share';
  end if;
  ...
  if found and not p_rotate then
    return query select true, v_share.id;
    return;
  end if;

  if p_rotate then
    v_today_start := (timezone('America/New_York', now())::date::timestamp at time zone 'America/New_York');
    ...
    if v_rotations >= p_max_rotations_per_day then
      raise exception 'Share rotation quota reached';
    end if;
  end if;
  ...
  values (v_itinerary.id, p_token_hash, 'active')
  returning * into v_share;

  return query select false, v_share.id;
end;
$$;

create or replace function public.revoke_live_share_operation(
  p_owner_user_id uuid
)
returns void
language plpgsql
...
begin
  if p_owner_user_id is distinct from auth.uid() then
    raise exception 'Cannot revoke another user plan share';
  end if;
...
grant execute on function public.get_or_create_active_itinerary(uuid) to authenticated;
...
grant execute on function public.delete_itinerary_operation(uuid, uuid) to authenticated;
grant execute on function public.create_live_share_operation(uuid, boolean, text, integer) to authenticated;
grant execute on function public.revoke_live_share_operation(uuid) to authenticated;
