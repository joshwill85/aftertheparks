...
- Full-calendar source-spanned fixture for Old Key West. It publishes 13 source-backed records, covers split `Near` movie location headings and fee-marker activities, repairs reviewed OCR-damaged descriptions for `Disney Fit Yoga`, `Conch Crazy Bingo`, and `Conch Flats Mosaics`, and keeps `Bike Rentals` quarantined because the captured source evidence still lacks a safe schedule span.
...
- Full-calendar source-spanned fixture for Riviera. It publishes 9 source-backed records, quarantines `Painting On The Riviera` because OCR dropped required age/phone text, and promotes `Find A Friend` through reviewed manual visual title evidence when OCR misses the heading.
...
- Full-calendar source-spanned fixture for Polynesian. It publishes 8 source-backed records, quarantines `Campfire` because OCR missed the visual location and `Aloha After Dark` because OCR dropped the `8:00pm` time, and promotes `Video Game Dance Party` through reviewed manual visual title evidence while preserving the supporting location/schedule/description spans.
- Full-calendar source-spanned fixture for Port Orleans Riverside. It publishes 10 source-backed records, repairs reviewed `Ol’ Man Island` OCR location text from source-spanned lines, quarantines `Medicine Show Arcade` and `Arts & Crafts` because OCR dropped or mangled required schedule/description text, and promotes `Nighttime Trivia Challenge` plus `Glow With The Flow` through reviewed manual visual evidence for the bottom-right activity blocks.
- Full-calendar source-spanned fixture for Port Orleans French Quarter. It publishes 9 source-backed records, repairs reviewed OCR-truncated location fields through `data/quality/activity_field_repairs.json`, joins split location continuations such as `Throughout Disney’s Port Orleans Resort - French Quarter`, and quarantines `Arts & Crafts` plus `Movie Under the Stars` because OCR still cannot safely preserve the craft table and movie table details.
...
- Full-calendar source-spanned fixture for Coronado Springs. It publishes 8 source-backed records, repairs the corrupted `FITNE Ss S CENTERS` OCR heading into `Fitness Centers`, suppresses craft-table option cells such as `Mickey Tie-Dye` as fake standalone activities, and quarantines `Spanish Mosaic Art`, `Colors Of Coronado Painting Experience`, `Arts & Crafts`, `Sangria University`, and `Resort Scavenger Hunt` because OCR or Disney's source omits required public schedule/location/description fidelity.
...
- Expand fixture coverage beyond the current twenty full-calendar PDF groups and one official-web group to every active source profile and activity record. Current audit: 225 fixture records / 222 Gold rows across 20 full-calendar PDF groups plus Fort Wilderness official web detail rows, with 33 expected quarantines and 0 expected unextractable source-visible records. No cached PDF-backed group lacks fixture-backed Gold coverage. Coverage-required count is now fixture-backed at 222, not the contaminated legacy count, because full-calendar fixtures, reviewed manual records, and explicit quarantines are authoritative and legacy overcounts/undercounts are reported separately from real gaps.
- Resolve the 33 expected quarantine records with a better extractor, reviewed manual spans, authoritative detail sources, or documented non-public decisions. They must not be hidden by lowering the fixture baseline.
...
- Add operator-facing review/reporting for changed PDF hashes, fixture gaps, and validation failures beyond the current CLI.
