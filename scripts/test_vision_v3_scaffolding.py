...
from scripts.ingest import promote_gold_v3, publish_gold_v3, run_pipeline
...
from scripts.ingest.source_status_v3 import SOURCE_STATUS_SCHEMA_VERSION, build_v3_source_statuses
...
def source_status_fixture(**overrides: Any) -> dict[str, Any]:
    return {"schema_version": SOURCE_STATUS_SCHEMA_VERSION, **overrides}
...
def clean_source_metrics_report(
    preview: Optional[dict[str, Any]] = None,
    *,
    generated_at: str = "2026-06-28T12:01:00+00:00",
) -> dict[str, Any]:
    bound_preview = preview or clean_publish_v3_preview()
    rows = bound_preview.get("rows") if isinstance(bound_preview.get("rows"), list) else []
    summary = bound_preview.get("summary") if isinstance(bound_preview.get("summary"), dict) else {}
    source_hashes = sorted(
        {
            str(row.get("source_sha256") or (row.get("source") or {}).get("documentHash") or "").strip()
            for row in rows
            if isinstance(row, dict)
            and str(row.get("source_sha256") or (row.get("source") or {}).get("documentHash") or "").strip()
        }
    )
    return {
        ...
        "summary": {
            ...
            "candidate_count": summary.get("input_candidates", len(rows)),
        },
        "sources": [{"source_sha256": source_hash} for source_hash in source_hashes],
    }


def clean_monitoring_report(*, generated_at: str = "2026-06-28T12:01:00+00:00") -> dict[str, Any]:
    return {
...
def with_source_hash(row: dict[str, Any], source_hash: str) -> dict[str, Any]:
    cloned = json.loads(json.dumps(row))
    cloned["source_sha256"] = source_hash
    cloned["source"]["documentHash"] = source_hash
    for evidence in cloned.get("field_evidence", {}).values():
        if isinstance(evidence, dict) and isinstance(evidence.get("source"), dict):
            evidence["source"]["content_sha256"] = source_hash
    return cloned
...
def empty_review_queue_cli_defaults(
    ...
    *,
    ...
    include_report_output: bool = True,
    include_html_output: bool = True,
    include_checklist_output: bool = True,
) -> list[str]:
    empty_list_path = root / "empty_review_queue_default_list.json"
    empty_report_path = root / "empty_review_queue_default_report.json"
    report_output_path = root / "empty_review_queue_report_output.json"
    html_output_path = root / "empty_review_queue.html"
    checklist_output_path = root / "empty_review_queue_checklist.csv"
    ...
    if include_source_inventory:
        args.extend(["--source-inventory", str(empty_list_path)])
    if include_review_decisions:
        args.extend(["--review-decisions", str(empty_list_path)])
    if include_report_output:
        args.extend(["--report-output", str(report_output_path)])
    if include_html_output:
        args.extend(["--html-output", str(html_output_path)])
    if include_checklist_output:
        args.extend(["--checklist-output", str(checklist_output_path)])
    return args
...
class VisionV3ScaffoldingTests(unittest.TestCase):
    ...
    def test_evaluation_report_combines_review_queue_manual_review_and_unextractables(self) -> None:
        ...
        self.assertEqual(report["report_kind"], "baseline_v2_report")
        self.assertEqual(report["schema_version"], "baseline_v2_report_001")
    ...
    def test_spike_report_is_no_publish_and_includes_runtime_budget_contract(self) -> None:
        ...
        self.assertEqual(report["report_kind"], "vision_v3_spike_report")
        self.assertEqual(report["schema_version"], "vision_v3_spike_report_001")
    ...
    def test_spike_report_summarizes_available_runtime_storage_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_image = root / "page_001.png"
            debug_overlay = root / "overlay_primary.png"
            field_crop = root / "schedule.png"
            page_image.write_bytes(b"p" * 120)
            debug_overlay.write_bytes(b"d" * 30)
            field_crop.write_bytes(b"c" * 50)

            report = build_spike_report(
                baseline_report={"known_failures": []},
                render_report={
                    "results": [
                        {
                            "render_elapsed_seconds": 2.0,
                            "source_document_pages": [
                                {"canonical_image_storage_path": str(page_image)}
                            ],
                        }
                    ]
                },
                vision_snapshot_report={
                    "results": [
                        {
                            "ocr_elapsed_seconds": 4.0,
                            "debug_overlay_path": str(debug_overlay),
                        }
                    ]
                },
                review_queue_report={
                    "summary": {
                        ...
                        "task_count_by_calendar_group": {"boardwalk": 2, "pop-century": 1},
                    }
                },
                review_tasks=[{"field_crop": str(field_crop)}],
            )

        metrics = report["runtime_storage_metrics"]
        self.assertEqual(2.0, metrics["average_render_time_per_page_seconds"])
        self.assertEqual(4.0, metrics["average_ocr_time_per_page_seconds"])
        self.assertEqual(120, metrics["average_page_image_size_bytes"])
        self.assertEqual(40, metrics["average_crop_debug_artifact_size_bytes"])
        self.assertEqual(50, metrics["average_field_crop_size_bytes"])
        self.assertEqual(1, metrics["field_crop_sample_count"])
        self.assertEqual("recorded", metrics["metric_status"]["average_field_crop_size_bytes"])
        self.assertEqual(3, report["manual_review_metrics"]["pending_task_count"])
        self.assertEqual("boardwalk", report["rollout_recommendation"]["first_review_focus"]["calendar_group_key"])
        self.assertEqual("aframe_recreation", report["rollout_recommendation"]["first_production_parser_target"])
    ...
    def test_runbook_quarter_filter_selects_only_matching_source_editions(self) -> None:
        ...
        q3_official_schedule_image = ActivitySource(
            "port-orleans-riverside",
            ("port-orleans-resort-riverside",),
            "https://disneyworld.disney.go.com/resorts/port-orleans-resort-riverside/recreation/",
            "https://cdn.example.com/activity-schedule/PORS_CKS-Digital-Rec-Sign_052626-FINAL.jpg",
            "official-schedule-0526",
            source_type="image",
        )
        ...
        sources = [q3_pdf, q4_image, q3_official_schedule_image, html_only]

        self.assertEqual(
            ["boardwalk", "port-orleans-riverside"],
            [source.calendar_group_key for source in filter_activity_sources_for_quarter(sources, "FY26-Q3")],
        )
    ...
    def test_render_source_document_renders_reviewed_visual_source_manifest_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source_dir = root / "source"
            output_dir = root / "pages"
            source_dir.mkdir()
            Image.new("RGB", (12, 8), color=(32, 64, 128)).save(source_dir / "page-1.png", "PNG")
            Image.new("RGB", (10, 14), color=(64, 32, 128)).save(source_dir / "page-2.png", "PNG")
            manifest_path = source_dir / "reviewed_visual_source.json"
            logical_hash = "a" * 64
                ...
                json.dumps(
                    {
                        ...
                        "canonical_url": "manual://fort-wilderness/example",
                        "content_sha256": logical_hash,
                        "images": [
                            {
                                "page": 1,
                                "path": "page-1.png",
                                "visible_title": "Recreation Activities",
                                "visible_validity": "May 26-Sept. 8, 2026",
                            },
                            {
                                "page": 2,
                                "path": "page-2.png",
                                "visible_title": "Recreation Activities continued",
                            },
                        ],
                    }
                )
            ...
            self.assertEqual("image_manifest", manifest["source_type"])
            self.assertEqual(logical_hash, manifest["source_sha256"])
            self.assertEqual(2, manifest["page_count"])
            self.assertEqual([1, 2], [page["page_number"] for page in manifest["pages"]])
            self.assertEqual(
                ["reviewed_visual_schedule_image_page", "reviewed_visual_schedule_image_page"],
                [page["page_kind"] for page in manifest["pages"]],
            )
            self.assertTrue(Path(manifest["pages"][0]["canonical_image_path"]).name == "page_001.png")
            self.assertTrue(Path(manifest["pages"][1]["canonical_image_path"]).name == "page_002.png")
            self.assertTrue(Path(manifest["pages"][0]["canonical_image_path"]).exists())
            self.assertTrue(Path(manifest["pages"][1]["canonical_image_path"]).exists())
            self.assertEqual("Recreation Activities", manifest["pages"][0]["visible_title"])
    ...
    def test_render_source_pages_batch_scans_source_documents_and_writes_status_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("v3_render_source_pages_001", report["schema_version"])
            ...
            self.assertGreaterEqual(report["results"][0]["render_elapsed_seconds"], 0)
            self.assertGreaterEqual(report["summary"]["average_render_time_per_page_seconds"], 0)
            self.assertGreater(report["summary"]["page_image_bytes"], 0)
            self.assertGreater(report["summary"]["thumbnail_bytes"], 0)
            self.assertGreater(report["summary"]["average_page_image_bytes"], 0)
            self.assertGreater(report["summary"]["average_thumbnail_bytes"], 0)
    ...
    def test_bronze_source_document_row_records_pdf_page_count(self) -> None:
        source_path = Path("data/raw/source_documents/boardwalk/fy26-q3-0526/a372587f7f03bfcdf30dc86df4660d75df9763571e5cec7e4556ef96924df54d.pdf")
        ...
        self.assertEqual("application/pdf", row["mime_type"])
        self.assertEqual(1, row["raw_page_count"])
        self.assertIsNone(row["raw_width"])
        self.assertIsNone(row["raw_height"])
    ...
    def test_vision_snapshot_batch_scans_page_manifests_and_writes_status_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("v3_vision_snapshot_001", report["schema_version"])
            ...
            self.assertGreaterEqual(report["results"][0]["ocr_elapsed_seconds"], 0)
            self.assertGreaterEqual(report["summary"]["average_ocr_time_per_page_seconds"], 0)
            self.assertTrue(report_path.exists())

    def test_vision_snapshot_report_existing_summarizes_snapshots_without_rerunning_ocr(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            snapshots = [
                {
                    ...
                    "debug_overlays": {"primary": {"overlay_path": "debug/source-a/overlay_primary.png"}},
                },
                {
                    "snapshot_kind": "vision_layout_v3",
                    ...
                    "edition": "fy26-q3-0526",
                    "document_family": "vertical_digital_rec_sign",
                    "quality": {"ocr_success": False},
                    "debug_overlays": {"primary": {"overlay_path": "debug/source-b/overlay_primary.png"}},
                },
            ]
            for snapshot in snapshots:
                (snapshots_dir / f"{snapshot['source_sha256']}.vision.json").write_text(
                    json.dumps(snapshot)
                )

            with patch(
                "scripts.ingest.vision_snapshot.PaddleOcrAdapter.run_page",
                side_effect=AssertionError("OCR should not run for report-only snapshot summaries"),
            ), patch(
                "scripts.ingest.vision_snapshot.RapidOcrAdapter.run_page",
                side_effect=AssertionError("OCR should not run for report-only snapshot summaries"),
            ):
                report = vision_snapshot.build_vision_snapshot_report_from_existing_snapshots(
                    snapshots_dir=snapshots_dir,
                    report_path=report_path,
                    quarter="fy26-q3",
                )

            self.assertEqual("v3_vision_snapshot", report["report_kind"])
            self.assertEqual("v3_vision_snapshot_001", report["schema_version"])
            self.assertEqual(2, report["summary"]["page_manifest_count"])
            self.assertEqual(2, report["summary"]["snapshot_count"])
            self.assertEqual(0, report["summary"]["parser_error_count"])
            self.assertIsNone(report["summary"]["average_ocr_time_per_page_seconds"])
            self.assertEqual(
                ["source-a", "source-b"],
                [result["source_sha256"] for result in report["results"]],
            )
            self.assertEqual(str(snapshots_dir / "source-a.vision.json"), report["results"][0]["snapshot_path"])
            self.assertIsNone(report["results"][0]["page_manifest_path"])
            self.assertTrue(report_path.exists())
    ...
    def test_region_segmentation_creates_major_crops_for_known_family(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            resort_region = next(region for region in result["regions"] if region["region_type"] == "resort_activities_section")
            movie_region = next(region for region in result["regions"] if region["region_type"] == "movie_section")
            self.assertGreater(resort_region["bbox_px"][3], int(1800 * 0.9))
            self.assertLess(movie_region["bbox_px"][2], int(1200 * 0.55))

    def test_vision_snapshot_can_refresh_regions_without_rerunning_ocr(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_path = root / "page.png"
            snapshots_dir = root / "snapshots"
                ...
                json.dumps(
                    {
                        ...
                        "regions": [
                            {"region_id": "old-region", "region_type": "resort_activities_section", "bbox_px": [576, 234, 1200, 1296]}
                        ],
                        "derived_source_links": [
                            {
                                "target_url": "https://qr.disneyworld.com/example",
                                "source_role": "derived_source_link",
                                "source_authority": "non_official",
                            }
                        ],
                        "tokens": [{"engine": "paddleocr_ppstructurev3", "text": "COMPLIMENTARY CRAFT"}],
                    }
                )
            )

            report = vision_snapshot.refresh_snapshot_regions(
                snapshots_dir=snapshots_dir,
                report_path=root / "report.json",
                group="all-star-movies",
                quarter="fy26-q3",
                debug_output_dir=root / "debug",
            )
            refreshed = json.loads(snapshot_path.read_text())
            resort_region = next(region for region in refreshed["regions"] if region["region_type"] == "resort_activities_section")
            movie_region = next(region for region in refreshed["regions"] if region["region_type"] == "movie_section")

            self.assertEqual(1, report["summary"]["refreshed_count"])
            self.assertEqual([{"engine": "paddleocr_ppstructurev3", "text": "COMPLIMENTARY CRAFT"}], refreshed["tokens"])
            self.assertEqual(
                [
                    {
                        "target_url": "https://qr.disneyworld.com/example",
                ...
                refreshed["derived_source_links"],
            )
            self.assertGreater(resort_region["bbox_px"][3], int(1800 * 0.9))
            self.assertLess(movie_region["bbox_px"][2], int(1200 * 0.55))

    def test_vision_snapshot_refresh_regions_reclassifies_stale_unknown_family_from_source_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_path = root / "page.png"
            snapshots_dir = root / "snapshots"
                ...
                json.dumps(
                    {
                        ...
                        "source_type": "pdf",
                        "canonical_url": "https://example.test/All-Star-Sports_Aframe_Recreation-0526_DIGITAL.pdf",
                        "source_pages": [
                            {
                                "page_number": 1,
                                "page_image_path": str(page_path),
                        ...
                        "regions": [
                            {"region_id": "old-region", "region_type": "unknown", "bbox_px": [0, 0, 1200, 1800]}
                        ],
                        "tokens": [{"engine": "paddleocr_ppstructurev3", "text": "POOL PARTY"}],
                    }
                )
            )
            ...
            report = vision_snapshot.refresh_snapshot_regions(
                snapshots_dir=snapshots_dir,
                report_path=root / "report.json",
                group="all-star-sports",
                quarter="fy26-q3",
                debug_output_dir=root / "debug",
            )

            refreshed = json.loads(snapshot_path.read_text())
            region_types = {region["region_type"] for region in refreshed["regions"]}
            self.assertEqual(1, report["summary"]["refreshed_count"])
            self.assertEqual("aframe_recreation", refreshed["document_family"])
            self.assertEqual("aframe_recreation", refreshed["family_classification"]["document_family"])
            self.assertIn("resort_activities_section", region_types)
            self.assertIn("movie_section", region_types)
    ...
    def test_extract_v3_accepts_primary_day_with_matching_secondary_time_fragment(self) -> None:
        ...
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["schedule"]["agreement"])
        self.assertEqual("Thursday from 6:30pm-7:30pm", candidate["schedule_raw"])
    ...
    def test_extract_v3_movie_location_schedule_rejoins_secondary_split_pipe_time(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "source_role": "resort_pdf",
            "canonical_url": "https://example.com/calendar.pdf",
            "fetched_url": "https://example.com/calendar.pdf",
            "http_status": 200,
            "currentness": "current",
            "captured_at": "2026-06-28T12:00:00+00:00",
            "source_sha256": "sourcehash",
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "MOVIEUNDER THESTARS", [362, 12984, 4921, 13252], 1),
                token("paddleocr_ppstructurev3", "LAWN BETWEEN BUILDINGS 5 AND 6 | 8:3OPM", [358, 13592, 5197, 13815], 2),
                token("paddleocr_ppstructurev3", "Sunday...", [350, 14033, 1106, 14248], 3),
                token("paddleocr_ppstructurev3", "..Big Hero 6 PG", [4029, 14053, 5222, 14269], 4),
                token("rapidocr", "MOVIE UNDER THE STARS", [707, 12779, 4950, 13016], 1),
                token("rapidocr", "LAWN BETWEEN BUILDINGS 5 AND 6 |", [699, 13350, 4460, 13538], 2),
                token("rapidocr", "I8:30PM", [4412, 13350, 5215, 13546], 3),
                token("rapidocr", "Sunday", [675, 13750, 1318, 13979], 4),
                token("rapidocr", ".Big Hero 6 [PG", [4122, 13750, 5247, 13971], 5),
            ],
        }
        ...
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["location"]["agreement"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["schedule"]["agreement"])
        self.assertEqual("LAWN BETWEEN BUILDINGS 5 AND 6", candidate["field_evidence"]["location"]["engines"][1]["text"])
        self.assertEqual("I8:30PM", candidate["field_evidence"]["schedule"]["engines"][1]["text"])
        self.assertEqual([], candidate["validation_findings"])
    ...
    def test_extract_v3_recovers_movie_title_from_secondary_when_primary_is_clipped_fragment(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "page_number": 1,
        ...
        snapshot = {
            ...
            "runtime_lineage": {
                ...
                "model_asset_hashes": {"paddleocr_ppstructurev3": "modelhash"},
                "ocr_engine_configs": {
                    "paddleocr_ppstructurev3": {"engine": "paddleocr_ppstructurev3"},
                    "rapidocr": {"engine": "rapidocr"},
                },
            },
            "regions": [region],
            "tokens": [
                {**base_token, "engine": "paddleocr_ppstructurev3", "text": "Movie Under the Stars", "field_hint": "title", "reading_order": 1, "confidence": 0.96},
                {**base_token, "engine": "rapidocr", "text": "MOVIE UNDER THE STARS", "field_hint": "title", "reading_order": 1, "confidence": 0.96},
                {**base_token, "engine": "paddleocr_ppstructurev3", "text": "Thursday at 8:30pm", "field_hint": "schedule", "reading_order": 2, "confidence": 0.96},
                {**base_token, "engine": "rapidocr", "text": "Thursday 8:30 PM", "field_hint": "schedule", "reading_order": 2, "confidence": 0.96},
                {**base_token, "engine": "paddleocr_ppstructurev3", "text": "Pembe Savanna Overlook", "field_hint": "location", "reading_order": 3, "confidence": 0.96},
                {**base_token, "engine": "rapidocr", "text": "Pembe Savanna Overlook", "field_hint": "location", "reading_order": 3, "confidence": 0.96},
                {**base_token, "engine": "paddleocr_ppstructurev3", "text": "oy 3", "field_hint": "movie_title", "reading_order": 4, "confidence": 0.72},
                {**base_token, "engine": "rapidocr", "text": "Toy Story 3", "field_hint": "movie_title", "reading_order": 4, "confidence": 0.99},
                {**base_token, "engine": "paddleocr_ppstructurev3", "text": "no fee marker", "field_hint": "fee_legend", "reading_order": 5, "confidence": 0.96},
                {**base_token, "engine": "rapidocr", "text": "no fee marker", "field_hint": "fee_legend", "reading_order": 5, "confidence": 0.96},
            ],
        }
        ...
        self.assertEqual("Toy Story 3", candidate["movie_title"])
        self.assertEqual("toy story 3", candidate["normalized_movie_title"])
        self.assertTrue(candidate["field_evidence"]["movie_title"]["recovered_from_secondary_ocr"])
        self.assertEqual("Toy Story 3", candidate["field_evidence"]["movie_title"]["raw_value"])
        self.assertEqual("Toy Story 3", candidate["source_spans"]["movie_title"][0]["text"])
        self.assertNotIn("engine_disagreement:movie_title", validated["validation_findings"])
    ...
    def test_extract_v3_vertical_sign_derives_field_evidence_from_unhinted_ocr_tokens(self) -> None:
        ...
        self.assertEqual("Monday from 2:30pm-3:30pm", candidate["schedule_raw"])
    ...
    def test_extract_v3_writes_field_specific_crop_from_page_image_when_bbox_is_available(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_path = root / "page.png"
            field_crops_dir = root / "field_crops_v3"
            Image.new("RGB", (120, 100), color=(255, 255, 255)).save(page_path)
            region = {
                "region_id": "page-001-region-03",
                "region_type": "vertical_activity_row",
                "region_family": "vertical_digital_rec_sign",
            ...
            snapshot = {
                ...
                "source_sha256": "sourcehash",
                "calendar_group_key": "port-orleans-riverside",
                "source_pages": [{"page_number": 1, "canonical_image_sha256": "pagehash", "canonical_image_path": str(page_path)}],
                "regions": [region],
                "tokens": [
                    token("paddleocr_ppstructurev3", "MONDAY", [10, 5, 70, 15], 1),
                    token("paddleocr_ppstructurev3", "POOL PARTY", [10, 20, 80, 35], 2),
                    token("paddleocr_ppstructurev3", "Ol' Man Island Pool Deck", [10, 40, 100, 55], 3),
                    token("paddleocr_ppstructurev3", "From 2:30pm-3:30pm", [10, 60, 105, 75], 4),
                    token("paddleocr_ppstructurev3", "TRIVIA CHALLENGE", [10, 80, 90, 92], 5),
                    token("paddleocr_ppstructurev3", "Lobby", [10, 93, 50, 96], 6),
                    token("paddleocr_ppstructurev3", "At 4:00pm", [60, 93, 110, 96], 7),
                    token("rapidocr", "MONDAY", [10, 5, 70, 15], 1),
                    token("rapidocr", "POOL PARTY", [10, 20, 80, 35], 2),
                    token("rapidocr", "Ol' Man Island Pool Deck", [10, 40, 100, 55], 3),
                    token("rapidocr", "From 2:30pm-3:30pm", [10, 60, 105, 75], 4),
                    token("rapidocr", "TRIVIA CHALLENGE", [10, 80, 90, 92], 5),
                    token("rapidocr", "Lobby", [10, 93, 50, 96], 6),
                    token("rapidocr", "At 4:00pm", [60, 93, 110, 96], 7),
                ],
            }

            with patch.object(extract_v3, "DEFAULT_FIELD_CROPS_DIR", field_crops_dir):
                candidates = extract_candidates_from_snapshot(snapshot)
            candidate = candidates[0]

            title_source = candidate["field_evidence"]["title"]["source"]
            self.assertEqual([10, 20, 80, 35], title_source["bbox_px"])
            self.assertNotEqual("verticalrowcrop", title_source["crop_sha256"])
            self.assertTrue(Path(title_source["crop_storage_path"]).exists())
            self.assertIn(str(field_crops_dir / "sourcehash" / "page-001-region-03"), title_source["crop_storage_path"])
            self.assertEqual(title_source["crop_sha256"], hashlib.sha256(Path(title_source["crop_storage_path"]).read_bytes()).hexdigest())
            self.assertEqual("verticalrowcrop", title_source["region_crop_sha256"])
            self.assertEqual("vertical-row.png", title_source["region_crop_storage_path"])
            self.assertEqual(2, len(candidates))
            second_title_source = candidates[1]["field_evidence"]["title"]["source"]
            self.assertNotEqual(title_source["crop_storage_path"], second_title_source["crop_storage_path"])
            self.assertEqual(second_title_source["crop_sha256"], hashlib.sha256(Path(second_title_source["crop_storage_path"]).read_bytes()).hexdigest())
    ...
    def test_extract_v3_uses_title_specific_fee_marker_token_when_title_token_omits_marker(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "COMMUNITY HALL", 5500, 5750, 1),
                token("paddleocr_ppstructurev3", "Daily from 10:00am-10:00pm", 5800, 6000, 2),
                token("paddleocr_ppstructurev3", "COMMUNITY HALL ($) There is a fee associated with this activity.", 15000, 15100, 3),
                token("rapidocr", "COMMUNITY HALL", 5520, 5770, 1),
                token("rapidocr", "Daily from 10:00am-10:00pm", 5820, 6020, 2),
                token("rapidocr", "COMMUNITY HALL ($) There is a fee associated with this activity.", 15020, 15120, 3),
            ],
        }
        ...
        self.assertTrue(candidate["fee_required"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["fee"]["agreement"])
        self.assertTrue(candidate["field_evidence"]["fee"]["normalized_value"])
        self.assertEqual(
            ["($)", "($)"],
            [engine["text"] for engine in candidate["field_evidence"]["fee"]["engines"]],
        )
        self.assertIn("COMMUNITY HALL ($)", candidate["field_evidence"]["fee"]["raw_value"])
        self.assertNotIn("fee_marker_conflict", validate_candidate(candidate)["validation_findings"])
    ...
    def test_extract_v3_aframe_activity_repairs_ocr_o_minutes_before_grouping(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
            "page_number": 1,
            "page_image_sha256": "pagehash",
            "bbox_px": [5000, 12000, 10800, 15500],
            "crop_path": "activities.png",
            "crop_sha256": "activitiescrop",
        }
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "NIGHTTIME POOL PARTY", 13065, 13284, 1),
                token("paddleocr_ppstructurev3", "Beach Pool Deck", 13389, 13543, 2),
                token("paddleocr_ppstructurev3", "Monday, Wednesday, and Friday at 8:OOpm", 13648, 13839, 3),
                token("rapidocr", "NIGHTTIME POOL PARTY", 12803, 13016, 1),
                token("rapidocr", "Beach Pool Deck", 13089, 13277, 2),
                token("rapidocr", "Monday, Wednesday, and Friday at 8:OOpm", 13350, 13538, 3),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("NIGHTTIME POOL PARTY", candidate["source_title"])
        self.assertEqual("Beach Pool Deck", candidate["location_text"])
        self.assertEqual("Monday, Wednesday, and Friday at 8:OOpm", candidate["schedule_raw"])
        self.assertEqual("20:00", candidate["schedule_normalized"]["start_time"])
        self.assertEqual([], candidate["validation_findings"])

    def test_extract_v3_aframe_activity_combines_wrapped_location_before_schedule(self) -> None:
        region = {
            "region_id": "page-001-region-03",
            "region_type": "wellness_section",
            "region_family": "aframe_recreation",
            "page_number": 1,
            "page_image_sha256": "pagehash",
            "bbox_px": [0, 4860, 5400, 8910],
            "crop_path": "wellness.png",
            "crop_sha256": "wellnesscrop",
        }
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "FIND A FRIEND", 5483, 5711, 1),
                token("paddleocr_ppstructurev3", "Throughout Disney's Port Orleans Resort -", 5775, 5958, 2),
                token("paddleocr_ppstructurev3", "French Quarter", 6022, 6201, 3),
                token("paddleocr_ppstructurev3", "Daily from 10:00am-11:00pm", 6265, 6452, 4),
                token("rapidocr", "FIND A FRIEND", 5626, 5838, 1),
                token("rapidocr", "Throughout Disney's Port Orleans Resort -", 5904, 6067, 2),
                token("rapidocr", "French Quarter", 6124, 6312, 3),
                token("rapidocr", "Daily from 10:00am-11:00pm", 6353, 6540, 4),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("FIND A FRIEND", candidate["source_title"])
        self.assertEqual("Throughout Disney's Port Orleans Resort - French Quarter", candidate["location_text"])
        self.assertEqual("throughout_disneys_port_orleans_resort_french_quarter", candidate["normalized_location_id"])
        self.assertEqual("Daily from 10:00am-11:00pm", candidate["schedule_raw"])
        self.assertEqual("10:00", candidate["schedule_normalized"]["start_time"])
        validated = validate_candidate(candidate)
        self.assertNotIn("engine_disagreement:location", validated["validation_findings"])

    def test_extract_v3_aframe_activity_combines_wrapped_secondary_location_for_agreement(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
            "page_number": 1,
            "page_image_sha256": "pagehash",
            "bbox_px": [5200, 11800, 10600, 14300],
            "crop_path": "movie.png",
            "crop_sha256": "moviecrop",
        }
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "CAMPFIRE", 12240, 12430, 1),
                token("paddleocr_ppstructurev3", "Courtyard Campfire Pit between Contemporary Tower", 12520, 12680, 2),
                token("paddleocr_ppstructurev3", "and Bay Lake Tower", 12715, 12880, 3),
                token("paddleocr_ppstructurev3", "Nightly from 6:30pm-7:30pm", 12970, 13140, 4),
                token("rapidocr", "CAMPFIRE", 12250, 12430, 1),
                token("rapidocr", "Courtyard Campfire Pit between Contemporary Tower", 12520, 12680, 2),
                token("rapidocr", "and Bay Lake Tower", 12715, 12880, 3),
                token("rapidocr", "Nightly from 6:30pm-7:30pm", 12970, 13140, 4),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("CAMPFIRE", candidate["source_title"])
        self.assertEqual(
            "Courtyard Campfire Pit between Contemporary Tower and Bay Lake Tower",
            candidate["location_text"],
        )
        self.assertEqual(
            "courtyard_campfire_pit_between_contemporary_tower_and_bay_lake_tower",
            candidate["normalized_location_id"],
        )
        validated = validate_candidate(candidate)
        self.assertNotIn("engine_disagreement:location", validated["validation_findings"])
        self.assertNotIn("unknown_location", validated["validation_findings"])

    def test_extract_v3_aframe_activity_does_not_use_day_list_as_secondary_location(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "NIGHTTIME TRIVIA", 10153, 10380, 1),
                token("paddleocr_ppstructurev3", "Riviera Courtyard", 10437, 10615, 2),
                token("paddleocr_ppstructurev3", "Sunday, Monday, Tuesday, Wednesday, Thursday and Friday at 8:00pm", 10660, 10830, 3),
                token("rapidocr", "NIGHTTIME TRIVIA", 10027, 10239, 1),
                token("rapidocr", "Riviera Courtyard", 10280, 10468, 2),
                token("rapidocr", "Sunday, Monday, Tuesday, Wednesday, Thursday", 10510, 10680, 3),
                token("rapidocr", "and Friday at 8:00pm", 10710, 10890, 4),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("Riviera Courtyard", candidate["location_text"])
        self.assertEqual("riviera_courtyard", candidate["normalized_location_id"])
        validated = validate_candidate(candidate)
        self.assertNotIn("engine_disagreement:location", validated["validation_findings"])
        self.assertNotIn("unknown_location", validated["validation_findings"])

    def test_extract_v3_aframe_activity_combines_wrapped_secondary_day_list_schedule(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "MICKEY TIE-DYE ($)", 5800, 6000, 1),
                token("paddleocr_ppstructurev3", "Outside of Community Hall", 6060, 6250, 2),
                token("paddleocr_ppstructurev3", "Sunday, Tuesday, Wednesday, Friday and Saturday from 2:30pm-3:30pm", 6320, 6700, 3),
                token("rapidocr", "MICKEY TIE-DYE ($)", 5800, 6000, 1),
                token("rapidocr", "Outside of Community Hall", 6060, 6250, 2),
                token("rapidocr", "Sunday, Tuesday, Wednesday,", 6320, 6480, 3),
                token("rapidocr", "Friday and Saturday from 2:30pm-3:30pm", 6510, 6700, 4),
            ],
        }
        ...
        self.assertEqual(
            "Sunday, Tuesday, Wednesday, Friday and Saturday from 2:30pm-3:30pm",
            candidate["schedule_raw"],
        )
        validated = validate_candidate(candidate)
        self.assertNotIn("engine_disagreement:schedule", validated["validation_findings"])

    def test_extract_v3_aframe_activity_does_not_merge_day_schedule_into_location(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "POOLSIDE ACTIVITIES", 8500, 8700, 1),
                token("paddleocr_ppstructurev3", "Doubloon Lagoon Pool Deck", 8750, 8950, 2),
                token("paddleocr_ppstructurev3", "Sunday, Monday, Tuesday, Thursday, Friday and Saturday from 12:30pm-2:00pm", 9000, 9200, 3),
                token("rapidocr", "POOLSIDE ACTIVITIES", 8500, 8700, 1),
                token("rapidocr", "Doubloon Lagoon Pool Deck", 8750, 8950, 2),
                token("rapidocr", "Sunday, Monday, Tuesday, Thursday, Friday and Saturday from 12:30pm-2:00pm", 9000, 9200, 3),
            ],
        }
        ...
        self.assertEqual("POOLSIDE ACTIVITIES", candidate["source_title"])
        self.assertEqual("Doubloon Lagoon Pool Deck", candidate["location_text"])
        self.assertEqual("Sunday, Monday, Tuesday, Thursday, Friday and Saturday from 12:30pm-2:00pm", candidate["schedule_raw"])
        self.assertEqual("doubloon_lagoon_pool_deck", candidate["normalized_location_id"])
        self.assertEqual([], candidate["validation_findings"])

    def test_extract_v3_aframe_activity_infers_location_from_title_when_schedule_follows_title(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "SAFARI SO GOOD ARCADE ($)", 5400, 5620, 1),
                token("paddleocr_ppstructurev3", "Daily from 8:00am-11:00pm", 5680, 5850, 2),
                token("paddleocr_ppstructurev3", "Put your skills to the test.", 5900, 6080, 3),
                token("paddleocr_ppstructurev3", "($) There is a fee associated with this activity.", 6500, 6650, 4),
                token("rapidocr", "SAFARI SO GOOD ARCADE ($)", 5400, 5620, 1),
                token("rapidocr", "Daily from 8:00am-11:00pm", 5680, 5850, 2),
                token("rapidocr", "Put your skills to the test.", 5900, 6080, 3),
                token("rapidocr", "($) There is a fee associated with this activity.", 6500, 6650, 4),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("SAFARI SO GOOD ARCADE ($)", candidate["source_title"])
        self.assertEqual("SAFARI SO GOOD ARCADE ($)", candidate["location_text"])
        self.assertEqual("safari_so_good_arcade", candidate["normalized_location_id"])
        self.assertEqual("Daily from 8:00am-11:00pm", candidate["schedule_raw"])
        self.assertEqual([], candidate["validation_findings"])
        validated = validate_candidate(candidate)
        self.assertNotIn("unknown_location", validated["validation_findings"])
        self.assertNotIn("engine_disagreement:location", validated["validation_findings"])

    def test_extract_v3_aframe_activity_prefers_secondary_title_for_inferred_location(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "PUMBAA'S FUN AND GAMES ARCADE ($)", 5400, 5620, 1),
                token("paddleocr_ppstructurev3", "Daily from 8:00am-11:00pm", 5680, 5850, 2),
                token("paddleocr_ppstructurev3", "($) There is a fee associated with this activity.", 6500, 6650, 3),
                token("rapidocr", "PUMBAA'S FUN AND GAMES ARCADE ($)", 5400, 5620, 1),
                token("rapidocr", "A", 5460, 5500, 2),
                token("rapidocr", "Daily from 8:00am-11:00pm", 5680, 5850, 3),
                token("rapidocr", "($) There is a fee associated with this activity.", 6500, 6650, 4),
            ],
        }
        ...
        location_evidence = candidate["field_evidence"]["location"]
        self.assertEqual("PUMBAA'S FUN AND GAMES ARCADE ($)", candidate["location_text"])
        self.assertEqual("pumbaa_s_fun_and_games_arcade", candidate["normalized_location_id"])
        self.assertEqual("exact_after_normalization", location_evidence["agreement"])
        self.assertEqual(
            ["PUMBAA'S FUN AND GAMES ARCADE ($)", "PUMBAA'S FUN AND GAMES ARCADE ($)"],
            [engine["text"] for engine in location_evidence["engines"]],
        )
        validated = validate_candidate(candidate)
        self.assertNotIn("unknown_location", validated["validation_findings"])
        self.assertNotIn("engine_disagreement:location", validated["validation_findings"])

    def test_extract_v3_aframe_activity_accepts_open_twenty_four_hour_schedule(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "FITNESS CENTER", 5400, 5620, 1),
                token("paddleocr_ppstructurev3", "Open 24 hours a day, 7 days a week", 5680, 5850, 2),
                token("rapidocr", "FITNESS CENTER", 5400, 5620, 1),
                token("rapidocr", "Open 24 hours a day, 7 days a week", 5680, 5850, 2),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("FITNESS CENTER", candidate["source_title"])
        self.assertEqual("Open 24 hours a day, 7 days a week", candidate["schedule_raw"])
        self.assertEqual("recurring", candidate["schedule_normalized"]["schedule_type"])
        self.assertEqual(ALL_DAYS, candidate["schedule_normalized"]["days_of_week"])
        self.assertEqual("00:00", candidate["schedule_normalized"]["start_time"])
        self.assertEqual("23:59", candidate["schedule_normalized"]["end_time"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["schedule"]["agreement"])

    def test_extract_v3_aframe_activity_recombines_split_secondary_title_tokens(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "ARTS & CRAFTS ($)", [5650, 5510, 8430, 5750], 1),
                token("paddleocr_ppstructurev3", "Fantasia Pool Deck", [5650, 5800, 7270, 5975], 2),
                token(
                ...
                token("paddleocr_ppstructurev3", "from 3:30pm-4:30pm", [5650, 6280, 7485, 6460], 4),
                token("rapidocr", "ARTS", [5630, 5675, 6365, 5895], 1),
                token("rapidocr", "&", [6405, 5700, 6615, 5880], 2),
                token("rapidocr", "CRAFTS ($)", [6655, 5665, 8240, 5915], 3),
                token("rapidocr", "Fantasia Pool Deck", [5625, 5945, 7170, 6130], 4),
                token(
                    "rapidocr",
                    "Sunday, Monday, Wednesday, Thursday and Friday",
                    [5620, 6165, 9595, 6355],
                ...
                token("rapidocr", "from 3:30pm-4:30pm", [5620, 6385, 7375, 6600], 6),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("ARTS & CRAFTS ($)", candidate["source_title"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["title"]["agreement"])
        self.assertEqual(
            ["ARTS & CRAFTS ($)", "ARTS & CRAFTS ($)"],
            [engine["text"] for engine in candidate["field_evidence"]["title"]["engines"]],
        )
        self.assertTrue(candidate["fee_required"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["fee"]["agreement"])
        self.assertNotIn("engine_disagreement:title", validate_candidate(candidate)["validation_findings"])
        self.assertNotIn("engine_disagreement:fee", validate_candidate(candidate)["validation_findings"])

    def test_extract_v3_aframe_activity_prefers_exact_secondary_title_over_nearby_header_fragment(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "CONCH FLATS COMMUNITY HALL", [5709, 2908, 10402, 3155], 1),
                token("paddleocr_ppstructurev3", "Hospitality House", [5709, 3195, 7194, 3378], 2),
                token("paddleocr_ppstructurev3", "Daily from 10:00am-10:00pm", [5704, 3422, 8194, 3621], 3),
                token("rapidocr", "RESOR", [5600, 2850, 6500, 3050], 1),
                token("rapidocr", "CONCH FLATS COMMUNITY HALL", [5665, 3176, 10053, 3389], 2),
                token("rapidocr", "Hospitality House", [5665, 3400, 7100, 3600], 3),
                token("rapidocr", "Daily from 10:00am-10:00pm", [5665, 3650, 8020, 3838], 4),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("CONCH FLATS COMMUNITY HALL", candidate["source_title"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["title"]["agreement"])
        self.assertEqual(
            ["CONCH FLATS COMMUNITY HALL", "CONCH FLATS COMMUNITY HALL"],
            [engine["text"] for engine in candidate["field_evidence"]["title"]["engines"]],
        )
        self.assertNotIn("engine_disagreement:title", validate_candidate(candidate)["validation_findings"])

    def test_extract_v3_aframe_activity_combines_day_list_with_time_range_schedule(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "tokens": [
                token("paddleocr_ppstructurev3", "POOLSIDE ACTIVITIES", 8500, 8700, 1),
                token("paddleocr_ppstructurev3", "Doubloon Lagoon Pool Deck", 8750, 8950, 2),
                token("paddleocr_ppstructurev3", "Sunday, Monday, Tuesday, Thursday, Friday and Saturday", 9000, 9200, 3),
                token("paddleocr_ppstructurev3", "from 12:30pm-2:00pm", 9250, 9450, 4),
                token("rapidocr", "POOLSIDE ACTIVITIES", 8500, 8700, 1),
                token("rapidocr", "Doubloon Lagoon Pool Deck", 8750, 8950, 2),
                token("rapidocr", "Sunday, Monday, Tuesday, Thursday, Friday and Saturday", 9000, 9200, 3),
                token("rapidocr", "from 12:30pm-2:00pm", 9250, 9450, 4),
            ],
        }
        ...
        self.assertEqual("Doubloon Lagoon Pool Deck", candidate["location_text"])
        self.assertEqual(
            "Sunday, Monday, Tuesday, Thursday, Friday and Saturday from 12:30pm-2:00pm",
            candidate["schedule_raw"],
        )
        self.assertEqual("12:30", candidate["schedule_normalized"]["start_time"])
        self.assertEqual("14:00", candidate["schedule_normalized"]["end_time"])
        self.assertEqual([], candidate["validation_findings"])
    ...
    def test_extract_v3_aframe_movie_rows_derive_field_evidence_from_unhinted_ocr_tokens(self) -> None:
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "MOVIEUNDER THE STARS", 12400, 12650, 1),
                ...
                token("rapidocr", "MOVIE UNDER THE STARS", 13020, 13270, 1),
        ...
        candidate = candidates[0]
        self.assertEqual("movie", candidate["candidate_type"])
        self.assertEqual("MOVIEUNDER THE STARS", candidate["source_title"])
    ...
    def test_extract_v3_movie_section_fallback_does_not_infer_movie_header_as_location(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "MOVIE UNDER THE STARS", 11700, 11900, 1),
                token("paddleocr_ppstructurev3", "Thursday at 8:30pm", 12000, 12200, 2),
                token("rapidocr", "UNDER THE STARS", 11720, 11920, 1),
                token("rapidocr", "Thursday at 8:30pm", 12020, 12220, 2),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)
        self.assertEqual("needs_review", candidate["validation_status"])
        self.assertEqual(["field_extraction_not_implemented"], candidate["validation_findings"])
        self.assertIsNone(candidate["normalized_location_id"])
        self.assertNotIn("location", candidate["field_evidence"])

    def test_extract_v3_movie_section_activity_keeps_primary_title_when_secondary_misses_it(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "runtime_lineage": {
                ...
                "package_versions": {"python": "3"},
                "model_asset_hashes": {"model": "hash"},
                "ocr_engine_configs": {"paddleocr_ppstructurev3": {}, "rapidocr": {}},
            },
            "document_family": "aframe_recreation",
            "source_document_id": "source-doc-1",
            "source_sha256": "sourcehash",
            "canonical_url": "https://example.com/source.pdf",
            "source_kind": "official_pdf",
            "http_status": 200,
            "currentness": "current",
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "PICKLEBALL CLINIC", 11700, 11900, 1),
                token("paddleocr_ppstructurev3", "BoardWalk Tennis and Pickleball Court", 12300, 12500, 2),
                token("paddleocr_ppstructurev3", "Friday from 9:00am-10:00am", 12600, 12800, 3),
                token("rapidocr", "BoardWalk Tennis and Pickleball Court", 12320, 12520, 1),
                token("rapidocr", "Friday from 9:00am-10:00am", 12620, 12820, 2),
            ],
        }
        ...
        self.assertEqual("activity", candidate["candidate_type"])
        self.assertEqual("PICKLEBALL CLINIC", candidate["source_title"])
        self.assertEqual("BoardWalk Tennis and Pickleball Court", candidate["location_text"])
        self.assertEqual("Friday from 9:00am-10:00am", candidate["schedule_raw"])
        self.assertIn("title", candidate["field_evidence"])
        self.assertEqual(1, len(candidate["field_evidence"]["title"]["engines"]))
        validated = validate_candidate(candidate)
        self.assertEqual("needs_review", validated["validation_status"])
        self.assertIn("missing_field_engine_evidence:title", validated["validation_findings"])

    def test_extract_v3_movie_section_keeps_partial_movie_rows_for_review(self) -> None:
        region = {
            "region_id": "page-001-region-05",
            "region_type": "movie_section",
            "region_family": "aframe_recreation",
            "page_number": 1,
            "page_image_sha256": "pagehash",
            "bbox_px": [0, 8900, 5400, 15552],
            "crop_path": "movies.png",
            "crop_sha256": "moviescrop",
        }

        def token(engine: str, text: str, x1: int, y1: int, x2: int, y2: int, order: int) -> dict[str, Any]:
            return {
                "engine": engine,
                "text": text,
                "page_number": 1,
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "Thursday.", 400, 10000, 1200, 10200, 1),
                token("paddleocr_ppstructurev3", "Frozen PG", 1700, 10000, 3200, 10200, 2),
                token("rapidocr", "Thursday", 420, 10020, 1200, 10220, 1),
                token("rapidocr", ".Frozen PG", 1720, 10020, 3200, 10220, 2),
            ],
        }
        ...
        self.assertEqual("movie", candidate["candidate_type"])
        self.assertEqual("Frozen", candidate["movie_title"])
        self.assertEqual("Thursday", candidate["schedule_raw"])
        self.assertNotIn("field_extraction_not_implemented", validated["validation_findings"])
        self.assertEqual("needs_review", validated["validation_status"])
        self.assertIn("missing_required_field:location", validated["validation_findings"])

    def test_extract_v3_activity_row_splits_location_pipe_time_before_title_location_fallback(self) -> None:
        region = {
            "region_id": "page-001-region-03",
            "region_type": "wellness_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "MOVIE UNDER THE STARS", 7310, 7615, 1),
                token("paddleocr_ppstructurev3", "SHIPWRECK BEACH | 8:3OPM", 7800, 8000, 2),
                token("rapidocr", "UNDER THE STARS", 7315, 7620, 1),
                token("rapidocr", "SHIPWRECK BEACH | 8:30PM", 7810, 8010, 2),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("shipwreck_beach", candidate["normalized_location_id"])
        self.assertEqual("SHIPWRECK BEACH", candidate["location_text"])
        self.assertEqual("8:3OPM", candidate["schedule_raw"])
        self.assertEqual("20:30", candidate["schedule_normalized"]["start_time"])
        self.assertNotIn("inferred_location_from_title", candidate["field_evidence"]["location"])

    def test_extract_v3_activity_row_does_not_split_daily_schedule_as_location(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "SIDE SHOW GAMES ARCADE ($)", 2900, 3150, 1),
                token("paddleocr_ppstructurev3", "Daily from 8:00am-11:00pm", 3300, 3500, 2),
                token("rapidocr", "SIDE SHOW GAMES ARCADE ($)", 2910, 3160, 1),
                token("rapidocr", "Daily from 8:00am-11:00pm", 3310, 3510, 2),
            ],
        }

        [candidate] = extract_candidates_from_snapshot(snapshot)

        self.assertEqual("side_show_games_arcade", candidate["normalized_location_id"])
        self.assertEqual("SIDE SHOW GAMES ARCADE ($)", candidate["location_text"])
        self.assertTrue(candidate["field_evidence"]["location"]["inferred_location_from_title"])
        self.assertEqual("Daily from 8:00am-11:00pm", candidate["schedule_raw"])

    def test_extract_v3_activity_row_combines_semicolon_schedule_continuation(self) -> None:
        region = {
            "region_id": "page-001-region-04",
            "region_type": "resort_activities_section",
            "region_family": "aframe_recreation",
        ...
        snapshot = {
            ...
            "regions": [region],
            "tokens": [
                token("paddleocr_ppstructurev3", "THE EVENTI ROOM ($)", 6326, 6585, 1),
                token("paddleocr_ppstructurev3", "Sunday, Tuesday and Thursday from 1:00pm-10:00pm;", 6614, 6800, 2),
                token("paddleocr_ppstructurev3", "Monday, Wednesday, Friday and Saturday", 6849, 7027, 3),
                token("paddleocr_ppstructurev3", "from 10:00am-10:00pm", 7071, 7246, 4),
                token("rapidocr", "THE EVENTI ROOM ($)", 6410, 6655, 1),
                token("rapidocr", "Sunday, Tuesday and Thursday from 1:00pm-10:00pm;", 6679, 6867, 2),
                token("rapidocr", "Monday, Wednesday, Friday and Saturday", 6875, 7079, 3),
                token("rapidocr", "from 10:00am-10:00pm", 7096, 7283, 4),
            ],
        }
        ...
        expected_schedule = (
            "Sunday, Tuesday and Thursday from 1:00pm-10:00pm; "
            "Monday, Wednesday, Friday and Saturday from 10:00am-10:00pm"
        )
        self.assertEqual(expected_schedule, candidate["schedule_raw"])
        self.assertEqual("exact_after_normalization", candidate["field_evidence"]["schedule"]["agreement"])
        self.assertEqual(
            [expected_schedule, expected_schedule],
            [engine["text"] for engine in candidate["field_evidence"]["schedule"]["engines"]],
        )
    ...
    def test_extract_v3_batch_scans_vision_snapshots_and_writes_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("v3_extract_001", report["schema_version"])
            self.assertEqual(1, report["summary"]["snapshot_count"])
    ...
    def test_title_agreement_allows_observed_find_a_friend_clipped_ocr(self) -> None:
        clipped = evaluate_field_agreement(
            "title",
            primary_text="FIND A FRIEND",
            secondary_text="FIND",
        )
        punctuation_spacing = evaluate_field_agreement(
            "title",
            primary_text="TOUCHDOWN!CAMPFIRE",
            secondary_text="TOUCHDOWN! CAMPFIRE",
        )
        curly_apostrophe = evaluate_field_agreement(
            "title",
            primary_text="SORCERER'S CAMPFIRE",
            secondary_text="SORCERER\u2019S CAMPFIRE",
        )
        unrelated = evaluate_field_agreement(
            "title",
            primary_text="ART & ARCHITECTURE TOUR",
            secondary_text="WONDERS OF THE LODGE",
        )

        self.assertEqual("exact_after_normalization", clipped["agreement"])
        self.assertEqual("find a friend", clipped["normalized_value"])
        self.assertEqual("exact_after_normalization", punctuation_spacing["agreement"])
        self.assertEqual("touchdown! campfire", punctuation_spacing["normalized_value"])
        self.assertEqual("exact_after_normalization", curly_apostrophe["agreement"])
        self.assertEqual("sorcerer's campfire", curly_apostrophe["normalized_value"])
        self.assertEqual("disagreement", unrelated["agreement"])

    def test_agreement_repairs_observed_character_greeting_ocr_clips(self) -> None:
        clipped_title = evaluate_field_agreement(
            "title",
            primary_text="DISNEY CHARACTER GREETINGS",
            secondary_text="DISNEY CHARACTER",
        )
        noisy_schedule = evaluate_field_agreement(
            "schedule",
            primary_text="Thursday at From 9:30am-9:50am and 2:40pm-3:00pm",
            secondary_text="Thursday at Fro* 9.30am-9:500m a1d 2:40pm-3:00pm",
        )

        self.assertEqual("exact_after_normalization", clipped_title["agreement"])
        self.assertEqual("disney character greetings", clipped_title["normalized_value"])
        self.assertEqual("exact_after_normalization", noisy_schedule["agreement"])
        self.assertEqual("09:30", noisy_schedule["normalized_value"]["start_time"])
        self.assertEqual("09:50", noisy_schedule["normalized_value"]["end_time"])
    ...
    def test_validate_v3_fails_closed_for_missing_evidence_unknowns_and_disagreement(self) -> None:
        ...
        secondary_schedule_garbage = json.loads(json.dumps(valid_candidate))
        secondary_schedule_garbage["field_evidence"]["schedule"]["agreement"] = "disagreement"
        secondary_schedule_garbage["field_evidence"]["schedule"]["engines"][0]["text"] = "Monday at 1:30pm"
        secondary_schedule_garbage["field_evidence"]["schedule"]["engines"][0]["confidence"] = 0.999
        secondary_schedule_garbage["field_evidence"]["schedule"]["engines"][1]["text"] = "OMan Isiznrl"
        secondary_schedule_garbage["field_evidence"]["schedule"]["engines"][1]["confidence"] = 0.77
        secondary_schedule_result = validate_candidate(secondary_schedule_garbage)
        self.assertEqual("auto_publishable", secondary_schedule_result["validation_status"])
        self.assertNotIn("engine_disagreement:schedule", secondary_schedule_result["validation_findings"])

        secondary_known_schedule_disagreement = json.loads(json.dumps(secondary_schedule_garbage))
        secondary_known_schedule_disagreement["field_evidence"]["schedule"]["engines"][1]["text"] = "Monday at 2:30pm"
        secondary_known_schedule_result = validate_candidate(secondary_known_schedule_disagreement)
        self.assertEqual("needs_review", secondary_known_schedule_result["validation_status"])
        self.assertIn("engine_disagreement:schedule", secondary_known_schedule_result["validation_findings"])

        secondary_location_garbage = json.loads(json.dumps(valid_candidate))
        secondary_location_garbage["location_text"] = "Ol' Man Island Pool Deck"
        secondary_location_garbage["normalized_location_id"] = "ol_man_island_pool_deck"
        secondary_location_garbage["field_evidence"]["location"]["raw_value"] = "Ol' Man Island Pool Deck"
        secondary_location_garbage["field_evidence"]["location"]["normalized_value"] = "ol_man_island_pool_deck"
        secondary_location_garbage["field_evidence"]["location"]["agreement"] = "disagreement"
        secondary_location_garbage["field_evidence"]["location"]["engines"][0]["text"] = "Ol' Man Island Pool Deck"
        secondary_location_garbage["field_evidence"]["location"]["engines"][0]["confidence"] = 0.998
        secondary_location_garbage["field_evidence"]["location"]["engines"][1]["text"] = "O. Man Ielanel Facl Derk"
        secondary_location_garbage["field_evidence"]["location"]["engines"][1]["confidence"] = 0.78
        secondary_location_garbage["source_spans"]["location"] = [{"page": 1, "text": "Ol' Man Island Pool Deck"}]
        secondary_location_result = validate_candidate(secondary_location_garbage)
        self.assertEqual("auto_publishable", secondary_location_result["validation_status"])
        self.assertNotIn("engine_disagreement:location", secondary_location_result["validation_findings"])

        secondary_known_location_disagreement = json.loads(json.dumps(secondary_location_garbage))
        secondary_known_location_disagreement["field_evidence"]["location"]["engines"][1]["text"] = "Village Green Lawn"
        secondary_known_location_result = validate_candidate(secondary_known_location_disagreement)
        self.assertEqual("needs_review", secondary_known_location_result["validation_status"])
        self.assertIn("engine_disagreement:location", secondary_known_location_result["validation_findings"])
        ...
        extraction_gap = json.loads(json.dumps(valid_candidate))
        extraction_gap["source_title"] = None
        extraction_gap["normalized_title"] = None
        extraction_gap["location_text"] = None
        extraction_gap["normalized_location_id"] = None
        extraction_gap["schedule_raw"] = None
        extraction_gap["schedule_normalized"] = None
        extraction_gap["fee_required"] = None
        extraction_gap["validation_findings"] = ["field_extraction_not_implemented"]
        extraction_gap["field_evidence"] = {"region": valid_candidate["field_evidence"]["region"]}
        extraction_gap["source_spans"] = {}
        validated_extraction_gap = validate_candidate(extraction_gap)
        self.assertEqual("rejected", validated_extraction_gap["validation_status"])
        self.assertIn("missing_reviewable_field_evidence", validated_extraction_gap["validation_findings"])
    ...
    def test_validate_v3_batch_scans_candidate_files_and_writes_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("v3_validate_001", report["schema_version"])
            self.assertEqual(1, report["summary"]["candidate_count"])
    ...
    def test_validate_v3_scoped_run_preserves_non_matching_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            candidates_dir = root / "candidates"
            output_dir = root / "validated"
            ...
            boardwalk_candidate = clean_publish_v3_preview()["rows"][0]
            boardwalk_candidate["candidate_type"] = "activity"
            boardwalk_candidate["edition"] = "official-schedule-0526"
            boardwalk_candidate["calendar_group_key"] = "boardwalk"
            boardwalk_candidate["content_sha256"] = "boardwalkhash"
            boardwalk_candidate["source_sha256"] = "boardwalkhash"
            (candidates_dir / "boardwalkhash.candidates.json").write_text(json.dumps([boardwalk_candidate]))

            pop_candidate = json.loads(json.dumps(boardwalk_candidate))
            pop_candidate["calendar_group_key"] = "pop-century"
            pop_candidate["content_sha256"] = "pophash"
            pop_candidate["source_sha256"] = "pophash"
            (candidates_dir / "pophash.candidates.json").write_text(json.dumps([pop_candidate]))

            boardwalk_validated_path = output_dir / "boardwalkhash.candidates.json"
            boardwalk_sentinel = [{"candidate_id": "keep-me"}]
            boardwalk_validated_path.write_text(json.dumps(boardwalk_sentinel))
            ...
            self.assertEqual(boardwalk_sentinel, json.loads(boardwalk_validated_path.read_text()))
            self.assertTrue((output_dir / "pophash.candidates.json").exists())
            self.assertEqual(1, report["summary"]["candidate_count"])
            self.assertEqual(1, len(report["results"]))
            self.assertIn("pophash.candidates.json", report["results"][0]["candidates_path"])
    ...
    def test_review_queue_v3_turns_blocked_candidates_into_source_hash_bound_tasks(self) -> None:
        candidate = {
            ...
            "runtime_lineage": {
                ...
                "package_versions": {"pypdfium2": "4.30.0"},
                "model_asset_hashes": {"paddleocr_ppstructurev3": "modelhash"},
                "ocr_engine_configs": {"paddleocr_ppstructurev3": {"mode": "ppstructurev3"}},
            },
            "field_evidence": {
                "region": {
                    "source": {
        ...
        self.assertEqual("https://example.test/boardwalk.pdf", tasks[0]["source_url"])
        self.assertEqual("region.png", tasks[0]["field_crop"])
        self.assertEqual("vision_v3_001", tasks[0]["pipeline_version"])
        self.assertEqual("confighash", tasks[0]["config_hash"])
        self.assertEqual("lineagehash", tasks[0]["runtime_lineage"]["lineage_hash"])
        ...
        self.assertEqual("pending", unreviewable_task["status"])
        self.assertEqual("cand-1", unreviewable_task["task_id"])
        self.assertEqual("sourcehash", unreviewable_task["content_sha256"])
        self.assertEqual("vision_v3_001", unreviewable_task["pipeline_version"])
        self.assertEqual("confighash", unreviewable_task["config_hash"])
        self.assertEqual("lineagehash", unreviewable_task["runtime_lineage"]["lineage_hash"])
    ...
    def test_review_queue_v3_splits_multi_field_ocr_disagreements_into_field_tasks(self) -> None:
        def evidence(field: str, raw: str, normalized: object) -> dict[str, Any]:
            return {
                "field": field,
                "raw_value": raw,
                ...
                "engines": [
                    {"engine": "paddleocr_ppstructurev3", "text": raw, "confidence": 0.97},
                    {"engine": "rapidocr", "text": f"bad {raw}", "confidence": 0.71},
                ],
            }
        ...
        candidate = {
            ...
            "field_evidence": {
                ...
                "title": evidence("title", "Pool Party", "pool party"),
                "schedule": evidence("schedule", "Daily at 1:30pm", {"start_time": "13:30"}),
                "location": evidence("location", "Luna Park Pool Deck", "luna_park_pool_deck"),
            },
        }

        tasks = build_review_tasks([candidate])

        self.assertEqual(["location", "schedule", "title"], sorted(task["field"] for task in tasks))
        self.assertEqual(
            [
                ["engine_disagreement:location"],
                ["engine_disagreement:schedule"],
            ...
            sorted((task["validation_findings"] for task in tasks), key=lambda findings: findings[0]),
        )
        self.assertEqual(
            ["cand-multi-location", "cand-multi-schedule", "cand-multi-title"],
            sorted(task["task_id"] for task in tasks),
        )
        self.assertEqual(["cand-multi", "cand-multi", "cand-multi"], sorted(task["candidate_id"] for task in tasks))
        self.assertTrue(all(task["status"] == "pending" for task in tasks))

    def test_review_queue_v3_marks_hash_current_review_decisions_in_tasks_and_report(self) -> None:
        candidate = {
            ...
            "field_evidence": {
                "schedule": {
                    ...
                    "engines": [{"text": "Daily at 1:30pm"}, {"text": "Daily at 7:30pm"}],
                }
            },
        }
        review_task = build_review_tasks([candidate])[0]
        decision = build_review_decision(
            review_task,
            reviewer="josh",
            decision="approve",
            approved_fields={"schedule": {"normalized_value": {"start_time": "13:30"}}},
            reason="Schedule time is visible in the rendered source crop.",
            decided_at="2026-06-28T12:05:00Z",
        )
        legacy_partial_decision = {
            key: value
            for key, value in decision.items()
            if key not in {"schema_version", "approved_fields", "original_task"}
        }
        stale_decision = {**decision, "task_id": "cand-review-location", "content_sha256": "oldhash"}
        placeholder_reason_decision = {
            **decision,
            "reason": "Verified against source crop",
            "decided_at": "2026-06-28T12:06:00Z",
        }

        tasks = build_review_tasks([candidate], review_decisions=[stale_decision, decision])
        report = build_review_queue_report(tasks)
        html = build_review_queue_html(tasks, report)
        placeholder_tasks = build_review_tasks([candidate], review_decisions=[placeholder_reason_decision])
        legacy_partial_tasks = build_review_tasks([candidate], review_decisions=[legacy_partial_decision])

        self.assertEqual("reviewed", tasks[0]["status"])
        self.assertEqual("approve", tasks[0]["review_decision_status"])
        self.assertEqual("josh", tasks[0]["review_decision"]["reviewer"])
        self.assertEqual("pending", placeholder_tasks[0]["status"])
        self.assertEqual(["placeholder_review_reason"], placeholder_tasks[0]["stale_review_decision"]["stale_reasons"])
        self.assertEqual("pending", legacy_partial_tasks[0]["status"])
        self.assertEqual(
            ["missing_review_decision_schema_version", "missing_approved_fields"],
            legacy_partial_tasks[0]["stale_review_decision"]["stale_reasons"],
        )
        self.assertEqual(1, report["summary"]["reviewed_task_count"])
        self.assertEqual(0, report["summary"]["pending_task_count"])
        self.assertEqual({"approve": 1}, report["summary"]["review_count_by_decision"])
        self.assertEqual([], report["top_calendar_group_blockers"])
        self.assertIn("Status: <strong>reviewed</strong>", html)
        self.assertIn("Review decision: <strong>approve</strong>", html)

    def test_review_queue_v3_keeps_stale_page_or_crop_review_decisions_pending(self) -> None:
        candidate = {
            "candidate_id": "cand-review",
            "validation_status": "needs_review",
            "validation_findings": ["engine_disagreement:schedule"],
            ...
            "field_evidence": {
                "schedule": {
                    ...
                    "source": {
                        "content_sha256": "sourcehash",
                        "page_number": 1,
                        "page_image_sha256": "pagehash-current",
                        "bbox_px": [0, 0, 400, 300],
                        "crop_storage_path": "schedule.png",
                        "crop_sha256": "crop-current",
                    },
                    "engines": [{"text": "Daily at 1:30pm"}, {"text": "Daily at 7:30pm"}],
                }
            },
        ...
        stale_page_decision = {
            ...
            "candidate_id": "cand-review",
            "content_sha256": "sourcehash",
            "page_image_sha256": "pagehash-old",
            "field_crop_sha256": "crop-current",
            "reviewer": "josh",
            "decision": "approve",
            "approved_fields": {"schedule": {"normalized_value": {"start_time": "13:30"}}},
            "reason": "Schedule time is visible in the rendered source crop.",
        ...
        stale_crop_decision = {
            **stale_page_decision,
            "page_image_sha256": "pagehash-current",
            "field_crop_sha256": "crop-old",
            "decided_at": "2026-06-28T12:06:00Z",
        }
        missing_crop_hash_decision = {
            **stale_page_decision,
            "page_image_sha256": "pagehash-current",
            "field_crop_sha256": None,
            "decided_at": "2026-06-28T12:07:00Z",
        }

        tasks = build_review_tasks(
            [candidate],
            review_decisions=[stale_page_decision, stale_crop_decision, missing_crop_hash_decision],
        )
        report = build_review_queue_report(tasks)
        html = build_review_queue_html(tasks, report)

        self.assertEqual("pending", tasks[0]["status"])
        self.assertNotIn("review_decision_status", tasks[0])
        self.assertEqual(
            {
                "decision": "approve",
                "reviewer": "josh",
                ...
                "page_image_sha256": "pagehash-current",
                "field_crop_sha256": None,
                "stale_reasons": ["missing_review_field_crop_sha256"],
            },
            tasks[0]["stale_review_decision"],
        )
        self.assertEqual(1, report["summary"]["pending_task_count"])
        self.assertEqual(0, report["summary"]["reviewed_task_count"])
        self.assertEqual(1, report["summary"]["stale_review_decision_count"])
        self.assertEqual(["cand-review"], report["stale_review_decision_task_ids"])
        self.assertIn("Stale Review Decision", html)
        self.assertIn("missing_review_field_crop_sha256", html)

    def test_review_queue_v3_report_tracks_publish_gate_blocker_progress(self) -> None:
        tasks = [
            {
                "task_id": "diff-reviewed",
                "task_type": "gold_conflict",
                "status": "reviewed",
                "review_decision_status": "approve",
                "calendar_group_key": "boardwalk",
                "field": "schedule",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
            },
            {
                "task_id": "diff-pending",
                "task_type": "gold_conflict",
                "status": "pending",
                "calendar_group_key": "boardwalk",
                "field": "location",
            ...
            },
            {
                ...
                "source_metadata": {"canonical_url": "https://example.test/pop.jpg"},
            },
            {
                "task_id": "ocr-pending",
        ...
        self.assertRegex(report["review_queue_fingerprint"], r"^[0-9a-f]{64}$")
        self.assertEqual(3, report["summary"]["publish_gate_blocker_task_count"])
        self.assertEqual(2, report["summary"]["pending_publish_gate_blocker_task_count"])
        self.assertEqual(1, report["summary"]["reviewed_publish_gate_blocker_task_count"])
        self.assertEqual(2, report["summary"]["publish_blocker_checklist_row_count"])
        self.assertEqual(2, report["summary"]["publish_blocker_checklist_batch_count"])
        self.assertEqual(
            {
                "unreviewed_new_v3_rows": 1,
                "unreviewed_v2_v3_field_diffs": 1,
            },
            report["summary"]["pending_publish_gate_blocker_count_by_reason"],
        )
        self.assertEqual(
            {
                "total_task_count": 3,
                ...
                "pending_task_ids": ["diff-pending", "new-pending"],
                "reviewed_task_ids": ["diff-reviewed"],
                "pending_review_artifact_missing_count": 2,
                "pending_review_artifact_missing_task_ids": ["diff-pending", "new-pending"],
                "all_pending_tasks_have_review_artifacts": False,
                "pending_task_count_by_reason": {
                    "unreviewed_new_v3_rows": 1,
                    "unreviewed_v2_v3_field_diffs": 1,
                },
                "pending_task_ids_by_reason": {
                    "unreviewed_new_v3_rows": ["new-pending"],
                    "unreviewed_v2_v3_field_diffs": ["diff-pending"],
                },
                "pending_task_count_by_source_url": {
                    "https://example.test/boardwalk.pdf": 1,
                    "https://example.test/pop.jpg": 1,
                },
                "pending_task_ids_by_source_url": {
                    "https://example.test/boardwalk.pdf": ["diff-pending"],
                    "https://example.test/pop.jpg": ["new-pending"],
                },
                "pending_task_count_by_source_url_and_reason": {
                    "https://example.test/boardwalk.pdf": {
                        "unreviewed_v2_v3_field_diffs": 1,
                    },
                    "https://example.test/pop.jpg": {
                        "unreviewed_new_v3_rows": 1,
                    },
                },
                "total_task_count_by_reason": {
                    "unreviewed_new_v3_rows": 1,
                    "unreviewed_v2_v3_field_diffs": 2,
                },
            },
            report["publish_gate_blockers"],
        )
        self.assertEqual(
            [
                {
                    "review_batch_key": "https://example.test/boardwalk.pdf | unreviewed_v2_v3_field_diffs",
                    "source_url": "https://example.test/boardwalk.pdf",
                    "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                    "pending_task_count": 1,
                    "calendar_group_keys": ["boardwalk"],
                    "fields": ["location"],
                    "task_ids": ["diff-pending"],
                },
                {
                    "review_batch_key": "https://example.test/pop.jpg | unreviewed_new_v3_rows",
                    "source_url": "https://example.test/pop.jpg",
                    "publish_gate_blocker_reason": "unreviewed_new_v3_rows",
                    "pending_task_count": 1,
                    "calendar_group_keys": ["boardwalk"],
                    "fields": ["new_record"],
                    "task_ids": ["new-pending"],
                },
            ],
            report["publish_blocker_batches"],
        )

    def test_review_queue_v3_report_fingerprint_changes_with_task_content(self) -> None:
        tasks = [
            {
                "task_id": "diff-pending",
                "task_type": "gold_conflict",
                ...
                "calendar_group_key": "boardwalk",
                "field": "location",
                "content_sha256": "current-content",
                "page_image_sha256": "current-page",
                "field_crop_sha256": "current-crop",
                "field_crop": "https://example.test/current-crop.png",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
            }
        ...
        report = build_review_queue_report(tasks)
        changed_report = build_review_queue_report(
            [
                {
                    **tasks[0],
                    "content_sha256": "changed-content",
                    "page_image_sha256": "changed-page",
                    "field_crop_sha256": "changed-crop",
                }
            ]
        )
        ...
        self.assertRegex(report["review_queue_fingerprint"], r"^[0-9a-f]{64}$")
        self.assertNotEqual(
            report["review_queue_fingerprint"],
            changed_report["review_queue_fingerprint"],
        )

    def test_review_queue_v3_builds_publish_blocker_checklist_csv(self) -> None:
        tasks = [
            {
                "task_id": "reviewed-diff",
                "status": "reviewed",
                "task_type": "gold_conflict",
                "calendar_group_key": "boardwalk",
                "field": "schedule",
                "candidate_title": "Poolside Activities",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
            },
            {
                "task_id": "pending-diff",
                "status": "pending",
                "task_type": "gold_conflict",
                "calendar_group_key": "boardwalk",
                "field": "location",
                "candidate_title": "Poolside Activities",
                "candidate_value": "Luna Park Pool Deck",
                "normalized_value": "luna_park_pool_deck",
                "review_key": "boardwalk:poolside-activities:location",
                "content_sha256": "sourcehash-boardwalk",
                "page_image_path": "data/processed/source_pages/boardwalk/page-001.png",
                "page_image_sha256": "pagehash-boardwalk",
                "field_crop": "data/processed/field_crops/boardwalk/location.png",
                "field_crop_sha256": "crophash-boardwalk-location",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
                "dual_run_diff": {
                    "v2_value": "Luna Park Pool",
                    "v3_value": "Luna Park Pool Deck",
                },
            },
            {
                "task_id": "pending-new",
                ...
                "calendar_group_key": "pop-century",
                "field": "new_record",
                "candidate_value": "Groovy Campfire",
                "review_key": "pop-century:groovy-campfire:new_record",
                "content_sha256": "sourcehash-pop",
                "page_image_path": "data/processed/source_pages/pop/page-001.png",
                "page_image_sha256": "pagehash-pop",
                "field_crop": "data/processed/field_crops/pop/title.png",
                "field_crop_sha256": "crophash-pop-title",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_new_v3_rows",
                "source_metadata": {"canonical_url": "https://example.test/pop.jpg"},
                "dual_run_new_record": {
                    "title": "Groovy Campfire",
                    "schedule": {"text": "Nightly at 7:00pm"},
                    "location": {"label": "Hippy Dippy Pool"},
                    "price": {"state": "free"},
                },
            },
            {
                "task_id": "pending-diff-extra",
                "status": "pending",
                "task_type": "gold_conflict",
                "calendar_group_key": "boardwalk",
                "field": "schedule",
                "candidate_title": "Campfire",
                "candidate_value": "Daily at 7:00pm",
                "normalized_value": "Daily at 7:00pm",
                "review_key": "boardwalk:campfire:schedule",
                "content_sha256": "sourcehash-boardwalk",
                "page_image_path": "data/processed/source_pages/boardwalk/page-001.png",
                "page_image_sha256": "pagehash-boardwalk",
                "field_crop": "data/processed/field_crops/boardwalk/schedule.png",
                "field_crop_sha256": "crophash-boardwalk-schedule",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
                "dual_run_diff": {
                    "v2_value": "Nightly at 7:00pm",
                    "v3_value": "Daily at 7:00pm",
                },
            },
            {
                "task_id": "ocr-only",
        ...
        csv_text = build_review_checklist_csv(tasks)
        rows = list(csv.DictReader(io.StringIO(csv_text)))

        self.assertEqual(
            [
                {
                    "checklist_total_pending_task_count": "3",
                    "checklist_total_batch_count": "2",
                    "source_url": "https://example.test/boardwalk.pdf",
                    "source_pending_task_count": "2",
                    "calendar_group_key": "boardwalk",
                    "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                    "reason_pending_task_count": "2",
                    "source_reason_pending_task_count": "2",
                    "review_batch_key": "https://example.test/boardwalk.pdf | unreviewed_v2_v3_field_diffs",
                    "review_batch_index": "1",
                    "review_batch_task_count": "2",
                    "source_task_index": "1",
                    "task_id": "pending-diff",
                    "task_type": "gold_conflict",
                    "field": "location",
                    ...
                    "v2_value": "Luna Park Pool",
                    "v3_value": "Luna Park Pool Deck",
                    "record_summary": "title=Poolside Activities; field=location; v2=Luna Park Pool; v3=Luna Park Pool Deck",
                    "approve_command": rows[0]["approve_command"],
                    "edit_command": rows[0]["edit_command"],
                    "reject_command": rows[0]["reject_command"],
                    "task_anchor": "#task-pending-diff",
                },
                {
                    "checklist_total_pending_task_count": "3",
                    "checklist_total_batch_count": "2",
                    ...
                    "v2_value": "Nightly at 7:00pm",
                    "v3_value": "Daily at 7:00pm",
                    "record_summary": "title=Campfire; field=schedule; v2=Nightly at 7:00pm; v3=Daily at 7:00pm",
                    "approve_command": rows[1]["approve_command"],
                    "edit_command": rows[1]["edit_command"],
                    "reject_command": rows[1]["reject_command"],
                    "task_anchor": "#task-pending-diff-extra",
                },
                {
                    "checklist_total_pending_task_count": "3",
                    "checklist_total_batch_count": "2",
                    "source_url": "https://example.test/pop.jpg",
                    "source_pending_task_count": "1",
                    "calendar_group_key": "pop-century",
                    "publish_gate_blocker_reason": "unreviewed_new_v3_rows",
                    "reason_pending_task_count": "1",
                    "source_reason_pending_task_count": "1",
                    "review_batch_key": "https://example.test/pop.jpg | unreviewed_new_v3_rows",
                    "review_batch_index": "1",
                    "review_batch_task_count": "1",
                    "source_task_index": "1",
                    "task_id": "pending-new",
                    "task_type": "manual_review_required",
                    "field": "new_record",
                    "candidate_title": "Groovy Campfire",
                    "review_key": "pop-century:groovy-campfire:new_record",
                    "content_sha256": "sourcehash-pop",
                    "page_image_path": "data/processed/source_pages/pop/page-001.png",
                    "page_image_sha256": "pagehash-pop",
                    ...
                    "v2_value": "",
                    "v3_value": "",
                    "record_summary": "calendar_group=pop-century; title=Groovy Campfire; schedule=Nightly at 7:00pm; location=Hippy Dippy Pool; price=free",
                    "approve_command": rows[2]["approve_command"],
                    "edit_command": rows[2]["edit_command"],
                    "reject_command": rows[2]["reject_command"],
                    "task_anchor": "#task-pending-new",
                },
            ],
            rows,
        )
        self.assertIn('--record-task-id "pending-diff"', rows[0]["approve_command"])
        self.assertIn("--decision approve", rows[0]["approve_command"])
        self.assertIn('--reviewer "${VISION_V3_REVIEWER:?Set VISION_V3_REVIEWER}"', rows[0]["approve_command"])
        self.assertIn('--reason "${VISION_V3_REVIEW_REASON:?Set VISION_V3_REVIEW_REASON}"', rows[0]["approve_command"])
        self.assertNotIn("Verified against source crop", rows[0]["approve_command"])
        self.assertNotIn("reviewer-name", rows[0]["approve_command"])
        self.assertIn('"location"', rows[0]["approve_command"])
        self.assertIn("luna_park_pool_deck", rows[0]["approve_command"])
        self.assertIn(
            "build_review_queue_v3.py --source-drift-report data/processed/review_queue/vision_v3_reviewed_source_drift_report.json",
            rows[0]["approve_command"],
        )
        self.assertIn("--output data/processed/eval/v3_dual_run_report.json", rows[0]["approve_command"])
        self.assertIn('--record-task-id "pending-new"', rows[2]["approve_command"])
        self.assertIn("--decision approve", rows[2]["approve_command"])
        self.assertIn('--reviewer "${VISION_V3_REVIEWER:?Set VISION_V3_REVIEWER}"', rows[2]["approve_command"])
        self.assertIn('--reason "${VISION_V3_REVIEW_REASON:?Set VISION_V3_REVIEW_REASON}"', rows[2]["approve_command"])
        self.assertNotIn("reviewer-name", rows[2]["approve_command"])
        self.assertNotIn("--approved-fields-json", rows[2]["approve_command"])

    def test_review_queue_v3_checklist_summarizes_sparse_new_or_missing_records(self) -> None:
        tasks = [
            {
                "task_id": "missing-v2-craft",
                "status": "pending",
                "task_type": "manual_review_required",
                "calendar_group_key": "port-orleans-riverside",
                "canonical_slug": "craft-activity",
                "field": "missing_v2_record",
                "candidate_value": "Craft Activity",
                "review_key": "port-orleans-riverside:craft-activity:missing_v2_record",
                "content_sha256": "sourcehash-por",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "v3_missing_v2_rows",
                "source_metadata": {"canonical_url": "https://example.test/por.jpg"},
                "dual_run_missing_v2_record": {
                    "title": "Craft Activity",
                },
            }
        ]

        rows = list(csv.DictReader(io.StringIO(build_review_checklist_csv(tasks))))

        self.assertEqual(
            "calendar_group=port-orleans-riverside; slug=craft-activity; title=Craft Activity",
            rows[0]["record_summary"],
        )
    ...
    def test_review_queue_v3_adds_source_level_parser_error_tasks(self) -> None:
        ...
        self.assertEqual("https://example.com/official-calendar.pdf", tasks[0]["source_url"])
    ...
    def test_review_queue_v3_adds_source_level_source_error_tasks(self) -> None:
        ...
        sparse_tasks = build_review_tasks(
            [],
            source_statuses=[
                {
                    "source_document_id": "source-doc-inventory",
                    "content_sha256": "inventoryhash",
                    "calendar_group_key": "pop-century",
                    "status": "source_error",
            ...
            source_inventory=[
                {
                    ...
                    "source_kind": "official_image",
                    "source_role": "resort_pdf",
                    "canonical_url": "https://example.com/pop-century.jpg",
                    "fetched_url": "https://cdn.example.com/pop-century.jpg",
                    "http_status": 503,
                    "currentness": "current",
                }
            ],
        ...
        self.assertEqual("https://example.com/pop-century.jpg", sparse_tasks[0]["source_url"])
        self.assertEqual("official_image", sparse_tasks[0]["source_metadata"]["source_kind"])
        self.assertEqual("https://cdn.example.com/pop-century.jpg", sparse_tasks[0]["source_metadata"]["fetched_url"])
    ...
    def test_review_queue_v3_cli_defaults_to_validated_candidates_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            html_output_path = root / "review_queue.html"
            checklist_output_path = root / "review_checklist.csv"
            ...
            with patch.object(build_review_queue_v3, "DEFAULT_VALIDATED_CANDIDATES_PATH", validated_dir):
                with patch.object(
                    ...
                    [
                        ...
                        "--checklist-output",
                        str(checklist_output_path),
                        *empty_review_queue_cli_defaults(
                            root,
                            include_html_output=False,
                            include_checklist_output=False,
                        ),
                    ],
                ):
            ...
            review_html = html_output_path.read_text()
            checklist_rows = list(csv.DictReader(io.StringIO(checklist_output_path.read_text())))

        self.assertEqual(1, len(tasks))
        self.assertEqual("cand-cli-review", tasks[0]["task_id"])
        self.assertIn("Vision v3 Review Queue", review_html)
        self.assertIn("cand-cli-review", review_html)
        self.assertEqual([], checklist_rows)
    ...
    def test_review_queue_v3_cli_accepts_dual_run_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source_inventory_path = root / "source_inventory.json"
            output_path = root / "review_queue.json"
            report_path = root / "review_queue_report.json"
            validated_dir.mkdir()
            ...
            with patch.object(
                ...
                [
                    ...
                    *empty_review_queue_cli_defaults(
                        root,
                        include_dual_run_report=False,
                        include_report_output=False,
        ...
        self.assertEqual("https://example.test/boardwalk.pdf", tasks[0]["source_metadata"]["canonical_url"])
        self.assertEqual("https://example.test/boardwalk.pdf", tasks[0]["source_url"])
        self.assertEqual("review_queue_v3_report", report["report_kind"])
        self.assertEqual(1, report["summary"]["task_count"])
        self.assertEqual({"gold_conflict": 1}, report["summary"]["task_count_by_type"])
        self.assertEqual({"gold_conflict:v2_v3:schedule": 1}, report["summary"]["task_count_by_finding"])
        self.assertEqual({"https://example.test/boardwalk.pdf": 1}, report["summary"]["task_count_by_source_url"])
        self.assertEqual(1, report["summary"]["visual_artifact_missing_count"])
        self.assertFalse(report["summary"]["all_tasks_have_visual_artifacts"])

    def test_review_queue_v3_cli_can_skip_stale_dual_run_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            validated_dir = root / "validated"
            dual_run_path = root / "stale_dual_run_report.json"
            output_path = root / "review_queue.json"
            validated_dir.mkdir()
            dual_run_path.write_text(
                json.dumps(
                    {
                        "status": "blocked",
                        "unreviewed_new_keys": ["boardwalk:stale-activity"],
                        "new_v3_records": [
                            {
                                "row_key": "boardwalk:stale-activity",
                                "review_key": "boardwalk:stale-activity",
                                "calendar_group_key": "boardwalk",
    ...
    def test_review_queue_v3_report_summarizes_backlog_and_visual_artifact_coverage(self) -> None:
        tasks = [
            {
                "task_id": "task-1",
                "task_type": "ocr_disagreement",
                ...
                "calendar_group_key": "boardwalk",
                "field": "schedule",
                "field_crop": "fields/schedule.png",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
            },
            {
                "task_id": "task-2",
                "task_type": "source_changed",
                "validation_findings": ["source_changed:validation_failure"],
                "calendar_group_key": "boardwalk",
                "field": "source",
                "page_image_path": "pages/page.png",
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
            },
            {
                "task_id": "task-3",
            ...
            },
            {
                "task_id": "task-4",
                "task_type": "gold_conflict",
                "validation_findings": ["gold_conflict:v2_v3:location"],
                "calendar_group_key": "pop-century",
                "field": "location",
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
            ...
            },
            {
                ...
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "v3_missing_v2_rows",
                "source_metadata": {"canonical_url": "https://example.test/all-star.pdf"},
            },
        ]
        ...
        self.assertEqual("review_queue_v3_report", report["report_kind"])
        self.assertEqual(5, report["summary"]["task_count"])
        self.assertEqual(
            {
                "gold_conflict": 1,
            ...
            report["summary"]["task_count_by_type"],
        )
        self.assertEqual(
            {
                "engine_disagreement:schedule": 1,
                "gold_conflict:v2_v3:location": 1,
                "manual_review_required:missing_v2_record": 1,
                "manual_review_required:new_record": 1,
                "source_changed:validation_failure": 1,
            },
            report["summary"]["task_count_by_finding"],
        )
        self.assertEqual(3, report["summary"]["visual_artifact_missing_count"])
        self.assertEqual(5, report["summary"]["visual_artifact_missing_file_count"])
        self.assertEqual(0, report["summary"]["source_url_missing_count"])
        self.assertTrue(report["summary"]["all_tasks_have_source_urls"])
        self.assertEqual(
            {"all-star-movies": 1, "boardwalk": 2, "pop-century": 2},
            report["summary"]["task_count_by_calendar_group"],
        )
        self.assertEqual(
            {"location": 1, "missing_v2_record": 1, "new_record": 1, "schedule": 1, "source": 1},
            report["summary"]["task_count_by_field"],
        )
        self.assertEqual(
            {
                "https://example.test/all-star.pdf": 1,
                "https://example.test/boardwalk.pdf": 2,
                "https://example.test/pop.jpg": 2,
            },
            report["summary"]["task_count_by_source_url"],
        )
        self.assertEqual(
            {
                "dual_run_field_diff": {
                    "task_count": 1,
                    "task_ids": ["task-4"],
                    "task_count_by_field": {"location": 1},
                },
                "new_v3_record": {
                    "task_count": 1,
                    "task_ids": ["task-3"],
                    "task_count_by_field": {"new_record": 1},
                },
                "missing_v2_record": {
                    "task_count": 1,
                    "task_ids": ["task-5"],
                    "task_count_by_field": {"missing_v2_record": 1},
                },
                "ocr_or_parser_review": {
                    "task_count": 1,
                    "task_ids": ["task-1"],
                    "task_count_by_field": {"schedule": 1},
                },
                "source_or_drift_review": {
                    "task_count": 1,
                    "task_ids": ["task-2"],
                    "task_count_by_field": {"source": 1},
                },
            },
            report["review_workstreams"],
        )
        self.assertEqual(
            {
                "total_task_count": 2,
                ...
                "pending_task_ids": ["task-4", "task-5"],
                "reviewed_task_ids": [],
                "pending_review_artifact_missing_count": 2,
                "pending_review_artifact_missing_task_ids": ["task-4", "task-5"],
                "all_pending_tasks_have_review_artifacts": False,
                "pending_task_count_by_reason": {
                    "unreviewed_v2_v3_field_diffs": 1,
                    "v3_missing_v2_rows": 1,
                },
                "pending_task_ids_by_reason": {
                    "unreviewed_v2_v3_field_diffs": ["task-4"],
                    "v3_missing_v2_rows": ["task-5"],
                },
                "pending_task_count_by_source_url": {
                    "https://example.test/all-star.pdf": 1,
                    "https://example.test/pop.jpg": 1,
                ...
                "pending_task_ids_by_source_url": {
                    "https://example.test/all-star.pdf": ["task-5"],
                    "https://example.test/pop.jpg": ["task-4"],
                },
                "pending_task_count_by_source_url_and_reason": {
                    "https://example.test/all-star.pdf": {
                        "v3_missing_v2_rows": 1,
                    },
                    "https://example.test/pop.jpg": {
                        "unreviewed_v2_v3_field_diffs": 1,
            ...
            [
                {
                    "calendar_group_key": "boardwalk",
                    "task_count": 2,
                    "task_count_by_type": {"ocr_disagreement": 1, "source_changed": 1},
                    "task_count_by_field": {"schedule": 1, "source": 1},
                    "source_urls": ["https://example.test/boardwalk.pdf"],
                },
                {
                    "calendar_group_key": "pop-century",
                    "task_count": 2,
                    "task_count_by_type": {"gold_conflict": 1, "manual_review_required": 1},
                    "task_count_by_field": {"location": 1, "new_record": 1},
                    "source_urls": ["https://example.test/pop.jpg"],
                },
                {
                    "calendar_group_key": "all-star-movies",
                    "task_count": 1,
                    "task_count_by_type": {"manual_review_required": 1},
                    "task_count_by_field": {"missing_v2_record": 1},
                    "source_urls": ["https://example.test/all-star.pdf"],
                },
            ],
            report["top_calendar_group_blockers"],
        )
        self.assertEqual(["task-3", "task-4", "task-5"], report["visual_artifact_missing_task_ids"])
        self.assertFalse(report["summary"]["all_tasks_have_visual_artifacts"])
        self.assertFalse(report["summary"]["all_tasks_have_existing_visual_artifacts"])

    def test_review_queue_v3_report_flags_tasks_missing_source_urls(self) -> None:
        report = build_review_queue_report(
            [
                {
                    "task_id": "task-with-url",
                    "task_type": "ocr_disagreement",
                    "validation_findings": ["engine_disagreement:schedule"],
                    "calendar_group_key": "boardwalk",
                    "source_url": "https://example.test/boardwalk.pdf",
        ...
        self.assertEqual(1, report["summary"]["source_url_missing_count"])
        self.assertFalse(report["summary"]["all_tasks_have_source_urls"])
        self.assertEqual(["task-missing-url"], report["source_url_missing_task_ids"])

    def test_review_queue_v3_html_renders_reviewable_evidence_and_commands(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            field_crop = root / "fields" / "schedule.png"
            region_crop = root / "regions" / "region.png"
            page_image = root / "pages" / "page_001.png"
            output_path = root / "vision_v3_review_queue.html"
            checklist_path = root / "vision_v3_publish_blocker_checklist.csv"
            for path in (field_crop, region_crop, page_image):
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(b"not-a-real-png-but-present")
            checklist_path.write_text("task_id\n")
            tasks = [
                {
                    "task_id": "task-1",
                    "task_type": "ocr_disagreement",
                    ...
                    "calendar_group_key": "boardwalk",
                    "canonical_slug": "poolside-activities",
                    "review_key": "boardwalk:poolside-activities:schedule",
                    "publish_gate_blocker": True,
                    "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                    "candidate_type": "activity",
                    "candidate_display_label": "Poolside Activities",
                    "candidate_title": "Poolside Activities",
                    "candidate_schedule": "Daily at 1:30pm",
                    "candidate_location": "Luna Park Pool",
                    "candidate_fee_required": False,
                    "page_number": 1,
                    "page_image_sha256": "pagehash",
                    "page_image_path": str(page_image),
                    "region_crop": str(region_crop),
                    "field_crop": str(field_crop),
                    "field_crop_sha256": "crop<&>hash",
                    "primary_text": "Daily at 1:30pm",
                    "secondary_text": "Daily at 7:30pm",
                    "candidate_value": "<script>alert('bad')</script>",
                    "normalized_value": {"start_time": "13:30"},
                    "validation_findings": ["engine_disagreement:schedule"],
                    "source_metadata": {"canonical_url": "https://example.com/official.pdf"},
                }
            ]

            report = build_review_queue_report(tasks)
            html = build_review_queue_html(tasks, report, output_path=output_path)

        self.assertIn("Vision v3 Review Queue", html)
        self.assertIn("Review Command Setup", html)
        self.assertIn('export VISION_V3_REVIEWER="your-name"', html)
        self.assertIn('export VISION_V3_REVIEW_REASON="source-backed rationale for this decision"', html)
        self.assertIn("Top Blockers", html)
        self.assertIn("Review Workstreams", html)
        self.assertIn("ocr_or_parser_review", html)
        self.assertIn("Publish Gate Summary", html)
        self.assertIn("<strong>1</strong> publish blockers", html)
        self.assertIn("<strong>1</strong> pending publish blockers", html)
        self.assertIn("<strong>0</strong> reviewed publish blockers", html)
        self.assertIn("Review Artifacts", html)
        self.assertIn('href="vision_v3_publish_blocker_checklist.csv"', html)
        self.assertIn("Publish blocker checklist CSV", html)
        self.assertIn("Publish Gate Blockers", html)
        self.assertIn("Publish Gate Review Batches", html)
        self.assertIn("https://example.com/official.pdf | unreviewed_v2_v3_field_diffs", html)
        self.assertIn("unreviewed_v2_v3_field_diffs", html)
        self.assertIn('<a href="#task-task-1"><strong>unreviewed_v2_v3_field_diffs</strong></a>', html)
        self.assertIn('<a href="#task-task-1">task-1</a>', html)
        self.assertIn('<article class="task" id="task-task-1">', html)
        self.assertIn("1 task", html)
        self.assertIn("boardwalk", html)
        self.assertIn("1 task", html)
        self.assertIn('href="#group-boardwalk"', html)
        self.assertIn('id="group-boardwalk"', html)
        self.assertIn('<section class="task-group" id="group-boardwalk">', html)
        self.assertIn("boardwalk · 1 task", html)
        self.assertIn("Poolside Activities", html)
        self.assertIn("Schedule: <strong>Daily at 1:30pm</strong>", html)
        self.assertIn("Location: <strong>Luna Park Pool</strong>", html)
        self.assertIn("Fee required: <strong>False</strong>", html)
        self.assertIn("https://example.com/official.pdf", html)
        self.assertIn("engine_disagreement:schedule", html)
        self.assertIn('href="fields/schedule.png"', html)
        self.assertIn('src="fields/schedule.png"', html)
        self.assertIn('href="regions/region.png"', html)
        self.assertIn('href="pages/page_001.png"', html)
        self.assertIn("python scripts/ingest/review_schema_v3.py", html)
        self.assertIn("--record-task-id &quot;task-1&quot;", html)
        self.assertIn('${VISION_V3_REVIEW_REASON:?Set VISION_V3_REVIEW_REASON}', html)
        self.assertIn('<button type="button" data-copy-command="task-1-approve">Approve</button>', html)
        self.assertIn('<button type="button" data-copy-command="task-1-edit">Edit</button>', html)
        self.assertIn('<button type="button" data-copy-command="task-1-reject">Reject</button>', html)
        self.assertIn('<pre id="task-1-approve">', html)
        self.assertIn("navigator.clipboard.writeText", html)
        self.assertIn("--fixture-candidates-output data/processed/review_queue/vision_v3_review_fixture_candidates.json", html)
        self.assertIn("--parser-rule-requests-output data/processed/review_queue/vision_v3_parser_rule_update_requests.json", html)
        self.assertIn("--dual-run-review-keys-output data/processed/review_queue/vision_v3_dual_run_review_keys.json", html)
        self.assertIn("&amp;&amp; python scripts/ingest/v3_dual_run_report.py", html)
        self.assertIn("--reviewed-diff-keys-file data/processed/review_queue/vision_v3_dual_run_review_keys.json", html)
        self.assertIn("&amp;&amp; python scripts/ingest/build_review_queue_v3.py", html)
        approve_command_html = html[
            html.index('<pre id="task-1-approve">') : html.index('<pre id="task-1-edit">')
        ]
        review_decision_index = approve_command_html.index("python scripts/ingest/review_schema_v3.py")
        first_queue_rebuild_index = approve_command_html.index(
            "&amp;&amp; python scripts/ingest/build_review_queue_v3.py"
        )
        dual_run_refresh_index = approve_command_html.index(
            "&amp;&amp; python scripts/ingest/v3_dual_run_report.py"
        )
        final_queue_rebuild_index = approve_command_html.rindex(
            "&amp;&amp; python scripts/ingest/build_review_queue_v3.py"
        )
        self.assertLess(review_decision_index, first_queue_rebuild_index)
        self.assertLess(first_queue_rebuild_index, dual_run_refresh_index)
        self.assertLess(dual_run_refresh_index, final_queue_rebuild_index)
        approve_command_start = html.index("--decision approve")
        edit_command_start = html.index("--decision edit")
        self.assertIn("--approved-fields-json", html[approve_command_start:edit_command_start])
        self.assertIn("--decision edit", html)
        self.assertIn("--approved-fields-json", html)
        self.assertIn("&quot;schedule&quot;: {", html)
        self.assertIn("&quot;raw_value&quot;: &quot;&lt;script&gt;alert", html)
        self.assertIn("&quot;normalized_value&quot;: {", html)
        self.assertIn("Slug: <strong>poolside-activities</strong>", html)
        self.assertIn("Review key: <code>boardwalk:poolside-activities:schedule</code>", html)
        self.assertIn("&lt;script&gt;alert(&#x27;bad&#x27;)&lt;/script&gt;", html)
        self.assertNotIn("<script>alert", html)
        self.assertIn("crop&lt;&amp;&gt;hash", html)

    def test_review_queue_v3_html_lists_publish_gate_task_shortcuts(self) -> None:
        tasks = [
            {
                "task_id": "diff-one",
                "task_type": "gold_conflict",
                ...
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_v2_v3_field_diffs",
                "validation_findings": ["gold_conflict:v2_v3:schedule"],
                "source_metadata": {"canonical_url": "https://example.test/boardwalk.pdf"},
            },
            {
                "task_id": "diff-two",
        ...
        html = build_review_queue_html(tasks, build_review_queue_report(tasks))

        self.assertIn('<a href="#task-diff-one">diff-one</a>', html)
        self.assertIn('<a href="#task-diff-two">diff-two</a>', html)
        self.assertIn("Publish Gate Source URLs", html)
        self.assertIn('<a href="https://example.test/boardwalk.pdf">https://example.test/boardwalk.pdf</a>', html)
        self.assertIn("2 tasks", html)
        self.assertIn("&quot;unreviewed_v2_v3_field_diffs&quot;: 2", html)
        self.assertIn('<article class="task" id="task-diff-one">', html)
        self.assertIn('<article class="task" id="task-diff-two">', html)

    def test_review_queue_v3_html_marks_missing_local_artifact_without_broken_image(self) -> None:
        html = build_review_queue_html(
            [
                {
                    "task_id": "task-missing-file",
                    "task_type": "manual_review_required",
                    "calendar_group_key": "boardwalk",
                    "field_crop": "missing/field.png",
                    "validation_findings": ["manual_review_required:schedule"],
                }
            ]
        )

        self.assertIn("file not found locally", html)
        self.assertIn('href="missing/field.png"', html)
        self.assertNotIn('src="missing/field.png"', html)

    def test_review_queue_v3_html_groups_multiple_tasks_under_one_calendar_group_heading(self) -> None:
        tasks = [
            {
                "task_id": "task-1",
                "task_type": "ocr_disagreement",
                ...
                "field": "schedule",
                "validation_findings": ["engine_disagreement:schedule"],
                "source_metadata": {"canonical_url": "https://example.com/boardwalk.pdf"},
            },
            {
                "task_id": "task-2",
                "task_type": "manual_review_required",
                "calendar_group_key": "boardwalk",
                "field": "location",
                "validation_findings": ["manual_review_required:location"],
                "source_metadata": {"canonical_url": "https://example.com/boardwalk.pdf"},
            },
        ]
        ...
        self.assertEqual(1, html.count('<section class="task-group" id="group-boardwalk">'))
        self.assertIn("boardwalk · 2 tasks", html)
        self.assertLess(html.index("task-1"), html.index("task-2"))

    def test_review_queue_v3_html_makes_repo_relative_artifact_paths_work_from_output_file(self) -> None:
        html = build_review_queue_html(
            [
                {
                    ...
                    "candidate_type": "activity",
                    "content_sha256": "sourcehash",
                    "page_image_path": "data/processed/page_images/sourcehash/page_001.png",
                    "field_crop": "data/processed/vision_debug/sourcehash/field.png",
                    "field_crop_sha256": "fieldhash",
                    "validation_findings": ["manual_review_required:schedule"],
                }
            ],
            output_path=Path("data/processed/review_queue/vision_v3_review_queue.html"),
        )

        self.assertIn('href="../vision_debug/sourcehash/field.png"', html)
        self.assertIn('href="../page_images/sourcehash/page_001.png"', html)
    ...
    def test_review_queue_v3_exposes_candidate_summary_for_frontend_review(self) -> None:
        candidate = {
            "candidate_id": "cand-ui-summary",
            "validation_status": "needs_review",
            "validation_findings": ["engine_disagreement:schedule"],
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
            ...
            "calendar_group_key": "boardwalk",
            "candidate_type": "activity",
            "source_title": "POOL PARTY",
            "normalized_title": "Pool Party",
            "movie_title": "Moana 2",
            "schedule_raw": "Daily from 1:30 PM to 2:30 PM",
            "location_text": "Luna Park Pool",
            "fee_required": False,
            "field_evidence": {
                "schedule": {
                    "field": "schedule",
                    "raw_value": "Daily from 1:30 PM to 2:30 PM",
                    "normalized_value": {
                        "schedule_type": "recurring",
                        "days_of_week": ALL_DAYS,
                        "start_time": "13:30",
                    ...
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "Daily from 1:30 PM to 2:30 PM"},
                        {"engine": "rapidocr", "text": "Daily from 7:30 PM to 8:30 PM"},
                    ],
                }
            },
        }
        ...
        self.assertEqual("Pool Party", task["candidate_title"])
        self.assertEqual("Pool Party", task["title"])
        self.assertEqual("Pool Party", task["candidate_display_label"])
        self.assertEqual("Moana 2", task["candidate_movie_title"])
        self.assertEqual("Daily from 1:30 PM to 2:30 PM", task["candidate_schedule"])
        self.assertEqual("Luna Park Pool", task["candidate_location"])
        self.assertFalse(task["candidate_fee_required"])
        self.assertEqual(candidate["field_evidence"]["schedule"], task["selected_field_evidence"])

        untitled_candidate = {
            **candidate,
            "candidate_id": "cand-ui-summary-missing-title",
            "source_title": None,
            "normalized_title": None,
            "movie_title": None,
            "schedule_raw": "Tuesday",
        }

        [untitled_task] = build_review_tasks([untitled_candidate])

        self.assertIsNone(untitled_task["candidate_title"])
        self.assertEqual("Tuesday", untitled_task["candidate_display_label"])
    ...
    def test_review_queue_v3_normalizes_prd_review_task_types(self) -> None:
        ...
        candidates = [
            ...
            {
                **base_candidate,
                "candidate_id": "cand-fee-ocr-disagreement",
                "validation_findings": [
                    "fee_legend_missing_for_unmarked_title",
                    "engine_disagreement:title",
                    "engine_disagreement:schedule",
                    "engine_disagreement:fee",
                ],
                "field_evidence": {
                    **base_candidate["field_evidence"],
                    "fee": {
                        ...
                        "source": {
                            ...
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "bbox_px": [20, 10, 120, 30],
                            "crop_storage_path": "fee-disagreement.png",
                            "crop_sha256": "feedisagreementcrop",
                        },
                        "engines": [
                            {"engine": "paddleocr_ppstructurev3", "text": "no fee marker", "confidence": 0.97},
                            {"engine": "rapidocr", "text": "($)", "confidence": 0.94},
                        ],
                    },
                },
            },
            {
                **base_candidate,
                "candidate_id": "cand-non-activity-title",
                "validation_findings": ["non_activity_title:title"],
                "field_evidence": {
                    **base_candidate["field_evidence"],
                    "title": {
                        "raw_value": "LOCATED NEAR",
                        "normalized_value": "located near",
                        "source": {
                            "content_sha256": "sourcehash",
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "bbox_px": [30, 10, 140, 30],
                            "crop_storage_path": "non-activity-title.png",
                            "crop_sha256": "nonactivitytitlecrop",
                        },
                        "engines": [],
                    },
                },
            ...
            {
                **base_candidate,
                "candidate_id": "cand-partial-movie-row",
                "candidate_type": "movie",
                "validation_findings": [
                    "partial_movie_row_review_required",
        ...
        self.assertEqual("fee", tasks[4]["field"])
        self.assertEqual("fee-disagreement.png", tasks[4]["field_crop"])
        self.assertEqual("title", tasks[5]["field"])
        self.assertEqual("non-activity-title.png", tasks[5]["field_crop"])
        self.assertEqual("title", tasks[6]["field"])
        self.assertEqual("regions/region.png", tasks[6]["field_crop"])
    ...
    def test_review_queue_v3_loads_gold_conflicts_from_preview_without_treating_rows_as_conflicts(self) -> None:
        preview_payload = {
            "publication_mode": "vision_v3_preview",
            "gold_conflicts": [],
            "rows": [
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            conflict_path = root / "activity_gold_v3_conflicts.json"
            preview_path.write_text(json.dumps(preview_payload))
            conflict_path.write_text(json.dumps(conflict_payload))

            self.assertEqual([], build_review_queue_v3._load_gold_conflicts(preview_path))
            self.assertEqual(
                conflict_payload["gold_conflicts"],
                build_review_queue_v3._load_gold_conflicts(conflict_path),
            )
    ...
    def test_review_queue_v3_does_not_create_first_baseline_new_title_drift_tasks(self) -> None:
        source_drift_report = {
            "report_kind": "v3_quarter_source_drift",
            "quarter": "fy26-q4",
            "status": "review_required",
            "publish_blockers": ["validation_failures"],
            "source_reports": [
                {
                    "source_document_id": "source-doc-1",
                    "calendar_group_key": "boardwalk",
                    "baseline_mode": "first_v3_baseline",
                    "new_hash": "newhash",
                    "status": "review_required",
                    "new_titles": ["Poolside Activities"],
                    "new_title_details": [
                        {
                            "title": "Poolside Activities",
                            "canonical_slug": "poolside-activities",
                    ...
                    "validation_failures": [
                        {
                            ...
                            "canonical_slug": "poolside-activities",
                            "title": "Poolside Activities",
                            "validation_findings": ["unparsed_schedule"],
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "crop_storage_path": "schedule.png",
                            "crop_sha256": "schedulecrop",
        ...
        self.assertEqual(
            ["source_changed:validation_failure"],
            [task["validation_findings"][0] for task in tasks],
        )
        self.assertNotIn("source_changed:new_title", [task["validation_findings"][0] for task in tasks])

    def test_review_queue_v3_keeps_validation_failure_blocker_when_no_detailed_failures_exist(self) -> None:
        source_drift_report = {
            "report_kind": "v3_quarter_source_drift",
            "quarter": "fy26-q4",
            "status": "review_required",
        ...
        self.assertEqual(["source_changed:validation_failures"], task["validation_findings"])
        self.assertEqual("validation_failures", task["candidate_value"])
    ...
    def test_review_queue_v3_source_drift_commands_export_reviewed_drift_report(self) -> None:
        source_drift_report = {
            "report_kind": "v3_quarter_source_drift",
            "quarter": "fy26-q4",
            "status": "review_required",
        ...
        tasks = build_review_tasks([], source_drift_report=source_drift_report)
        csv_text = build_review_checklist_csv([{**tasks[0], "publish_gate_blocker": True, "publish_gate_blocker_reason": "source_drift"}])
        [row] = list(csv.DictReader(io.StringIO(csv_text)))
        approve_command = row["approve_command"]

        self.assertIn("--export reviewed-source-drift-report", approve_command)
        self.assertIn("--source-drift-report data/processed/source_drift_report.json", approve_command)
        self.assertIn(
            "--output data/processed/review_queue/vision_v3_reviewed_source_drift_report.json",
            approve_command,
        )
        self.assertIn(
            "build_review_queue_v3.py --source-drift-report data/processed/review_queue/vision_v3_reviewed_source_drift_report.json",
    ...
    def test_review_queue_v3_builds_tasks_from_dual_run_report_blockers(self) -> None:
        dual_run_report = {
            ...
            "field_diffs": [
                {
                    ...
                    "v3_field_evidence": {
                        "source": {
                            ...
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "page_image_path": "pages/source-page.png",
                            "bbox_px": [10, 20, 200, 44],
                            "crop_storage_path": "fields/schedule.png",
                            "crop_sha256": "schedulecrop",
                        },
            ...
            "new_v3_records": [
                {
                    ...
                    "v3_field_evidence": {
                        "title": {
                            "source": {
                                "page_number": 1,
                                "page_image_sha256": "newpagehash",
                                "page_image_path": "pages/new.png",
                                "bbox_px": [30, 40, 260, 70],
                                "crop_storage_path": "fields/title.png",
                                "crop_sha256": "titlecrop",
                            },
                            "engines": [
                                {"engine": "paddleocr_ppstructurev3", "text": "Movie Under the Stars"},
                                {"engine": "rapidocr", "text": "Movie Under the Stars"},
                            ],
                        }
                    },
                }
            ...
            "missing_v2_records": [
                {
                    "review_key": "boardwalk:campfire:missing_v2_record",
                    "row_key": "boardwalk:campfire",
                    "canonical_slug": "campfire",
                    "calendar_group_key": "boardwalk",
                    "title": "Campfire",
                    "content_sha256": "oldsourcehash",
                    "page_number": 1,
                    "page_image_path": "data/processed/pdf_page_images/oldsourcehash-page-001.png",
                    "field_bbox": [40, 50, 260, 80],
                    "v2_value": {
                        "title": "Campfire",
                        "schedule": "Nightly at 6:30pm",
                        "location": "Village Green Lawn",
                        "price": "free",
                    },
                }
        ...
        self.assertEqual(
            [
                ...
                "dual-run-missing-v2-boardwalk-campfire-missing-v2-record",
            ],
            [task["task_id"] for task in tasks],
        )
        self.assertEqual(
            ["gold_conflict", "manual_review_required", "manual_review_required", "manual_review_required"],
            [task["task_type"] for task in tasks],
        )
        self.assertEqual(
            ["schedule", "status_transition", "new_record", "missing_v2_record"],
        ...
        self.assertEqual(
            [
                ...
                "boardwalk:poolside-activities:needs_review_to_auto_publishable",
                "boardwalk:movie-under-the-stars:new_record",
                "boardwalk:campfire:missing_v2_record",
            ],
            [task["review_key"] for task in tasks],
        )
        self.assertEqual([True, True, True, True], [task["publish_gate_blocker"] for task in tasks])
        self.assertEqual(
            [
                "unreviewed_v2_v3_field_diffs",
                "unreviewed_v3_status_transitions",
            ...
            [task["publish_gate_blocker_reason"] for task in tasks],
        )
        self.assertEqual(
            ["poolside-activities", "poolside-activities", "movie-under-the-stars", "campfire"],
            [task["canonical_slug"] for task in tasks],
        )
        self.assertEqual(["sourcehash", None, "newsourcehash", "oldsourcehash"], [task["content_sha256"] for task in tasks])
        self.assertEqual("Daily at 1:30pm", tasks[0]["source_drift"]["old"])
        self.assertEqual("Daily at 2:30pm", tasks[0]["candidate_value"])
        ...
        self.assertEqual("pages/source-page.png", tasks[0]["page_image_path"])
        ...
        self.assertEqual("newpagehash", tasks[2]["page_image_sha256"])
        self.assertEqual("pages/new.png", tasks[2]["page_image_path"])
        self.assertEqual("fields/title.png", tasks[2]["field_crop"])
        self.assertEqual("titlecrop", tasks[2]["field_crop_sha256"])
        self.assertEqual([30, 40, 260, 70], tasks[2]["field_bbox"])
        self.assertEqual("Movie Under the Stars", tasks[2]["primary_text"])
        self.assertEqual(["manual_review_required:missing_v2_record"], tasks[3]["validation_findings"])
        self.assertEqual("data/processed/pdf_page_images/oldsourcehash-page-001.png", tasks[3]["page_image_path"])
        self.assertEqual([40, 50, 260, 80], tasks[3]["field_bbox"])
        ...
        self.assertEqual(dual_run_report["missing_v2_records"][0], tasks[3]["dual_run_missing_v2_record"])

        reviewed_missing_report = {
            **dual_run_report,
            "missing_v2_records": [
                {**dual_run_report["missing_v2_records"][0], "reviewed": True}
            ],
        }
        reviewed_missing_tasks = build_review_tasks([], dual_run_report=reviewed_missing_report)

        self.assertNotIn(
            "dual-run-missing-v2-boardwalk-campfire-missing-v2-record",
            [task["task_id"] for task in reviewed_missing_tasks],
        )

    def test_review_queue_materializes_missing_v2_visual_artifacts_for_approval(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_image = root / "page_images" / "oldsourcehash" / "page_001.png"
            page_image.parent.mkdir(parents=True)
            Image.new("RGB", (300, 200), "white").save(page_image)
            dual_run_report = {
                "status": "blocked",
                "missing_v2_records": [
                    {
            ...
            with patch.object(build_review_queue_v3, "PROCESSED_DIR", root):
                tasks = build_review_tasks([], dual_run_report=dual_run_report)

            task = tasks[0]
            crop_path = Path(task["field_crop"])

            self.assertEqual(hashlib.sha256(page_image.read_bytes()).hexdigest(), task["page_image_sha256"])
            self.assertTrue(crop_path.exists())
            self.assertEqual(hashlib.sha256(crop_path.read_bytes()).hexdigest(), task["field_crop_sha256"])
            with Image.open(crop_path) as crop_image:
                self.assertEqual((220, 30), crop_image.size)
            decision = build_review_decision(
                task,
                reviewer="josh",
                decision="approve",
                reason="Verified old row was intentionally removed from v3",
            )
            self.assertEqual("approve", decision["decision"])

    def test_review_queue_suppresses_expected_large_official_image_warning(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_image = root / "page_images" / "oldsourcehash" / "page_001.png"
            page_image.parent.mkdir(parents=True)
            Image.new("RGB", (120, 80), "white").save(page_image)
            dual_run_report = {
                "status": "blocked",
                "missing_v2_records": [
                    {
                        ...
                        "page_number": 1,
                        "page_image_path": str(page_image),
                        "field_bbox": [40, 50, 100, 70],
                    }
                ],
            }

            original_limit = Image.MAX_IMAGE_PIXELS
            Image.MAX_IMAGE_PIXELS = 119 * 80
            try:
            ...
            warning_categories = [warning.category for warning in captured]

            self.assertNotIn(Image.DecompressionBombWarning, warning_categories)
            self.assertTrue(Path(tasks[0]["field_crop"]).exists())

    def test_review_queue_uses_page_artifact_for_missing_v2_record_without_bbox(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page_image = root / "page_images" / "oldsourcehash" / "page_001.png"
            page_image.parent.mkdir(parents=True)
            ...
            task = tasks[0]
            page_hash = hashlib.sha256(page_image.read_bytes()).hexdigest()

            self.assertEqual(page_hash, task["page_image_sha256"])
            self.assertEqual(str(page_image), task["field_crop"])
            self.assertEqual(page_hash, task["field_crop_sha256"])
            decision = build_review_decision(
                task,
                reviewer="josh",
                decision="approve",
                reason="Verified old row should not publish in v3",
            )
            self.assertEqual("approve", decision["decision"])
    ...
    def test_promote_gold_v3_filters_preview_rows_to_quarter_expected_sources(self) -> None:
        matching_candidate = {
            "validation_status": "auto_publishable",
            "source_document_id": "source-doc-current",
            "content_sha256": "current-sourcehash",
            "source_kind": "official_image",
            "source_role": "supporting_price_image",
            "source_type": "image",
            "calendar_group_key": "port-orleans-riverside",
            "edition": "fy26-q4-0626",
            "candidate_type": "activity",
            "normalized_title": "Campfire on de Bayou",
            "location_text": "Campfire Area",
            "schedule_raw": "Nightly at 7:00pm",
            "schedule_normalized": {"schedule_type": "recurring", "start_time": "19:00"},
            "fee_required": False,
            "field_evidence": {},
        }
        stale_candidate = {
            **matching_candidate,
            "source_document_id": "source-doc-stale",
            "content_sha256": "stale-sourcehash",
            "calendar_group_key": "boardwalk",
            "edition": "fy26-q3-0526",
            "normalized_title": "Stale Poolside Activities",
        }

        preview = build_gold_v3_preview(
            [matching_candidate, stale_candidate],
            expected_sources=[
                {
                    "content_sha256": "current-sourcehash",
                    "source_kind": "official_image",
        ...
        self.assertEqual(1, preview["summary"]["input_candidates"])
        self.assertEqual(["Campfire on de Bayou"], [row["title"] for row in preview["rows"]])

    def test_promote_gold_v3_uses_parser_repaired_schedule_text_for_public_rows(self) -> None:
        def candidate_with_schedule(
            *,
            schedule_raw: str,
            schedule_normalized: dict[str, Any],
            schedule_evidence_raw: str,
        ) -> dict[str, Any]:
            return {
            "validation_status": "auto_publishable",
            "source_document_id": "source-doc-1",
            ...
            "document_family": "aframe_recreation",
            "canonical_page_image_sha256s": ["pagehash"],
            "source_pages": [{"page_number": 1, "page_image_sha256": "pagehash"}],
            "edition": "fy26-q3-0526",
            "source_type": "pdf",
            "pipeline_version": "vision_v3_001",
            "resort_slug": "boardwalk-inn",
            "candidate_type": "activity",
            "normalized_title": "Movie Under the Stars",
            "source_title": "Movie Under the Stars",
            "normalized_location_id": "village_green_lawn",
            "location_text": "Village Green Lawn",
            ...
            "field_evidence": {
                field: {
                    ...
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": value, "confidence": 0.99},
                        {"engine": "rapidocr", "text": value, "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                }
                for field, value, normalized in (
                    ("title", "Movie Under the Stars", "movie under the stars"),
                    (
                        "schedule",
                        schedule_evidence_raw,
                        schedule_normalized,
                    ),
                    ("location", "Village Green Lawn", "village_green_lawn"),
                    ("fee", "no fee marker", False),
                )
            },
            "source_spans": {
                "title": [{"page": 1, "text": "Movie Under the Stars"}],
                "schedule": [{"page": 1, "text": schedule_evidence_raw}],
                "location": [{"page": 1, "text": "Village Green Lawn"}],
                "fee": [{"page": 1, "text": "no fee marker"}],
            },
        }
        ...
        cases = [
            ...
            (
                ...
                "From 3:30pm-4:30pm",
                {
                    ...
                    "end_time": "16:30",
                    "timezone": "America/New_York",
                    "raw_text": "From 3:30pm-4:30pm",
                },
            ),
        ]
        for expected_public_text, schedule_evidence_raw, schedule_normalized in cases:
            with self.subTest(expected_public_text=expected_public_text):
                preview = build_gold_v3_preview(
                    [
                        candidate_with_schedule(
                            schedule_raw=schedule_evidence_raw,
                            schedule_normalized=schedule_normalized,
                            schedule_evidence_raw=schedule_evidence_raw,
                        )
                    ]
                )

                self.assertEqual(expected_public_text, preview["rows"][0]["schedule"]["text"])
                self.assertEqual(schedule_evidence_raw, preview["rows"][0]["field_evidence"]["schedule"]["raw_value"])
                self.assertEqual(schedule_evidence_raw, preview["rows"][0]["source_spans"]["schedule"][0]["text"])

    def test_promote_gold_v3_slugifies_titles_with_fee_markers_apostrophes_and_accents(self) -> None:
        cases = {
            "Mickey Tie-Dye ($)": "mickey-tie-dye",
            "Sorcerer's Campfire": "sorcerers-campfire",
            "Note'able Games Arcade ($)": "noteable-games-arcade",
            "Campfire on de'Bayou": "campfire-on-de-bayou",
            "Côte d'Azur Campfire": "cote-dazur-campfire",
            "Winner's Circle Mosaics ($)": "winners-circle-mosaics",
        }

        for title, expected_slug in cases.items():
            with self.subTest(title=title):
                self.assertEqual(expected_slug, promote_gold_v3._slugify(title))

    def test_promote_gold_v3_merges_duplicate_recurring_day_rows_for_same_activity(self) -> None:
        def candidate(day: str, candidate_id: str) -> dict[str, Any]:
            return {
                ...
                "source_role": "resort_pdf",
                "source_type": "image",
                "canonical_url": "https://example.com/pors.jpg",
                "http_status": 200,
                "currentness": "current",
                "calendar_group_key": "port-orleans-riverside",
                "document_family": "vertical_digital_rec_sign",
                "canonical_page_image_sha256s": ["pagehash"],
                "edition": "official-schedule-0526",
                "pipeline_version": "vision_v3_001",
                "candidate_type": "activity",
                "source_title": "Campfire on de'Bayou",
                "normalized_title": "campfire on de' bayou" if day == "Thursday" else "campfire on de'bayou",
                "normalized_location_id": "ol_man_island",
                "location_text": "Ol' Man Island",
                "schedule_raw": f"{day} from 6:30pm-7:30pm",
                "schedule_normalized": {
                    "schedule_type": "recurring",
                    "days_of_week": [day],
                    "start_time": "18:30",
                    "end_time": "19:30",
                    "timezone": "America/New_York",
                    "raw_text": f"{day} from 6:30pm-7:30pm",
                ...
                "field_evidence": {
                    field: {
                        "raw_value": f"{day} from 6:30pm-7:30pm" if field == "schedule" else "value",
                        "normalized_value": {
                            "schedule_type": "recurring",
                            "days_of_week": [day],
                            "start_time": "18:30",
                        ...
                        if field == "schedule"
                        else ("ol_man_island" if field == "location" else (False if field == "fee" else "campfire on de'bayou")),
                        "source": {
                            "content_sha256": "sourcehash",
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "bbox_px": [10, 10, 100, 30],
                            "crop_sha256": f"{candidate_id}-{field}crop",
                            "crop_storage_path": f"{candidate_id}-{field}.png",
                        },
                        "engines": [
                            {"engine": "paddleocr_ppstructurev3", "text": "value", "confidence": 0.99},
                            {"engine": "rapidocr", "text": "value", "confidence": 0.98},
                ...
                "source_spans": {
                    "title": [{"page": 1, "text": "Campfire on de'Bayou"}],
                    "schedule": [{"page": 1, "text": f"{day} from 6:30pm-7:30pm"}],
                    "location": [{"page": 1, "text": "Ol' Man Island"}],
                    "fee": [{"page": 1, "text": "no fee marker"}],
                },
            }

        preview = build_gold_v3_preview([candidate("Thursday", "thu"), candidate("Friday", "fri")])

        self.assertEqual(1, preview["summary"]["gold_rows"])
        self.assertEqual(1, len(preview["rows"]))
        row = preview["rows"][0]
        self.assertEqual("campfire-on-de-bayou", row["canonical_slug"])
        self.assertEqual(["Thursday", "Friday"], row["schedule"]["normalized"]["days_of_week"])
        self.assertEqual("Thursday and Friday from 6:30pm-7:30pm", row["schedule"]["text"])
        self.assertEqual(
            "Thursday and Friday from 6:30pm-7:30pm",
            row["field_evidence"]["schedule"]["raw_value"],
        )
        self.assertEqual(
            ["Thursday", "Friday"],
            row["field_evidence"]["schedule"]["normalized_value"]["days_of_week"],
        )
        self.assertEqual(
            ["Thursday from 6:30pm-7:30pm", "Friday from 6:30pm-7:30pm"],
            [
                component["raw_value"]
                for component in row["field_evidence"]["schedule"]["merged_component_field_evidence"]
            ],
        )
        self.assertEqual(
            ["Thursday from 6:30pm-7:30pm", "Friday from 6:30pm-7:30pm"],
            [span["text"] for span in row["source_spans"]["schedule"]],
        )

    def test_promote_gold_v3_strips_fee_marker_from_inferred_location_public_label(self) -> None:
        candidate = {
            "validation_status": "auto_publishable",
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
            ...
            "canonical_page_image_sha256s": ["pagehash"],
            "source_pages": [{"page_number": 1, "page_image_sha256": "pagehash"}],
            "source_title": "ARCADIA GAMES ($)",
            "normalized_title": "arcadia games ($)",
            "normalized_location_id": "arcadia_games",
            "location_text": "ARCADIA GAMES ($)",
            "schedule_raw": "Daily from 8:00am-11:00pm",
            "schedule_normalized": {
                "schedule_type": "recurring",
                "days_of_week": ALL_DAYS,
                "start_time": "08:00",
                "end_time": "23:00",
                "timezone": "America/New_York",
                "raw_text": "Daily from 8:00am-11:00pm",
            },
            "fee_required": True,
            "field_evidence": {
                "title": {
                    "raw_value": "ARCADIA GAMES ($)",
                    "normalized_value": "arcadia games ($)",
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "ARCADIA GAMES ($)", "confidence": 0.99},
                        {"engine": "rapidocr", "text": "ARCADIA GAMES ($)", "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
                "schedule": {
                    "raw_value": "Daily from 8:00am-11:00pm",
                    "normalized_value": {
                        "schedule_type": "recurring",
                        "days_of_week": ALL_DAYS,
                        "start_time": "08:00",
                    ...
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "Daily from 8:00am-11:00pm", "confidence": 0.99},
                        {"engine": "rapidocr", "text": "Daily from 8:00am-11:00pm", "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
                "location": {
                    "raw_value": "ARCADIA GAMES ($)",
                    "normalized_value": "arcadia_games",
                ...
                "fee": {
                    ...
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "($)", "confidence": 0.99},
                        {"engine": "rapidocr", "text": "($)", "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
            },
            "source_spans": {
                "title": [{"page": 1, "text": "ARCADIA GAMES ($)"}],
                "schedule": [{"page": 1, "text": "Daily from 8:00am-11:00pm"}],
                "location": [{"page": 1, "text": "ARCADIA GAMES ($)"}],
                "fee": [{"page": 1, "text": "ARCADIA GAMES ($)"}],
            },
        }

        [row] = build_gold_v3_preview([candidate])["rows"]

        self.assertEqual("ARCADIA GAMES", row["location"]["label"])
        self.assertEqual("ARCADIA GAMES ($)", row["field_evidence"]["location"]["raw_value"])
        self.assertEqual("ARCADIA GAMES ($)", row["source_spans"]["location"][0]["text"])

    def test_promote_gold_v3_uses_controlled_public_location_label_without_rewriting_evidence(self) -> None:
        def candidate_with_location(
            *,
            ...
            "canonical_page_image_sha256s": ["pagehash"],
            "source_pages": [{"page_number": 1, "page_image_sha256": "pagehash"}],
            "source_title": "ARTS AND CRAFTS ($)",
            "normalized_title": "arts and crafts ($)",
            "normalized_location_id": normalized_location_id,
            "location_text": location_text,
            "schedule_raw": "3:30pm-4:30pm",
            "schedule_normalized": {
                "schedule_type": "time_range",
                "start_time": "15:30",
                "end_time": "16:30",
                "timezone": "America/New_York",
                "raw_text": "3:30pm-4:30pm",
            },
            "fee_required": True,
            "field_evidence": {
                "title": {
                    "raw_value": "ARTS AND CRAFTS ($)",
                    "normalized_value": "arts and crafts ($)",
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "ARTS AND CRAFTS ($)", "confidence": 0.99},
                        {"engine": "rapidocr", "text": "ARTS AND CRAFTS ($)", "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
                "schedule": {
                    "raw_value": "3:30pm-4:30pm",
                    "normalized_value": {
                        "schedule_type": "time_range",
                        "start_time": "15:30",
                        "end_time": "16:30",
                    ...
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": "3:30pm-4:30pm", "confidence": 0.99},
                        {"engine": "rapidocr", "text": "3:30pm-4:30pm", "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
                "location": {
                    "raw_value": location_text,
                    "normalized_value": normalized_location_id,
                    "source": {"content_sha256": "sourcehash", "page_number": 1, "page_image_sha256": "pagehash", "bbox_px": [1, 2, 3, 4]},
                    "engines": [
                        {"engine": "paddleocr_ppstructurev3", "text": location_text, "confidence": 0.99},
                        {"engine": "rapidocr", "text": location_text, "confidence": 0.98},
                    ],
                    "agreement": "exact_after_normalization",
                },
                "fee": {
            ...
            "source_spans": {
                "title": [{"page": 1, "text": "ARTS AND CRAFTS ($)"}],
                "schedule": [{"page": 1, "text": "3:30pm-4:30pm"}],
                "location": [{"page": 1, "text": location_text}],
                "fee": [{"page": 1, "text": "ARTS AND CRAFTS ($)"}],
            },
        }

        cases = [
            ("the_dig_site", "The Dig Site from", "The Dig Site"),
            (
                "contemporary_feature_pool_and_bay_cove_pool",
                "Contemporary Feature Pool and Bay Cove Pool:",
                "Contemporary Feature Pool and Bay Cove Pool",
            ),
            ("the_reel_spot", "THE REEL SPOT I", "The Reel Spot"),
        ]
        for normalized_location_id, location_text, expected_label in cases:
            with self.subTest(normalized_location_id=normalized_location_id):
                [row] = build_gold_v3_preview(
                ...
                self.assertEqual(expected_label, row["location"]["label"])
                self.assertEqual(location_text, row["field_evidence"]["location"]["raw_value"])
                self.assertEqual(location_text, row["source_spans"]["location"][0]["text"])
    ...
    def test_promote_gold_v3_requires_review_coverage_for_every_field_finding(self) -> None:
        needs_review = {
            "candidate_id": "cand-review-multi",
            "validation_status": "needs_review",
            "validation_findings": ["engine_disagreement:title", "engine_disagreement:schedule"],
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
            "calendar_group_key": "boardwalk",
            "resort_slug": "boardwalk-inn",
        ...
        schedule_task = {
            "task_id": "cand-review-multi-schedule",
            "task_type": "ocr_disagreement",
            "field": "schedule",
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
        ...
        title_task = {
            **schedule_task,
            "task_id": "cand-review-multi-title",
            "field": "title",
            "field_crop": "title.png",
            "field_crop_sha256": "titlecrop",
        }
        schedule_decision = build_review_decision(
            schedule_task,
            reviewer="josh",
            decision="edit",
            approved_fields={
        ...
        title_decision = build_review_decision(
            title_task,
            reviewer="josh",
            decision="approve",
            approved_fields={"title": {"raw_value": "Poolside Activities", "normalized_value": "Poolside Activities"}},
            reason="Confirmed visible title crop.",
            decided_at="2026-06-28T12:01:00Z",
        )

        partial_preview = build_gold_v3_preview([needs_review], review_decisions=[schedule_decision])
        covered_preview = build_gold_v3_preview(
            [needs_review],
            review_decisions=[schedule_decision, title_decision],
        )

        self.assertEqual([], partial_preview["rows"])
        self.assertEqual(1, partial_preview["summary"]["skipped_incomplete_review_decision"])
        self.assertEqual(1, len(covered_preview["rows"]))
        row = covered_preview["rows"][0]
        self.assertEqual([], row["validation_findings"])
        self.assertEqual(
            ["engine_disagreement:title", "engine_disagreement:schedule"],
            row["review_decision"]["original_validation_findings"],
        )
        self.assertEqual({"schedule", "title"}, set(row["review_decision"]["approved_fields"]))

    def test_promote_gold_v3_rejects_review_with_stale_field_page_image_hash(self) -> None:
        needs_review = {
            "candidate_id": "cand-review-page-hash",
            "validation_status": "needs_review",
            "validation_findings": ["engine_disagreement:schedule"],
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
            ...
            "field_evidence": {
                "title": {
                    "source": {
                        "content_sha256": "sourcehash",
                        "page_number": 1,
                        "page_image_sha256": "pagehash-page-1",
                        "bbox_px": [10, 10, 100, 30],
                        "crop_storage_path": "title.png",
                        "crop_sha256": "titlecrop",
                    },
                ...
                "schedule": {
                    "source": {
                        "content_sha256": "sourcehash",
                        "page_number": 2,
                        "page_image_sha256": "pagehash-page-2",
                        "bbox_px": [10, 10, 100, 30],
                        "crop_storage_path": "schedule.png",
                        "crop_sha256": "schedulecrop",
                    },
                ...
                "location": {
                    "source": {
                        ...
                        "page_image_sha256": "pagehash-page-1",
                        "bbox_px": [10, 10, 100, 30],
                        "crop_storage_path": "location.png",
                        "crop_sha256": "locationcrop",
                    },
                    "agreement": "manual_review_required",
                },
                "fee": {
        ...
        stale_page_task = {
            "task_id": "cand-review-page-hash",
            "task_type": "ocr_disagreement",
            "field": "schedule",
            "source_document_id": "source-doc-1",
            "content_sha256": "sourcehash",
            "page_image": "pagehash-page-1",
            "page_image_sha256": "pagehash-page-1",
            "field_crop": "schedule.png",
            "field_crop_sha256": "schedulecrop",
            "candidate": needs_review,
        ...
        stale_page_decision = build_review_decision(
            stale_page_task,
            reviewer="josh",
            decision="edit",
        ...
        current_page_decision = build_review_decision(
            current_page_task,
            reviewer="josh",
            decision="edit",
            approved_fields=approved_fields,
        ...
        stale_preview = build_gold_v3_preview([needs_review], review_decisions=[stale_page_decision])
        current_preview = build_gold_v3_preview([needs_review], review_decisions=[current_page_decision])

        self.assertEqual([], stale_preview["rows"])
        self.assertEqual(1, stale_preview["summary"]["skipped_stale_review_decision"])
        self.assertEqual(1, len(current_preview["rows"]))
        self.assertEqual("pagehash-page-2", current_preview["rows"][0]["review_decision"]["page_image_sha256"])
    ...
    def test_publish_gold_v3_requires_feature_flag_and_clean_preview(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
                            ...
                            "schedule": {
                                **clean_preview["rows"][0]["field_evidence"]["schedule"],
                                ...
                                "engines": [
                                    ...
                                    {"engine": "rapidocr", "text": "From 3:30pm-4:30pm", "confidence": 0.94},
                                ],
                            },
                        },
                        "source_spans": {
                            **clean_preview["rows"][0]["source_spans"],
                            "schedule": [{"page": 1, "text": "From 3:30pm-4:30pm"}],
                        },
                    }
                ],
            }
            ...
            trusted_primary_schedule_preview = {
                "publication_mode": "vision_v3_preview",
                "generated_at": "2026-06-28T12:00:00+00:00",
                "summary": {"gold_rows": 1, "skipped_not_publishable": 0},
                "rows": [
                            ...
                            "schedule": {
                                **clean_preview["rows"][0]["field_evidence"]["schedule"],
                                ...
                                "engines": [
                                    {
                                        "engine": "paddleocr_ppstructurev3",
                                        "text": "Monday at 1:30pm",
                                        "confidence": 0.999,
                                    },
                                    {"engine": "rapidocr", "text": "OMan Isiznrl", "confidence": 0.77},
                                ],
                            },
                        },
                    }
            ...
            trusted_primary_location_preview = {
                "publication_mode": "vision_v3_preview",
                "generated_at": "2026-06-28T12:00:00+00:00",
                "summary": {"gold_rows": 1, "skipped_not_publishable": 0},
                "rows": [
                    {
                        **clean_preview["rows"][0],
                        "field_evidence": {
                            **clean_preview["rows"][0]["field_evidence"],
                            "location": {
                                **clean_preview["rows"][0]["field_evidence"]["location"],
                                "agreement": "disagreement",
                                "engines": [
                                    {
                                        "engine": "paddleocr_ppstructurev3",
                                        "text": "Village Green Lawn",
                                        "confidence": 0.999,
                                    },
                                    {"engine": "rapidocr", "text": "O. Man Ielanel Facl Derk", "confidence": 0.78},
                                ],
                            },
                        },
                    }
            ...
            unknown_family_auto_publishable_preview = {
                "publication_mode": "vision_v3_preview",
                "generated_at": "2026-06-28T12:00:00+00:00",
                "summary": {"gold_rows": 1, "skipped_not_publishable": 0},
                "rows": [{**clean_preview["rows"][0], "document_family": "unknown_visual_schedule"}],
            }
            ...
            manual_with_placeholder_reviewer_preview = {
                "publication_mode": "vision_v3_preview",
                "summary": {"gold_rows": 1, "skipped_not_publishable": 0},
                "rows": [
                    {
            ...
            manual_with_placeholder_reason_preview = {
                "publication_mode": "vision_v3_preview",
                "summary": {"gold_rows": 1, "skipped_not_publishable": 0},
                "rows": [
                    {
            ...
            placeholder_reviewer_source_drift_report = {
                **reviewed_source_drift_report,
                "review_decision": {
                    **reviewed_source_drift_report["review_decision"],
                    "reviewer": "reviewer-name",
                },
            }
            placeholder_reason_source_drift_report = {
                **reviewed_source_drift_report,
                "review_decision": {
                    **reviewed_source_drift_report["review_decision"],
                    "reason": "Verified against source crop",
            ...
            unknown_family_auto_publishable = evaluate_publish_readiness(
                unknown_family_auto_publishable_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
            )
            ...
            time_range_schedule = evaluate_publish_readiness(
                time_range_schedule_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_coverage_report(time_range_schedule_preview),
            )
            ...
            trusted_primary_schedule = evaluate_publish_readiness(
                trusted_primary_schedule_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
            )
            trusted_primary_location = evaluate_publish_readiness(
                trusted_primary_location_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
            )
            ...
            manual_with_placeholder_reviewer = evaluate_publish_readiness(
                manual_with_placeholder_reviewer_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
            )
            manual_with_placeholder_reason = evaluate_publish_readiness(
                manual_with_placeholder_reason_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
            )
            ...
            source_drift_placeholder_reviewer = evaluate_publish_readiness(
                clean_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
                source_drift_report=placeholder_reviewer_source_drift_report,
            )
            source_drift_placeholder_reason = evaluate_publish_readiness(
                clean_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report=clean_dual_run_report,
                source_drift_report=placeholder_reason_source_drift_report,
            )
            ...
            dual_run_stale_source_status_schema = evaluate_publish_readiness(
                clean_preview,
                flags=load_publish_flags(enabled_flags),
                dual_run_report={
                    **clean_dual_run_report,
                    "source_coverage": {
                        **clean_dual_run_report["source_coverage"],
                        "stale_source_status_schema_ids": ["source-doc-schema"],
                    },
                },
            )
        ...
        self.assertFalse(unknown_family_auto_publishable["ready"])
        self.assertIn("row_document_family_not_auto_publishable:0", unknown_family_auto_publishable["errors"])
        ...
        self.assertTrue(time_range_schedule["ready"], time_range_schedule)
        ...
        self.assertTrue(trusted_primary_schedule["ready"], trusted_primary_schedule)
        self.assertTrue(trusted_primary_location["ready"], trusted_primary_location)
        ...
        self.assertFalse(manual_with_placeholder_reviewer["ready"])
        self.assertIn("row_placeholder_manual_review_reviewer:0", manual_with_placeholder_reviewer["errors"])
        self.assertFalse(manual_with_placeholder_reason["ready"])
        self.assertIn("row_placeholder_manual_review_reason:0", manual_with_placeholder_reason["errors"])
        ...
        self.assertFalse(source_drift_placeholder_reviewer["ready"])
        self.assertIn(
            "source_drift_report_placeholder_reviewer",
            source_drift_placeholder_reviewer["errors"],
        )
        self.assertFalse(source_drift_placeholder_reason["ready"])
        self.assertIn(
            "source_drift_report_placeholder_reason",
            source_drift_placeholder_reason["errors"],
        )
        ...
        self.assertFalse(dual_run_stale_source_status_schema["ready"])
        self.assertIn(
            "dual_run_stale_source_status_schema",
            dual_run_stale_source_status_schema["errors"],
        )

    def test_publish_gold_v3_rejects_unparseable_review_decided_at(self) -> None:
        flags = {"public_gold_source": "v3", "v3_publish_enabled": True}
        manual_preview = clean_publish_v3_preview()
        manual_preview["rows"][0] = {
            **manual_preview["rows"][0],
            "validation_status": "manually_approved",
            "review_status": "manual_review_approved",
            "review_decision": {
        ...
        source_drift_report = {
            ...
            "review_decision": {
                ...
                "reviewer": "josh",
                "decided_at": "not-a-date",
                "reason": "Reviewed source drift report.",
            },
        }

        manual_result = evaluate_publish_readiness(
            manual_preview,
            flags=flags,
            dual_run_report=clean_dual_run_coverage_report(manual_preview),
        )
        source_drift_result = evaluate_publish_readiness(
            clean_publish_v3_preview(),
            flags=flags,
            dual_run_report=clean_dual_run_coverage_report(),
            source_drift_report=source_drift_report,
        )

        self.assertFalse(manual_result["ready"])
        self.assertIn("row_invalid_manual_review_decided_at:0", manual_result["errors"])
        self.assertFalse(source_drift_result["ready"])
        self.assertIn("source_drift_report_invalid_decided_at", source_drift_result["errors"])
    ...
    def test_publish_gold_v3_requires_fresh_passing_monitoring_report_when_enabled(self) -> None:
        ...
        wrong_kind = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            monitoring_report={**passing_monitoring_report, "report_kind": "other_report"},
            require_monitoring_report=True,
        )
        missing_schema_version = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            monitoring_report={
                key: value
                for key, value in passing_monitoring_report.items()
                if key != "schema_version"
            },
            require_monitoring_report=True,
        )
        stale_schema_version = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            monitoring_report={
                **passing_monitoring_report,
                "schema_version": "source_trust_monitoring_report_000",
            },
            require_monitoring_report=True,
        )
        ...
        self.assertFalse(wrong_kind["ready"])
        self.assertIn("invalid_monitoring_report_kind", wrong_kind["errors"])
        self.assertFalse(missing_schema_version["ready"])
        self.assertIn("monitoring_report_missing_schema_version", missing_schema_version["errors"])
        self.assertFalse(stale_schema_version["ready"])
        self.assertIn("invalid_monitoring_report_schema_version", stale_schema_version["errors"])
        self.assertTrue(passed["ready"])

    def test_publish_gold_v3_requires_fresh_source_metrics_report_when_enabled(self) -> None:
        preview = clean_publish_v3_preview(generated_at="2026-06-28T12:00:00+00:00")
        preview["summary"]["input_candidates"] = 1
        flags = {"public_gold_source": "v3", "v3_publish_enabled": True}
        clean_dual_run_report = clean_dual_run_coverage_report()
        clean_metrics_report = {
            "report_kind": "vision_v3_source_metrics",
            "schema_version": "vision_v3_source_metrics_001",
            "generated_at": "2026-06-28T12:01:00+00:00",
            "v3_preview_fingerprint": publish_gold_v3._dual_run_preview_fingerprint(preview),
            "summary": {
                "source_count": 1,
                "candidate_count": preview["summary"]["input_candidates"],
            },
        ...
        stale = evaluate_publish_readiness(
            ...
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={**clean_metrics_report, "generated_at": "2026-06-28T11:59:00+00:00"},
            require_source_metrics_report=True,
        )
        wrong_kind = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={**clean_metrics_report, "report_kind": "other_report"},
            require_source_metrics_report=True,
        )
        count_mismatch = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={
                **clean_metrics_report,
                "summary": {**clean_metrics_report["summary"], "candidate_count": 0},
            },
            require_source_metrics_report=True,
        )
        missing_source = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={**clean_metrics_report, "sources": [{"source_sha256": "otherhash"}]},
            require_source_metrics_report=True,
        )
        fingerprint_mismatch = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={**clean_metrics_report, "v3_preview_fingerprint": "old-preview-fingerprint"},
            require_source_metrics_report=True,
        )
        missing_fingerprint = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={
                key: value
                for key, value in clean_metrics_report.items()
        ...
        stale_schema_version = evaluate_publish_readiness(
            ...
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_metrics_report={**clean_metrics_report, "schema_version": "vision_v3_source_metrics_000"},
            require_source_metrics_report=True,
        )
        missing_expected_source = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report={
                **clean_dual_run_report,
                "source_coverage": {
                    **clean_dual_run_report["source_coverage"],
                    "expected_source_count": 2,
                    "expected_source_hashes": ["sourcehash", "source-without-preview-row"],
                    "expected_source_hash_count": 2,
                    "processed_source_count": 2,
                },
            },
            source_metrics_report=clean_metrics_report,
            require_source_metrics_report=True,
        )
        passed = evaluate_publish_readiness(
            preview,
        ...
        self.assertFalse(missing["ready"])
        self.assertIn("missing_source_metrics_report", missing["errors"])
        self.assertFalse(stale["ready"])
        self.assertIn("source_metrics_report_stale", stale["errors"])
        self.assertFalse(wrong_kind["ready"])
        self.assertIn("invalid_source_metrics_report_kind", wrong_kind["errors"])
        self.assertFalse(count_mismatch["ready"])
        self.assertIn("source_metrics_candidate_count_mismatch", count_mismatch["errors"])
        self.assertFalse(missing_source["ready"])
        self.assertIn("source_metrics_missing_preview_sources", missing_source["errors"])
        self.assertFalse(fingerprint_mismatch["ready"])
        self.assertIn("source_metrics_preview_fingerprint_mismatch", fingerprint_mismatch["errors"])
        self.assertFalse(missing_fingerprint["ready"])
        self.assertIn("source_metrics_missing_preview_fingerprint", missing_fingerprint["errors"])
        self.assertFalse(missing_schema_version["ready"])
        self.assertIn("source_metrics_missing_schema_version", missing_schema_version["errors"])
        self.assertFalse(stale_schema_version["ready"])
        self.assertIn("invalid_source_metrics_schema_version", stale_schema_version["errors"])
        self.assertFalse(missing_expected_source["ready"])
        self.assertIn("source_metrics_missing_dual_run_expected_sources", missing_expected_source["errors"])
        self.assertTrue(passed["ready"], passed)

    def test_publish_gold_v3_blocks_pending_review_queue_when_required(self) -> None:
        preview = clean_publish_v3_preview(generated_at="2026-06-28T12:00:00+00:00")
        flags = {"public_gold_source": "v3", "v3_publish_enabled": True}
        clean_dual_run_report = clean_dual_run_coverage_report()
        blocked_dual_run_report = {
            ...
            "field_diffs": [
                {
                    ...
                    "canonical_slug": "poolside-activities",
                    "field": "schedule",
                    "v2_value": "Daily at 1:00pm",
                    "v3_value": "Daily at 1:30pm",
                    "content_sha256": "sourcehash",
                    "v3_field_evidence": {
                        "source": {
                            "content_sha256": "sourcehash",
        ...
        pending = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=[
        ...
        resolved_tasks = [
            {
                ...
                "task_type": "ocr_disagreement",
                "content_sha256": "hash-1",
                "field_crop": "https://example.test/review-1.png",
                "source_url": "https://example.test/official.pdf",
                "validation_findings": ["engine_disagreement:schedule"],
            }
        ]
        resolved_report = build_review_queue_report(resolved_tasks)
        resolved = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report=resolved_report,
            require_review_queue=True,
        )
        missing_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=[],
        ...
        stale_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=[],
            review_queue_report={
                **build_review_queue_report([]),
                "summary": {**build_review_queue_report([])["summary"], "task_count": 1},
            },
            require_review_queue=True,
        )
        missing_visual_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=[],
            review_queue_report={
                **build_review_queue_report([]),
                "summary": {
                    **build_review_queue_report([])["summary"],
                    "visual_artifact_missing_count": 1,
                    "all_tasks_have_visual_artifacts": False,
                },
                "visual_artifact_missing_task_ids": ["review-1"],
            },
            require_review_queue=True,
        )
        missing_visual_file_tasks = [
            {
                **resolved_tasks[0],
                "field_crop": "fields/review-1.png",
            }
        ]
        missing_visual_file_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=missing_visual_file_tasks,
            review_queue_report=build_review_queue_report(missing_visual_file_tasks),
            require_review_queue=True,
        )
        missing_source_url_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={
                **resolved_report,
                "summary": {
                    **resolved_report["summary"],
                    "source_url_missing_count": 1,
                    "all_tasks_have_source_urls": False,
                },
                "source_url_missing_task_ids": ["review-1"],
            },
            require_review_queue=True,
        )
        stale_pending_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={
                **resolved_report,
                "summary": {**resolved_report["summary"], "pending_task_count": 1},
            },
            require_review_queue=True,
        )
        stale_review_decision_tasks = [
            {
                **resolved_tasks[0],
                "stale_review_decision": {
                    "decision": "approve",
                    "reviewer": "josh",
                    "stale_reasons": ["stale_review_page_image_sha256"],
                },
            }
        ]
        stale_review_decision_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=stale_review_decision_tasks,
            review_queue_report=build_review_queue_report(stale_review_decision_tasks),
            require_review_queue=True,
        )
        current_content_tasks = [
            {
                **resolved_tasks[0],
                "content_sha256": "current-content",
                "page_image_sha256": "current-page",
        ...
        stale_content_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=current_content_tasks,
            review_queue_report=build_review_queue_report(
                [
                    {
                        **current_content_tasks[0],
                        "content_sha256": "stale-content",
                        "page_image_sha256": "stale-page",
                        "field_crop_sha256": "stale-crop",
                    }
                ]
            ),
            require_review_queue=True,
        )
        missing_fingerprint_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={
                key: value
                for key, value in resolved_report.items()
                if key != "review_queue_fingerprint"
            },
            require_review_queue=True,
        )
        missing_schema_version_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
        ...
        stale_schema_version_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={**resolved_report, "schema_version": "review_queue_v3_report_000"},
            require_review_queue=True,
        )
        missing_review_queue_generated_at_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
        ...
        stale_review_queue_generated_at_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={**resolved_report, "generated_at": "2026-06-28T11:59:00+00:00"},
            require_review_queue=True,
        )
        stale_publish_gate_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=resolved_tasks,
            review_queue_report={
                **resolved_report,
                "publish_gate_blockers": {
                    **resolved_report["publish_gate_blockers"],
                    "pending_task_count": 1,
                    "pending_task_count_by_reason": {"unreviewed_new_v3_rows": 1},
                },
            },
            require_review_queue=True,
        )
        unresolved_publish_gate_tasks = [
            {
                **resolved_tasks[0],
                "publish_gate_blocker": True,
                "publish_gate_blocker_reason": "unreviewed_new_v3_rows",
        ...
        stale_publish_gate_summary_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "summary": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["summary"],
                    "publish_gate_blocker_task_count": 2,
                    "pending_publish_gate_blocker_task_count": 0,
                    "reviewed_publish_gate_blocker_task_count": 2,
                    "publish_blocker_checklist_row_count": 0,
                    "publish_blocker_checklist_batch_count": 0,
                    "pending_publish_gate_blocker_count_by_reason": {},
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_reason_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_gate_blockers": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["publish_gate_blockers"],
                    "pending_task_count_by_reason": {"unreviewed_v2_v3_field_diffs": 1},
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_source_url_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_gate_blockers": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["publish_gate_blockers"],
                    "pending_task_count_by_source_url": {"https://example.test/wrong.pdf": 1},
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_reason_ids_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_gate_blockers": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["publish_gate_blockers"],
                    "pending_task_ids_by_reason": {"unreviewed_new_v3_rows": ["wrong-task"]},
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_source_url_ids_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_gate_blockers": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["publish_gate_blockers"],
                    "pending_task_ids_by_source_url": {"https://example.test/official.pdf": ["wrong-task"]},
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_pending_ids_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_gate_blockers": {
                    **build_review_queue_report(unresolved_publish_gate_tasks)["publish_gate_blockers"],
                    "pending_task_ids": ["wrong-task"],
                },
            },
            require_review_queue=True,
        )
        stale_publish_gate_batches_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report={
                **build_review_queue_report(unresolved_publish_gate_tasks),
                "publish_blocker_batches": [
                    {
                        **build_review_queue_report(unresolved_publish_gate_tasks)["publish_blocker_batches"][0],
                        "task_ids": ["wrong-task"],
                    }
                ],
            },
        ...
        unresolved_publish_gate_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            review_tasks=unresolved_publish_gate_tasks,
            review_queue_report=build_review_queue_report(unresolved_publish_gate_tasks),
            require_review_queue=True,
        )
        missing_dual_run_review_task_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=blocked_dual_run_report,
            review_tasks=[],
            review_queue_report=build_review_queue_report([]),
            require_review_queue=True,
        )
        covered_dual_run_tasks = build_review_tasks([], dual_run_report=blocked_dual_run_report)
        covered_dual_run_review_task_report = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=blocked_dual_run_report,
            review_tasks=covered_dual_run_tasks,
            review_queue_report=build_review_queue_report(covered_dual_run_tasks),
            require_review_queue=True,
        )

        self.assertFalse(missing["ready"])
        self.assertIn("missing_review_queue", missing["errors"])
        self.assertFalse(pending["ready"])
        self.assertIn("pending_review_queue_tasks", pending["errors"])
        self.assertEqual(1, pending["pending_review_task_count"])
        self.assertFalse(missing_report["ready"])
        self.assertIn("missing_review_queue_report", missing_report["errors"])
        self.assertFalse(stale_report["ready"])
        self.assertIn("review_queue_report_task_count_mismatch", stale_report["errors"])
        self.assertFalse(missing_visual_report["ready"])
        self.assertIn("review_queue_report_missing_visual_artifacts", missing_visual_report["errors"])
        self.assertFalse(missing_visual_file_report["ready"])
        self.assertIn("review_queue_report_missing_visual_artifact_files", missing_visual_file_report["errors"])
        self.assertFalse(missing_source_url_report["ready"])
        self.assertIn("review_queue_report_missing_source_urls", missing_source_url_report["errors"])
        self.assertFalse(stale_pending_report["ready"])
        self.assertIn("review_queue_report_pending_task_count_mismatch", stale_pending_report["errors"])
        self.assertFalse(stale_review_decision_report["ready"])
        self.assertIn("review_queue_report_stale_review_decisions", stale_review_decision_report["errors"])
        self.assertFalse(stale_content_report["ready"])
        self.assertIn("review_queue_report_fingerprint_mismatch", stale_content_report["errors"])
        self.assertFalse(missing_fingerprint_report["ready"])
        self.assertIn("review_queue_report_missing_fingerprint", missing_fingerprint_report["errors"])
        self.assertFalse(missing_schema_version_report["ready"])
        self.assertIn("review_queue_report_missing_schema_version", missing_schema_version_report["errors"])
        self.assertFalse(stale_schema_version_report["ready"])
        self.assertIn("invalid_review_queue_report_schema_version", stale_schema_version_report["errors"])
        self.assertFalse(missing_review_queue_generated_at_report["ready"])
        self.assertIn(
            "review_queue_report_missing_generated_at",
            missing_review_queue_generated_at_report["errors"],
        )
        self.assertFalse(stale_review_queue_generated_at_report["ready"])
        self.assertIn("review_queue_report_stale", stale_review_queue_generated_at_report["errors"])
        self.assertFalse(stale_publish_gate_report["ready"])
        self.assertIn("review_queue_report_pending_publish_gate_blockers", stale_publish_gate_report["errors"])
        self.assertFalse(stale_publish_gate_summary_report["ready"])
        self.assertIn(
            "review_queue_report_summary_publish_gate_blocker_count_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertIn(
            "review_queue_report_summary_pending_publish_gate_blocker_count_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertIn(
            "review_queue_report_summary_reviewed_publish_gate_blocker_count_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertIn(
            "review_queue_report_summary_publish_gate_blocker_reason_counts_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertIn(
            "review_queue_report_summary_publish_blocker_checklist_row_count_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertIn(
            "review_queue_report_summary_publish_blocker_checklist_batch_count_mismatch",
            stale_publish_gate_summary_report["errors"],
        )
        self.assertFalse(stale_publish_gate_reason_report["ready"])
        self.assertIn(
            "review_queue_report_publish_gate_blocker_reason_counts_mismatch",
            stale_publish_gate_reason_report["errors"],
        )
        self.assertFalse(stale_publish_gate_source_url_report["ready"])
        self.assertIn(
            "review_queue_report_publish_gate_blocker_source_url_counts_mismatch",
            stale_publish_gate_source_url_report["errors"],
        )
        self.assertFalse(stale_publish_gate_reason_ids_report["ready"])
        self.assertIn(
            "review_queue_report_publish_gate_blocker_reason_task_ids_mismatch",
            stale_publish_gate_reason_ids_report["errors"],
        )
        self.assertFalse(stale_publish_gate_source_url_ids_report["ready"])
        self.assertIn(
            "review_queue_report_publish_gate_blocker_source_url_task_ids_mismatch",
            stale_publish_gate_source_url_ids_report["errors"],
        )
        self.assertFalse(stale_publish_gate_pending_ids_report["ready"])
        self.assertIn(
            "review_queue_report_pending_publish_gate_task_ids_mismatch",
            stale_publish_gate_pending_ids_report["errors"],
        )
        self.assertFalse(stale_publish_gate_batches_report["ready"])
        self.assertIn(
            "review_queue_report_publish_blocker_batches_mismatch",
            stale_publish_gate_batches_report["errors"],
        )
        self.assertFalse(unresolved_publish_gate_report["ready"])
        self.assertIn("review_queue_report_pending_publish_gate_blockers", unresolved_publish_gate_report["errors"])
        self.assertIn(
            "review_queue_report_pending_publish_gate_review_artifacts_missing",
            unresolved_publish_gate_report["errors"],
        )
        self.assertNotIn(
            "review_queue_report_pending_publish_gate_blocker_count_mismatch",
            unresolved_publish_gate_report["errors"],
        )
        self.assertFalse(missing_dual_run_review_task_report["ready"])
        self.assertIn(
            "review_queue_missing_dual_run_review_tasks",
            missing_dual_run_review_task_report["errors"],
        )
        self.assertIn(
            "dual-run-diff-boardwalk-poolside-activities-schedule",
            missing_dual_run_review_task_report["missing_dual_run_review_task_ids"],
        )
        self.assertNotIn(
            "review_queue_missing_dual_run_review_tasks",
            covered_dual_run_review_task_report["errors"],
        )
        self.assertTrue(resolved["ready"])
        self.assertEqual(0, resolved["pending_review_task_count"])

    def test_publish_gold_v3_requires_source_backed_dual_run_review_manifests(self) -> None:
        preview = clean_publish_v3_preview(generated_at="2026-06-28T12:00:00+00:00")
        flags = {"public_gold_source": "v3", "v3_publish_enabled": True}
        base_review = {
        ...
        missing_source = evaluate_publish_readiness(
            ...
            dual_run_report={
                **clean_dual_run_coverage_report(preview),
                "reviewed_diff_reviews": [base_review],
            },
        )
        non_public_source = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report={
                **clean_dual_run_coverage_report(preview),
                "reviewed_diff_reviews": [
                    {
                        **base_review,
                        "source_url": "manual://boardwalk/review",
                        "source_metadata": {"canonical_url": "manual://boardwalk/review"},
                    }
                ],
            },
        )
        missing_metadata = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report={
                **clean_dual_run_coverage_report(preview),
        ...
        self.assertFalse(missing_source["ready"])
        self.assertIn("dual_run_review_manifest_missing_source_url:reviewed_diff_reviews:0", missing_source["errors"])
        self.assertFalse(non_public_source["ready"])
        self.assertIn("dual_run_review_manifest_non_public_source_url:reviewed_diff_reviews:0", non_public_source["errors"])
        self.assertFalse(missing_metadata["ready"])
        self.assertIn("dual_run_review_manifest_missing_source_metadata:reviewed_diff_reviews:0", missing_metadata["errors"])
        self.assertTrue(passed["ready"], passed)
    ...
    def test_publish_gold_v3_accepts_display_cleaned_time_range_schedule_text(self) -> None:
        preview = clean_publish_v3_preview()
        row = preview["rows"][0]
        row["schedule"] = {
            "text": "3:30pm-4:30pm",
        ...
        row["field_evidence"]["schedule"]["raw_value"] = "From 3:30pm-4:30pm"
        row["field_evidence"]["schedule"]["normalized_value"] = {
            "schedule_type": "time_range",
            "start_time": "15:30",
            "end_time": "16:30",
            "timezone": "America/New_York",
        ...
        row["field_evidence"]["schedule"]["engines"] = [
            {"engine": "paddleocr_ppstructurev3", "text": "From 3:30pm-4:30pm", "confidence": 0.99},
            {"engine": "rapidocr", "text": "From 3:30pm-4:30pm", "confidence": 0.98},
        ]
        ...
        self.assertNotIn("row_critical_field_raw_value_mismatch:0:schedule", readiness["errors"])

    def test_publish_gold_v3_accepts_inferred_location_from_title_evidence(self) -> None:
        preview = clean_publish_v3_preview()
        row = preview["rows"][0]
        row["title"] = "arcadia games ($)"
        row["location"] = {"id": "arcadia_games", "label": "ARCADIA GAMES ($)"}
        row["field_evidence"]["title"]["raw_value"] = "ARCADIA GAMES ($)"
        row["field_evidence"]["title"]["normalized_value"] = "arcadia games ($)"
        row["field_evidence"]["title"]["engines"] = [
            {"engine": "paddleocr_ppstructurev3", "text": "ARCADIA GAMES ($)", "confidence": 0.99},
            {"engine": "rapidocr", "text": "ARCADIA GAMES ($)", "confidence": 0.98},
        ]
        row["field_evidence"]["location"] = {
            **row["field_evidence"]["location"],
            "raw_value": "ARCADIA GAMES ($)",
            "normalized_value": "arcadia_games",
            "inferred_location_from_title": True,
        ...
        row["source_spans"]["title"] = [{"page": 1, "text": "ARCADIA GAMES ($)"}]
        row["source_spans"]["location"] = [{"page": 1, "text": "ARCADIA GAMES ($)"}]
        ...
        self.assertNotIn("row_critical_field_raw_value_mismatch:0:location", readiness["errors"])

    def test_publish_gold_v3_accepts_recovered_movie_title_from_secondary_ocr(self) -> None:
        preview = clean_publish_v3_preview()
        row = preview["rows"][0]
        row["candidate_type"] = "movie"
        row["canonical_slug"] = "movie-under-the-stars"
        row["movie_title"] = "Toy Story 3"
        row["normalized_movie_title"] = "toy story 3"
        row["source_spans"]["movie_title"] = [{"page": 1, "text": "Toy Story 3"}]
        row["field_evidence"]["movie_title"] = {
            "field": "movie_title",
            "raw_value": "Toy Story 3",
            "normalized_value": "toy story 3",
            "agreement": "exact_after_normalization",
            "review_status": "not_required",
            "recovered_from_secondary_ocr": True,
            "source": {
                "content_sha256": "sourcehash",
                "page_number": 1,
                "region_id": "region-1",
                "bbox_px": [10, 10, 110, 30],
                "crop_sha256": "cropmovie",
                "crop_storage_path": "/tmp/movie.png",
                "page_image_sha256": "pagehash",
            },
            "engines": [
                {"engine": "paddleocr_ppstructurev3", "text": "oy 3", "confidence": 0.72},
                {"engine": "rapidocr", "text": "Toy Story 3", "confidence": 0.99},
            ],
        }
        ...
        self.assertTrue(readiness["ready"])
        self.assertNotIn("row_unresolved_engine_disagreement:0:movie_title", readiness["errors"])
        self.assertNotIn("row_unresolved_engine_disagreement:0:location", readiness["errors"])
    ...
    def test_publish_gold_v3_uses_scoped_preview_for_scoped_dual_run_report(self) -> None:
        preview = clean_publish_v3_preview()
        extra_row = with_source_hash(preview["rows"][0], "other-sourcehash")
        extra_row["canonical_slug"] = "other-activity"
        preview["rows"] = [preview["rows"][0], extra_row]
        preview["summary"] = {"gold_rows": 2, "skipped_not_publishable": 0}
        scoped_preview = {**preview, "rows": [preview["rows"][0]]}
        scoped_report = clean_dual_run_coverage_report(scoped_preview)
        scoped_report["source_coverage"]["expected_source_hashes"] = ["sourcehash"]

        readiness = evaluate_publish_readiness(
            preview,
            flags={"public_gold_source": "v3", "v3_publish_enabled": True},
            dual_run_report=scoped_report,
        )

        self.assertFalse(readiness["ready"])
        self.assertIn("dual_run_preview_has_unscoped_sources", readiness["errors"])
        self.assertNotIn("dual_run_preview_row_count_mismatch", readiness["errors"])
        self.assertNotIn("dual_run_preview_source_hash_mismatch", readiness["errors"])
        self.assertNotIn("dual_run_preview_fingerprint_mismatch", readiness["errors"])
    ...
    def test_publish_gold_v3_rejects_inconsistent_clean_dual_run_backlog(self) -> None:
        preview = clean_publish_v3_preview()
        inconsistent_report = {
            **clean_dual_run_coverage_report(preview),
            "status": "clean",
            "publish_blockers": [],
            "unreviewed_diff_keys": [],
            "unreviewed_new_keys": ["boardwalk:poolside-activities"],
            "unreviewed_missing_v2_keys": ["boardwalk:campfire"],
            "unreviewed_status_transition_keys": [
                "boardwalk:community-hall:needs_review_to_auto_publishable"
            ],
            "manual_review_backlog": 3,
        }

        readiness = evaluate_publish_readiness(
            preview,
            flags={"public_gold_source": "v3", "v3_publish_enabled": True},
            dual_run_report=inconsistent_report,
        )

        self.assertFalse(readiness["ready"])
        self.assertIn("unreviewed_new_v3_row:boardwalk:poolside-activities", readiness["errors"])
        self.assertIn("unreviewed_missing_v2_row:boardwalk:campfire", readiness["errors"])
        self.assertIn(
            "unreviewed_status_transition:boardwalk:community-hall:needs_review_to_auto_publishable",
            readiness["errors"],
        )
        self.assertIn("dual_run_manual_review_backlog_nonzero", readiness["errors"])
    ...
    def test_publish_gold_v3_requires_current_dual_run_report_timestamp(self) -> None:
        ...
        stale_version_report = {
            **clean_dual_run_coverage_report(preview),
            "report_version": "v3_dual_run_report_000",
        }
        stale_report = {
            **clean_dual_run_coverage_report(preview),
        ...
        missing_version = evaluate_publish_readiness(
            preview,
            flags={"public_gold_source": "v3", "v3_publish_enabled": True},
            dual_run_report=missing_version_report,
        )
        stale_version = evaluate_publish_readiness(
            preview,
            flags={"public_gold_source": "v3", "v3_publish_enabled": True},
            dual_run_report=stale_version_report,
        )
        ...
        self.assertFalse(missing_version["ready"])
        self.assertIn("dual_run_missing_report_version", missing_version["errors"])
        self.assertFalse(stale_version["ready"])
        self.assertIn("invalid_dual_run_report_version", stale_version["errors"])
    ...
    def test_publish_gold_v3_requires_source_drift_report_when_enabled(self) -> None:
        ...
        missing_schema_version_source_drift_report = {
            key: value
            for key, value in source_drift_report.items()
            if key != "schema_version"
        }
        stale_schema_version_source_drift_report = {
            **source_drift_report,
            "schema_version": "v3_quarter_source_drift_000",
        }
        ...
        missing_schema_version = evaluate_publish_readiness(
            ...
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_drift_report=missing_schema_version_source_drift_report,
            require_source_drift_report=True,
        )
        stale_schema_version = evaluate_publish_readiness(
            preview,
            flags=flags,
            dual_run_report=clean_dual_run_report,
            source_drift_report=stale_schema_version_source_drift_report,
            require_source_drift_report=True,
        )
        missing_expected_source = evaluate_publish_readiness(
            preview,
        ...
        self.assertFalse(missing_schema_version["ready"])
        self.assertIn("source_drift_report_missing_schema_version", missing_schema_version["errors"])
        self.assertFalse(stale_schema_version["ready"])
        self.assertIn("invalid_source_drift_report_schema_version", stale_schema_version["errors"])
        self.assertFalse(missing_expected_source["ready"])
        self.assertIn("source_drift_report_missing_dual_run_expected_sources", missing_expected_source["errors"])
    ...
    def test_source_trust_monitoring_report_includes_generated_at(self) -> None:
        ...
        self.assertEqual("source_trust_monitoring_report", monitoring_report["report_kind"])
        self.assertEqual("source_trust_monitoring_report_001", monitoring_report["schema_version"])
    ...
    def test_publish_gold_v3_cli_loads_default_dual_run_report_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dual_run_path = root / "v3_dual_run_report.json"
            drift_path = root / "source_drift_report.json"
            source_metrics_path = root / "v3_source_metrics.json"
            monitoring_path = root / "source_trust_monitoring_report.json"
            review_queue_path = root / "vision_v3_review_queue.json"
            review_queue_report_path = root / "v3_review_queue_report.json"
            flags_path.write_text("public_gold_source: v3\nv3_publish_enabled: true\n")
            preview_path.write_text(json.dumps(clean_publish_v3_preview()))
            dual_run_path.write_text(json.dumps(clean_dual_run_coverage_report(json.loads(preview_path.read_text()))))
            drift_path.write_text(json.dumps(clean_source_drift_report()))
            source_metrics_path.write_text(json.dumps(clean_source_metrics_report(json.loads(preview_path.read_text()))))
            review_queue_path.write_text("[]")
            review_queue_report_path.write_text(json.dumps(build_review_queue_report([])))
            monitoring_path.write_text(json.dumps(clean_monitoring_report()))
            ...
            with patch.object(publish_gold_v3, "DEFAULT_DUAL_RUN_REPORT_PATH", dual_run_path):
                with patch.object(publish_gold_v3, "DEFAULT_SOURCE_DRIFT_REPORT_PATH", drift_path):
                    with patch.object(publish_gold_v3, "DEFAULT_SOURCE_METRICS_REPORT_PATH", source_metrics_path):
                        with patch.object(publish_gold_v3, "DEFAULT_MONITORING_REPORT_PATH", monitoring_path):
                            with patch.object(publish_gold_v3, "DEFAULT_REVIEW_QUEUE_PATH", review_queue_path):
                                with patch.object(publish_gold_v3, "DEFAULT_REVIEW_QUEUE_REPORT_PATH", review_queue_report_path):
                                    with patch.object(
                                        sys,
                                        "argv",
                                        [
                                ...
                                with patch.object(publish_gold_v3, "DEFAULT_REVIEW_QUEUE_PATH", review_queue_path):
                                    with patch.object(publish_gold_v3, "DEFAULT_REVIEW_QUEUE_REPORT_PATH", review_queue_report_path):
                                        with patch.object(publish_gold_v3, "SupabaseClient", return_value=db, create=True):
                                            with patch.object(
                                                publish_gold_v3,
                                                "publish_gold_v3_preview",
                                                side_effect=publish_spy,
                                            ):
                                                with patch.object(
                                                    sys,
                                                    "argv",
    ...
    def test_publish_gold_v3_rollback_requires_scope_and_retires_matching_current_rows(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.selects: list[tuple[str, str, dict[str, str]]] = []
                self.updates: list[tuple[str, dict[str, str], dict[str, Any]]] = []

            def select(self, table: str, *, columns: str = "*", filters: Optional[dict[str, str]] = None) -> list[dict[str, Any]]:
                self.selects.append((table, columns, filters or {}))
                return [{"id": "boardwalk:poolside-activities"}, {"id": "boardwalk:campfire"}]
        ...
        with self.assertRaisesRegex(RuntimeError, "gold_v3_rollback_requires_source_hash_or_row_key"):
            publish_gold_v3.rollback_gold_v3_rows(db)
        result = publish_gold_v3.rollback_gold_v3_rows(
            db,
            source_sha256="sourcehash",
            calendar_group_key="boardwalk",
        )

        self.assertEqual({"retired_gold_v3_rows": 2}, result)
        self.assertEqual(
            [
                (
                    ...
                    "id",
                    {
                        ...
                        "calendar_group_key": "eq.boardwalk",
                    },
                )
            ],
            db.selects,
        ...
        self.assertEqual(
            [
                ("public_activity_gold", {"id": "eq.boardwalk:poolside-activities"}, {"is_current": False}),
                ("public_activity_gold", {"id": "eq.boardwalk:campfire"}, {"is_current": False}),
            ],
            db.updates,
        )

    def test_publish_gold_v3_cli_can_run_scoped_rollback_without_preview_readiness(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.selects: list[tuple[str, str, dict[str, str]]] = []
                self.updates: list[tuple[str, dict[str, str], dict[str, Any]]] = []
            ...
            def select(self, table: str, *, columns: str = "*", filters: Optional[dict[str, str]] = None) -> list[dict[str, Any]]:
                self.selects.append((table, columns, filters or {}))
                return [{"id": "boardwalk:poolside-activities"}]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            missing_preview_path = Path(tmp) / "missing-preview.json"
            with patch.object(publish_gold_v3, "SupabaseClient", return_value=db, create=True):
                with patch.object(publish_gold_v3, "publish_gold_v3_preview", side_effect=publish_spy):
                    with patch.object(
                        sys,
                        ...
                        [
                            "publish_gold_v3.py",
                            str(missing_preview_path),
                            "--rollback-source-sha256",
                            "sourcehash",
                            "--rollback-calendar-group",
                            "boardwalk",
                        ],
                    ):
                        publish_gold_v3.main()
    ...
    def test_publish_gold_v3_preview_rejects_non_public_source_url_before_db_writes(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.upserts: list[tuple[str, Any, str]] = []
                self.inserts: list[tuple[str, Any]] = []
        ...
        preview = clean_publish_v3_preview()
        preview["rows"][0]["canonical_slug"] = "poolside-activities"
        preview["rows"][0]["source_url"] = "manual://boardwalk/fy26-q3-0526/recreation"
        db = RecordingDb()

        with self.assertRaisesRegex(RuntimeError, "gold_v3_non_public_source_url:boardwalk:poolside-activities"):
            publish_gold_v3.publish_gold_v3_preview(db, preview)

        self.assertEqual([], db.upserts)
        self.assertEqual([], db.inserts)

    def test_publish_gold_v3_preview_rejects_unpromotable_status_before_db_writes(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.upserts: list[tuple[str, Any, str]] = []
                self.inserts: list[tuple[str, Any]] = []
        ...
        preview = clean_publish_v3_preview()
        preview["rows"][0]["canonical_slug"] = "poolside-activities"
        preview["rows"][0]["validation_status"] = "needs_review"
        db = RecordingDb()

        with self.assertRaisesRegex(
            RuntimeError,
            "gold_v3_unpromotable_validation_status:boardwalk:poolside-activities",
        ):
            publish_gold_v3.publish_gold_v3_preview(db, preview)
    ...
    def test_publish_gold_v3_preview_rejects_missing_field_provenance_before_db_writes(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.upserts: list[tuple[str, Any, str]] = []
                self.inserts: list[tuple[str, Any]] = []
            ...
            def upsert(self, table: str, rows: Any, on_conflict: str) -> list[dict[str, Any]]:
                self.upserts.append((table, rows, on_conflict))
                payload = rows if isinstance(rows, list) else [rows]
                return [{**row, "id": f"db-source-{row['content_sha256']}"} for row in payload] if table == "source_documents" else payload
        ...
        preview = clean_publish_v3_preview()
        preview["rows"][0]["canonical_slug"] = "poolside-activities"
        preview["rows"][0]["source_spans"].pop("schedule")
        db = RecordingDb()

        with self.assertRaisesRegex(
            RuntimeError,
            "gold_v3_missing_field_provenance:boardwalk:poolside-activities:schedule",
        ):
            publish_gold_v3.publish_gold_v3_preview(db, preview)
    ...
    def test_publish_gold_v3_preview_rejects_incomplete_field_evidence_before_db_writes(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.upserts: list[tuple[str, Any, str]] = []
                self.inserts: list[tuple[str, Any]] = []
        ...
        preview = clean_publish_v3_preview()
        preview["rows"][0]["canonical_slug"] = "poolside-activities"
        preview["rows"][0]["field_evidence"]["schedule"]["source"].pop("crop_sha256")
        db = RecordingDb()

        with self.assertRaisesRegex(
            RuntimeError,
            "gold_v3_missing_field_crop_sha256:boardwalk:poolside-activities:schedule",
        ):
            publish_gold_v3.publish_gold_v3_preview(db, preview)
    ...
    def test_publish_gold_v3_preview_rejects_inconsistent_field_evidence_before_db_writes(self) -> None:
        class RecordingDb:
            def __init__(self) -> None:
                self.upserts: list[tuple[str, Any, str]] = []
                self.inserts: list[tuple[str, Any]] = []
        ...
        hash_mismatch_preview = clean_publish_v3_preview()
        hash_mismatch_preview["rows"][0]["canonical_slug"] = "poolside-activities"
        hash_mismatch_preview["rows"][0]["field_evidence"]["schedule"]["source"]["content_sha256"] = "otherhash"
        hash_db = RecordingDb()

        with self.assertRaisesRegex(
            RuntimeError,
            "gold_v3_field_source_hash_mismatch:boardwalk:poolside-activities:schedule",
        ):
            publish_gold_v3.publish_gold_v3_preview(hash_db, hash_mismatch_preview)

        page_mismatch_preview = clean_publish_v3_preview()
        page_mismatch_preview["rows"][0]["canonical_slug"] = "poolside-activities"
        page_mismatch_preview["rows"][0]["field_evidence"]["schedule"]["source"]["page_image_sha256"] = "otherpage"
        page_db = RecordingDb()

        with self.assertRaisesRegex(
            RuntimeError,
            "gold_v3_field_page_image_not_canonical:boardwalk:poolside-activities:schedule",
        ):
            publish_gold_v3.publish_gold_v3_preview(page_db, page_mismatch_preview)

        self.assertEqual([], hash_db.upserts)
        self.assertEqual([], hash_db.inserts)
        self.assertEqual([], page_db.upserts)
        self.assertEqual([], page_db.inserts)
    ...
    def test_source_drift_report_uses_region_evidence_for_extraction_gap_validation_failures(self) -> None:
        new_rows = [
            {
                "candidate_id": "cand-extraction-gap",
                "candidate_type": "activity",
                "canonical_slug": "wellness-section",
                "title": None,
                "validation_findings": [
                    "field_extraction_not_implemented",
                    "missing_reviewable_field_evidence",
                ],
                "field_evidence": {
                    "region": {
                        "raw_value": "Wellness Activities",
                        "source": {
                            "page_number": 1,
                            "page_image_sha256": "pagehash",
                            "page_image_path": "data/processed/source_pages/source-doc-a/page-001.png",
                            "crop_storage_path": "data/processed/region_crops/source-doc-a/wellness.png",
                            "crop_sha256": "wellnesscrophash",
                            "bbox_px": [24, 48, 420, 180],
    ...
    def test_source_drift_report_does_not_duplicate_needs_review_candidate_validation_findings(self) -> None:
        new_rows = [
            {
                "candidate_id": "cand-needs-review",
                "candidate_type": "activity",
                "canonical_slug": "poolside-activities",
                "title": "Poolside Activities",
                "validation_status": "needs_review",
            ...
            },
            {
                ...
                "validation_findings": ["field_extraction_not_implemented"],
                "field_evidence": {
                    "region": {
                        "raw_value": "Wellness Activities",
                        "source": {
        ...
        self.assertEqual(["cand-rejected"], [failure["candidate_id"] for failure in report["validation_failures"]])
        self.assertIn("validation_failures", report["publish_blockers"])
    ...
    def test_source_drift_report_quarter_cli_loads_unknown_regions_from_snapshot_directory(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            old_dir = root / "old_candidates"
            new_dir = root / "new_candidates"
            snapshot_dir = root / "vision_snapshots"
            output_path = root / "source_drift_report.json"
            old_dir.mkdir()
            new_dir.mkdir()
            snapshot_dir.mkdir()
            ...
            (old_dir / "oldhash.validated.json").write_text(json.dumps([old_row]))
            (new_dir / "newhash.validated.json").write_text(json.dumps([new_row]))
            (snapshot_dir / "newhash.vision.json").write_text(
                json.dumps(
                    {
                        "source_document_id": "source-doc-boardwalk",
                        "content_sha256": "newhash",
                        "regions": [
                            {
                                ...
                                "page_image_sha256": "pagehash",
                                "crop_sha256": "crophash",
                                "crop_storage_path": "data/processed/field_crops_v3/newhash/region.png",
                            }
                        ],
                    }
                )
        ...
        source_report = report["source_reports"][0]
        self.assertEqual("region-unknown-1", source_report["new_unknown_regions"][0]["region_id"])
        self.assertIn("new_unknown_regions", report["publish_blockers"])
    ...
    def test_source_drift_report_quarter_batch_carries_unknown_regions_per_source(self) -> None:
        report = source_drift_report.build_quarter_source_drift_report(
            quarter="fy26-q4",
            old_rows=[
                {
        ...
        self.assertIn("new_unknown_regions", report["publish_blockers"])
        source_report = report["source_reports"][0]
        self.assertEqual(["new_unknown_regions"], source_report["publish_blockers"])
        self.assertEqual("region-unknown-1", source_report["new_unknown_regions"][0]["region_id"])
        self.assertEqual("pagehash", source_report["new_unknown_regions"][0]["page_image_sha256"])
    ...
    def test_schedule_normalization_handles_recurring_point_time_and_phrase_schedules(self) -> None:
        ...
        open_24_hours = normalize_schedule_text("Open 24 hours a day, 7 days a week")
        time_range = normalize_schedule_text("From 3:30pm-4:30pm")
        contextual_time = normalize_schedule_text("SHIPWRECK BEACH | 8:30PM")
        multi_segment = normalize_schedule_text(
            "Sunday, Tuesday and Thursday from 1:00pm-10:00pm; "
            "Monday, Wednesday, Friday and Saturday from 10:00am-10:00pm"
        )
        ...
        self.assertEqual("recurring", open_24_hours["schedule_type"])
        self.assertEqual(ALL_DAYS, open_24_hours["days_of_week"])
        self.assertEqual("00:00", open_24_hours["start_time"])
        self.assertEqual("23:59", open_24_hours["end_time"])
        self.assertEqual("time_range", time_range["schedule_type"])
        self.assertEqual("15:30", time_range["start_time"])
        self.assertEqual("16:30", time_range["end_time"])
        self.assertNotIn("days_of_week", time_range)
        self.assertEqual("time_point", contextual_time["schedule_type"])
        self.assertEqual("20:30", contextual_time["start_time"])
        self.assertEqual("SHIPWRECK BEACH | 8:30PM", contextual_time["raw_text"])
        self.assertNotIn("days_of_week", contextual_time)
        self.assertEqual("recurring", multi_segment["schedule_type"])
        self.assertEqual(["Sunday", "Tuesday", "Thursday"], multi_segment["days_of_week"])
        self.assertEqual("13:00", multi_segment["start_time"])
        self.assertEqual("22:00", multi_segment["end_time"])
        self.assertEqual(
            [
                {
                    ...
                    "start_time": "13:00",
                    "end_time": "22:00",
                    "raw_text": "Sunday, Tuesday and Thursday from 1:00pm-10:00pm",
                },
                {
                    "days_of_week": ["Monday", "Wednesday", "Friday", "Saturday"],
                    "start_time": "10:00",
                    "end_time": "22:00",
                    "raw_text": "Monday, Wednesday, Friday and Saturday from 10:00am-10:00pm",
                },
            ],
            multi_segment["segments"],
        )

    def test_location_normalization_includes_observed_twenty_four_hour_facility_locations(self) -> None:
        self.assertEqual(
            "contemporary_tower_3rd_floor",
            normalize_location_text("Contemporary Tower, 3rd Floor")["location_id"],
        )
        self.assertEqual(
            "gran_destino_tower_and_near_casitas_4",
            normalize_location_text("Gran Destino Tower and near Casitas 4")["location_id"],
        )
        self.assertEqual(
            "the_grand_floridian_spa",
            normalize_location_text("The Grand Floridian Spa")["location_id"],
        )

    def test_location_normalization_repairs_movie_location_separator_read_as_i(self) -> None:
        self.assertEqual(
            "the_reel_spot",
            normalize_location_text("The Reel Spot")["location_id"],
        )
        self.assertEqual(
            "the_reel_spot",
            normalize_location_text("THE REEL SPOT I")["location_id"],
        )
        agreement = evaluate_field_agreement(
            "location",
            primary_text="THE REEL SPOT I",
            secondary_text="THE REEL SPOT",
        )
        self.assertEqual("exact_after_normalization", agreement["agreement"])
        self.assertEqual("the_reel_spot", agreement["normalized_value"])

    def test_location_normalization_includes_refreshed_aframe_observed_locations(self) -> None:
        observed_locations = {
            "Near Beach Pool": "near_beach_pool",
            "Outside Of The Artist's Palette": "outside_of_the_artists_palette",
            ...
            "Throughout Disney's Old Key West Resort": "throughout_disneys_old_key_west_resort",
            "Hank's Rent 'N Return at Community Hall": "hanks_rent_n_return_at_community_hall",
            "THE VOLLEYBALL COURT I": "the_volleyball_court",
            ...
            "Lobby": "lobby",
            "Inside and Outside the Lobby Area": "inside_and_outside_the_lobby_area",
            "Oak Manor Lawn near Building 90": "oak_manor_lawn_near_building_90",
            "The River Roost": "the_river_roost",
        }
        for raw_location, expected_id in observed_locations.items():
            with self.subTest(raw_location=raw_location):
                self.assertEqual(expected_id, normalize_location_text(raw_location)["location_id"])
    ...
    def test_engine_agreement_is_field_specific_after_deterministic_normalization(self) -> None:
        ...
        movie_header_single_character_fragment = evaluate_field_agreement(
            "title",
            primary_text="MOVIE UNDER THE STARS",
            secondary_text="E",
        )
        movie_header_stars_fragment = evaluate_field_agreement(
            "title",
            primary_text="MOVIE UNDER THE STARS",
            secondary_text="STARS",
        )
        movie_header_sparse_tail_fragment = evaluate_field_agreement(
            "title",
            primary_text="MOVIE UNDER THE STARS",
            secondary_text="R S",
        )
        ...
        movie_dangling_rating_bracket = evaluate_field_agreement(
            "title",
            primary_text="Teen Beach Movie",
            secondary_text="Teen Beach Movie [",
        )
        ...
        apostrophe_spacing = evaluate_field_agreement(
            "title",
            primary_text="JAMMIN'AT JAMBO HOUSE",
            secondary_text="JAMMIN’ AT JAMBO HOUSE",
        )
        compact_activity_words = evaluate_field_agreement(
            "title",
            primary_text="MEMORIESACTIVITIES",
            secondary_text="MEMORIES ACTIVITIES",
        )
        ...
        self.assertEqual("exact_after_normalization", movie_header_single_character_fragment["agreement"])
        self.assertEqual("movie under the stars", movie_header_single_character_fragment["normalized_value"])
        self.assertEqual("exact_after_normalization", movie_header_stars_fragment["agreement"])
        self.assertEqual("movie under the stars", movie_header_stars_fragment["normalized_value"])
        self.assertEqual("exact_after_normalization", movie_header_sparse_tail_fragment["agreement"])
        self.assertEqual("movie under the stars", movie_header_sparse_tail_fragment["normalized_value"])
        ...
        self.assertEqual("exact_after_normalization", movie_dangling_rating_bracket["agreement"])
        self.assertEqual("teen beach movie", movie_dangling_rating_bracket["normalized_value"])
        ...
        self.assertEqual("exact_after_normalization", apostrophe_spacing["agreement"])
        self.assertEqual("jammin' at jambo house", apostrophe_spacing["normalized_value"])
        self.assertEqual("exact_after_normalization", compact_activity_words["agreement"])
        self.assertEqual("memories activities", compact_activity_words["normalized_value"])
    ...
    def test_validate_v3_blocks_non_activity_label_titles_from_auto_publish(self) -> None:
        def evidence(field: str, raw_value: Any, normalized_value: Any) -> dict[str, Any]:
            return {
                "field": field,
                "raw_value": raw_value,
                ...
                "source": {
                    ...
                    "crop_sha256": f"{field}crop",
                    "crop_storage_path": f"field-crops/{field}.png",
                },
                "engines": [
                    {"engine": "paddleocr_ppstructurev3", "text": raw_value, "confidence": 0.99},
                    {"engine": "rapidocr", "text": raw_value, "confidence": 0.98},
                ],
                "agreement": "exact_after_normalization",
            }

        def candidate_with_title(source_title: str, normalized_title: str) -> dict[str, Any]:
            return {
            "candidate_id": "old-key-west-located-near",
            "pipeline_version": "vision_v3_001",
            "config_hash": "confighash",
            "runtime_lineage": {
                "config_hash": "confighash",
                "lineage_hash": "lineagehash",
                "git_sha": "deadbeef",
                "config": {"ocr": {"primary": "paddleocr_ppstructurev3", "secondary": "rapidocr"}},
                "package_versions": {"pypdfium2": "1.0"},
                "model_asset_hashes": {"paddleocr_ppstructurev3": "modelhash"},
                "ocr_engine_configs": {"paddleocr_ppstructurev3": {"mode": "ppstructurev3"}},
            },
            "source_document_id": "source-doc",
            "content_sha256": "sourcehash",
            "canonical_url": "https://example.test/okwr.pdf",
            "source_kind": "official_pdf",
            "http_status": 200,
            "currentness": "current",
            ...
            "location_text": "THE VOLLEYBALL COURT I",
            "normalized_location_id": "the_volleyball_court",
            "fee_required": False,
            "field_evidence": {
                "region": evidence("region", "movie section", "movie section"),
                "title": evidence("title", source_title, normalized_title),
                "schedule": evidence(
                    "schedule",
                    "8:00PM",
                    {
                ...
                "location": evidence("location", "THE VOLLEYBALL COURT I", "the_volleyball_court"),
                "fee": evidence("fee", "No fee marker present", False),
            },
            "source_spans": {
                field: [{"page": 1, "text": value}]
                for field, value in {
                    "title": source_title,
                    "schedule": "8:00PM",
                    "location": "THE VOLLEYBALL COURT I",
                    "fee": "No fee marker present",
                }.items()
            },
        }
        ...
        for source_title, normalized_title in cases:
            with self.subTest(source_title=source_title):
                validated = validate_candidate(candidate_with_title(source_title, normalized_title))

                self.assertEqual("needs_review", validated["validation_status"])
                self.assertIn("non_activity_title:title", validated["validation_findings"])
    ...
    def test_source_metrics_v3_rolls_up_per_source_quality_and_candidate_counts(self) -> None:
        snapshots = [
            {
                ...
                "fetched_url": "https://cdn.example.test/source.pdf",
                "storage_path": "data/raw/source_documents/boardwalk/fy26-q3/hash-1.pdf",
                "calendar_group_key": "boardwalk",
                "edition": "fy26-q3-0526",
                "mime_type": "application/pdf",
        ...
        self.assertEqual("https://example.test/source.pdf", by_hash["hash-1"]["canonical_url"])
        self.assertEqual("https://cdn.example.test/source.pdf", by_hash["hash-1"]["fetched_url"])
        self.assertEqual(
            "data/raw/source_documents/boardwalk/fy26-q3/hash-1.pdf",
            by_hash["hash-1"]["storage_path"],
        )
        self.assertEqual("boardwalk", by_hash["hash-1"]["calendar_group_key"])
        self.assertEqual("fy26-q3-0526", by_hash["hash-1"]["edition"])
        self.assertEqual("application/pdf", by_hash["hash-1"]["mime_type"])
        self.assertEqual("application/pdf", by_hash["hash-1"]["http_content_type"])
        self.assertEqual("application/pdf", by_hash["hash-1"]["detected_content_type"])
        self.assertEqual(".pdf", by_hash["hash-1"]["file_extension"])
    ...
    def test_source_metrics_v3_can_scope_to_official_visual_sources(self) -> None:
        snapshots = [
            {
                "source_sha256": "official-pdf-hash",
                "source_kind": "official_pdf",
                "source_role": "resort_pdf",
                "source_type": "pdf",
                "source_pages": [{"page_number": 1}],
            ...
            {
                "source_sha256": "manual-image-hash",
                "source_kind": "reviewed_visual_schedule_image",
                "source_role": "image_manifest",
                "source_type": "image",
                "source_pages": [{"page_number": 1}],
            },
            {
                "source_sha256": "html-hash",
                "source_kind": "official_html",
                "source_role": "supporting_page",
                "source_type": "html",
                "source_pages": [{"page_number": 1}],
            },
        ]
        candidates = [
            {
                "content_sha256": "official-pdf-hash",
                "candidate_type": "activity",
                "validation_status": "auto_publishable",
            },
            {
                "content_sha256": "manual-image-hash",
                "candidate_type": "activity",
                "validation_status": "needs_review",
            },
            {
                "content_sha256": "html-hash",
                "candidate_type": "activity",
                "validation_status": "needs_review",
            },
        ]
        ...
        self.assertEqual(1, report["summary"]["source_count"])
        self.assertEqual(1, report["summary"]["candidate_count"])
        self.assertEqual(["official-pdf-hash"], [row["source_sha256"] for row in report["sources"]])
        self.assertEqual(1, report["summary"]["auto_publishable_count"])
        self.assertEqual(0, report["summary"]["needs_review_count"])

    def test_source_metrics_v3_enriches_sources_from_bronze_inventory(self) -> None:
        snapshots = [{"source_sha256": "hash-1", "source_pages": [{"page_number": 1}]}]
        source_inventory = [
            {
                "content_sha256": "hash-1",
                ...
                "canonical_url": "https://example.test/source.pdf",
                "fetched_url": "https://cdn.example.test/source.pdf",
                "storage_path": "data/raw/pdfs/source.pdf",
                "calendar_group_key": "boardwalk",
                "source_pdf_edition": "fy26-q3-0526",
                "mime_type": "application/pdf",
                "http_content_type": "application/pdf; charset=binary",
                "detected_content_type": "application/pdf",
                "file_extension": ".pdf",
                "http_status": 200,
            }
        ...
        self.assertEqual("official_pdf", source["source_kind"])
        self.assertEqual("pdf", source["source_type"])
        self.assertEqual("https://example.test/source.pdf", source["canonical_url"])
        self.assertEqual("https://cdn.example.test/source.pdf", source["fetched_url"])
        self.assertEqual("data/raw/pdfs/source.pdf", source["storage_path"])
        self.assertEqual("boardwalk", source["calendar_group_key"])
        self.assertEqual("fy26-q3-0526", source["edition"])
        self.assertEqual("application/pdf", source["mime_type"])
        self.assertEqual("application/pdf; charset=binary", source["http_content_type"])
        self.assertEqual("application/pdf", source["detected_content_type"])
        self.assertEqual(".pdf", source["file_extension"])
        self.assertEqual(200, source["http_status"])

    def test_source_metrics_v3_binds_report_to_gold_preview_fingerprint(self) -> None:
        preview = clean_publish_v3_preview()
        ...
        self.assertEqual(
            publish_gold_v3._dual_run_preview_fingerprint(preview),
            report["v3_preview_fingerprint"],
        )
    ...
    def test_source_metrics_v3_cli_writes_report_from_snapshot_and_candidate_directories(self) -> None:
        ...
        self.assertEqual("vision_v3_source_metrics", report["report_kind"])
        self.assertEqual(1, report["summary"]["source_count"])
        self.assertEqual("https://example.test/source.pdf", report["sources"][0]["canonical_url"])
        self.assertEqual("data/raw/pdfs/source.pdf", report["sources"][0]["storage_path"])
        self.assertEqual(1, report["sources"][0]["auto_publishable_count"])

    def test_run_pipeline_generates_source_metrics_after_v3_preview(self) -> None:
        steps = run_pipeline.vision_v3_report_steps(local_only=False, quarter="fy26-q4")
        step_names = [step[0] for step in steps]
        publish_step = next(step for step in steps if step[0] == "publish_gold_v3.py")
        dual_run_step = next(step for step in steps if step[0] == "v3_dual_run_report.py")
        source_drift_step = next(step for step in steps if step[0] == "source_drift_report.py")
        source_metrics_step = next(step for step in steps if step[0] == "source_metrics_v3.py")
        source_status_step = next(step for step in steps if step[0] == "source_status_v3.py")
        review_queue_steps = [step for step in steps if step[0] == "build_review_queue_v3.py"]

        self.assertEqual("data/processed/activity_gold_v3_preview.json", publish_step[1])
        self.assertIn("--flags", publish_step)
        self.assertIn("config/publish_flags.yaml", publish_step)
        self.assertIn("--output", source_drift_step)
        self.assertIn("data/processed/source_drift_report.json", source_drift_step)
        self.assertIn("--new-regions", source_drift_step)
        self.assertIn("data/processed/vision_snapshots", source_drift_step)
        self.assertIn("--output", source_metrics_step)
        self.assertIn("data/processed/eval/v3_source_metrics.json", source_metrics_step)
        self.assertIn("--preview", source_metrics_step)
        self.assertIn("data/processed/activity_gold_v3_preview.json", source_metrics_step)
        self.assertIn("--v3-visual-only", source_metrics_step)
        self.assertIn("--v3-visual-only", source_status_step)
        self.assertIn("--output", dual_run_step)
        self.assertIn("data/processed/eval/v3_dual_run_report.json", dual_run_step)
        self.assertEqual(2, len(review_queue_steps))
        for review_queue_step in review_queue_steps:
            self.assertIn("--output", review_queue_step)
            self.assertIn("data/processed/review_queue/vision_v3_review_queue.json", review_queue_step)
            self.assertIn("--report-output", review_queue_step)
            self.assertIn("data/processed/eval/v3_review_queue_report.json", review_queue_step)
            self.assertIn("--html-output", review_queue_step)
            self.assertIn("data/processed/review_queue/vision_v3_review_queue.html", review_queue_step)
            self.assertIn("--checklist-output", review_queue_step)
            self.assertIn("data/processed/review_queue/vision_v3_publish_blocker_checklist.csv", review_queue_step)
        self.assertIn("--no-dual-run-report", review_queue_steps[0])
        self.assertIn("--dual-run-report", review_queue_steps[-1])
        self.assertIn("data/processed/eval/v3_dual_run_report.json", review_queue_steps[-1])
        self.assertLess(
            step_names.index("promote_gold_v3.py"),
            step_names.index("publish_gold_v3.py"),
        )
        self.assertLess(
            step_names.index("promote_gold_v3.py"),
            step_names.index("source_metrics_v3.py"),
        )
        self.assertLess(
            step_names.index("source_metrics_v3.py"),
            step_names.index("publish_gold_v3.py"),
        ...
        self.assertLess(
            step_names.index("trust_report.py"),
            step_names.index("publish_gold_v3.py"),
        )
        self.assertIn("--monitoring-report", publish_step)
        self.assertIn("data/processed/source_trust_monitoring_report.json", publish_step)
        self.assertIn("--review-queue", publish_step)
        self.assertIn("data/processed/review_queue/vision_v3_review_queue.json", publish_step)
        self.assertIn("--review-queue-report", publish_step)
        self.assertIn("data/processed/eval/v3_review_queue_report.json", publish_step)
        self.assertIn("--dual-run-report", publish_step)
        self.assertIn("data/processed/eval/v3_dual_run_report.json", publish_step)
    ...
    def test_review_schema_records_hash_bound_approval_and_fixture_candidate(self) -> None:
        ...
        for placeholder_reviewer in ("reviewer-name", "your-name", "todo"):
            with self.subTest(reviewer=placeholder_reviewer):
                with self.assertRaisesRegex(ValueError, "placeholder_reviewer"):
                    build_review_decision(
                        task,
                        reviewer=placeholder_reviewer,
                        decision="approve",
                        approved_fields={"schedule": {"raw_value": "Daily at 1:30pm", "normalized_value": "13:30"}},
                        reason="Visible in crop.",
                        decided_at="2026-06-28T12:00:00Z",
                    )

        for placeholder_reason in (
            "Verified against source crop",
            "source-backed rationale for this decision",
            "reviewed",
        ):
            with self.subTest(reason=placeholder_reason):
                with self.assertRaisesRegex(ValueError, "placeholder_reason"):
                    build_review_decision(
                        task,
                        reviewer="josh",
                        decision="approve",
        ...
        self.assertEqual("https://example.com/official-calendar.pdf", fixture_candidate["source_url"])
        self.assertEqual(
            "https://example.com/official-calendar.pdf",
            fixture_candidate["source_metadata"]["canonical_url"],
        )
        self.assertEqual("official_pdf", fixture_candidate["source_metadata"]["source_kind"])
        self.assertEqual("Daily at 1:30pm", fixture_candidate["approved_fields"]["schedule"]["raw_value"])

    def test_review_schema_records_queue_task_decision_file(self) -> None:
        task = {
            "task_id": "vision-v3-cand-1",
            "task_type": "ocr_disagreement",
            "field": "schedule",
            "content_sha256": "sourcehash",
            "page_image_sha256": "pagehash",
            ...
            "candidate_value": "Daily at 1:30pm",
            "normalized_value": {
                "schedule_type": "recurring",
                "days_of_week": ALL_DAYS,
                "start_time": "13:30",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first_report = review_schema_v3.record_review_decision_from_queue(
                queue_path=queue_path,
                decisions_path=decisions_path,
                task_id="vision-v3-cand-1",
                reviewer="josh",
                decision="approve",
                reason="Visible in source crop.",
                decided_at="2026-06-28T12:00:00Z",
            )
            replacement_report = review_schema_v3.record_review_decision_from_queue(
                queue_path=queue_path,
                decisions_path=decisions_path,
                task_id="vision-v3-cand-1",
                reviewer="josh",
                decision="reject",
                reason="Second pass rejected this task.",
                decided_at="2026-06-28T12:01:00Z",
            )
            [decision] = json.loads(decisions_path.read_text())

        self.assertEqual(["schedule"], first_report["approved_field_names"])
        self.assertEqual(1, first_report["decision_count"])
        self.assertEqual(1, replacement_report["decision_count"])
        self.assertEqual("reject", decision["decision"])
        self.assertEqual({}, decision["approved_fields"])
        self.assertEqual("sourcehash", decision["content_sha256"])
        self.assertEqual("pagehash", decision["page_image_sha256"])
        self.assertEqual("schedulecrop", decision["field_crop_sha256"])

    def test_review_schema_recording_approval_refreshes_fixture_and_parser_exports(self) -> None:
        task = {
            "task_id": "vision-v3-cand-1",
            "task_type": "ocr_disagreement",
            "field": "schedule",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            queue_path = root / "review_queue.json"
            decisions_path = root / "review_decisions.json"
            fixture_path = root / "vision_v3_review_fixture_candidates.json"
            parser_path = root / "vision_v3_parser_rule_update_requests.json"
            queue_path.write_text(json.dumps([task]))

            report = review_schema_v3.record_review_decision_from_queue(
                queue_path=queue_path,
                decisions_path=decisions_path,
                task_id="vision-v3-cand-1",
                reviewer="josh",
            ...
            parser_payload = json.loads(parser_path.read_text())

        self.assertEqual(str(fixture_path), report["fixture_candidates_path"])
        self.assertEqual(str(parser_path), report["parser_rule_requests_path"])
        self.assertEqual("cand-1", fixture_payload["candidates"][0]["candidate_id"])
        self.assertEqual("sourcehash", fixture_payload["candidates"][0]["content_sha256"])
        self.assertEqual("vision_v3_001", fixture_payload["candidates"][0]["pipeline_version"])
        self.assertEqual("confighash", fixture_payload["candidates"][0]["config_hash"])
        self.assertEqual("lineagehash", fixture_payload["candidates"][0]["runtime_lineage"]["lineage_hash"])
        self.assertEqual("vision-v3-cand-1", parser_payload["requests"][0]["task_id"])
        self.assertEqual("vision_v3_001", parser_payload["requests"][0]["pipeline_version"])
        self.assertEqual("confighash", parser_payload["requests"][0]["config_hash"])
        self.assertEqual("lineagehash", parser_payload["requests"][0]["runtime_lineage"]["lineage_hash"])
        self.assertEqual(["engine_disagreement:schedule"], parser_payload["requests"][0]["validation_findings"])

    def test_review_schema_recording_dual_run_approval_refreshes_review_keys_export(self) -> None:
        task = {
            "task_id": "dual-run-diff-boardwalk-poolside-activities-schedule",
            "task_type": "gold_conflict",
            "field": "schedule",
            ...
            "field_crop_sha256": "schedulecrop",
            "candidate_value": "Daily at 1:30pm",
            "normalized_value": "Daily at 1:30pm",
            ...
            "dual_run_diff": {
                ...
                "field": "schedule",
                "content_sha256": "sourcehash",
                "v3_field_evidence": {"source": {"content_sha256": "sourcehash"}},
            },
        }
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            queue_path = root / "review_queue.json"
            decisions_path = root / "review_decisions.json"
            review_keys_path = root / "vision_v3_dual_run_review_keys.json"
            queue_path.write_text(json.dumps([task]))
            ...
            report = review_schema_v3.record_review_decision_from_queue(
                ...
                reviewer="josh",
                decision="approve",
                reason="Reviewed v2/v3 schedule diff against source crop.",
                decided_at="2026-06-28T12:00:00Z",
            )

            payload = json.loads(review_keys_path.read_text())

        self.assertEqual(str(review_keys_path), report["dual_run_review_keys_path"])
        self.assertEqual(["boardwalk:poolside-activities:schedule"], payload["reviewed_diff_keys"])
        self.assertEqual(
            ["boardwalk:poolside-activities:schedule"],
            [review["key"] for review in payload["reviewed_diff_reviews"]],
        )

    def test_review_schema_record_cli_writes_queue_task_decision(self) -> None:
        task = {
            "task_id": "vision-v3-cand-1",
            "task_type": "ocr_disagreement",
            "field": "location",
            "content_sha256": "sourcehash",
            "page_image_sha256": "pagehash",
            "field_crop": "location.png",
            "field_crop_sha256": "locationcrop",
            "candidate_value": "Luna Park Pool Deck",
            "normalized_value": "luna_park_pool_deck",
            "candidate": {
                "candidate_id": "cand-1",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            queue_path = root / "review_queue.json"
            decisions_path = root / "review_decisions.json"
            fixture_path = root / "custom_fixture_candidates.json"
            parser_path = root / "custom_parser_rule_requests.json"
            dual_run_keys_path = root / "custom_dual_run_review_keys.json"
            queue_path.write_text(json.dumps([task]))
            ...
            with patch.object(
                ...
                [
                    ...
                    "josh",
                    "--reason",
                    "Visible in source crop.",
                    "--fixture-candidates-output",
                    str(fixture_path),
                    "--parser-rule-requests-output",
                    str(parser_path),
                    "--dual-run-review-keys-output",
                    str(dual_run_keys_path),
                    "--json",
                ],
            ):
            ...
            fixture_payload = json.loads(fixture_path.read_text())
            parser_payload = json.loads(parser_path.read_text())
            dual_run_payload = json.loads(dual_run_keys_path.read_text())

        self.assertEqual("approve", decision["decision"])
        self.assertEqual(
            {"location": {"raw_value": "Luna Park Pool Deck", "normalized_value": "luna_park_pool_deck"}},
            decision["approved_fields"],
        )
        self.assertEqual("cand-1", fixture_payload["candidates"][0]["candidate_id"])
        self.assertEqual("vision-v3-cand-1", parser_payload["requests"][0]["task_id"])
        self.assertEqual([], dual_run_payload["reviewed_diff_keys"])
    ...
    def test_review_schema_rejects_fixture_exports_with_stale_original_task_hashes(self) -> None:
        task = {
            "task_id": "cand-stale-source",
            "task_type": "ocr_disagreement",
            "field": "schedule",
            "source_document_id": "source-doc-1",
            "content_sha256": "current-sourcehash",
            "page_image_sha256": "current-pagehash",
            "field_crop": "schedule.png",
            "field_crop_sha256": "current-crophash",
            "candidate": {
                "candidate_id": "cand-stale-source",
                "calendar_group_key": "boardwalk",
                "candidate_type": "activity",
            },
        }
        ...
        stale_source = {
            **approved,
            "task_id": "cand-stale-content",
            "candidate_id": "cand-stale-content",
            "content_sha256": "old-sourcehash",
        }
        stale_page = {
            **approved,
            "task_id": "cand-stale-page",
            "candidate_id": "cand-stale-page",
            "page_image_sha256": "old-pagehash",
        }
        stale_crop = {
            **approved,
            "task_id": "cand-stale-crop",
            "candidate_id": "cand-stale-crop",
            "field_crop_sha256": "old-crophash",
        }

        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            decisions_path = root / "review_decisions.json"
            fixture_path = root / "fixture_candidates.json"
            parser_path = root / "parser_rule_update_requests.json"
            decisions_path.write_text(json.dumps([stale_source, stale_page, stale_crop]))
    ...
    def test_review_schema_exports_dual_run_review_keys_from_review_decisions(self) -> None:
        ...
        placeholder_status_transition = {
            **approved_status_transition,
            "task_id": "dual-run-status-boardwalk-movie-night-needs-review-to-auto-publishable",
            "candidate_id": "dual-run-status-boardwalk-movie-night-needs-review-to-auto-publishable",
            "reviewer": "reviewer-name",
            "reason": "Placeholder reviewer should not count.",
            "original_task": {
                "content_sha256": "sourcehash",
                "task_type": "manual_review_required",
                "field": "status_transition",
                "dual_run_status_transition": {
                    "transition_key": "boardwalk:movie-night:needs_review_to_auto_publishable"
                },
            },
        }
        rejected_diff = {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            decisions_path.write_text(json.dumps([approved_diff, approved_status_transition, placeholder_status_transition, rejected_diff, {}]))
        ...
        self.assertEqual(5, report["summary"]["decision_count"])
        ...
        self.assertEqual(1, report["summary"]["skipped_non_approval_count"])
        self.assertEqual(2, report["summary"]["skipped_invalid_review_decision_count"])
        self.assertEqual(["boardwalk:poolside-activities:schedule"], payload["reviewed_diff_keys"])
        self.assertEqual(
            ["boardwalk:poolside-activities:needs_review_to_auto_publishable"],
    ...
    def test_review_schema_does_not_export_dual_run_reviews_without_source_provenance(self) -> None:
        missing_source_decision = {
            ...
            "reason": "Reviewed schedule diff without source provenance.",
            "decided_at": "2026-06-28T12:00:00Z",
            "original_task": {
                "content_sha256": "sourcehash",
                "page_image_sha256": "pagehash",
        ...
        non_public_source_decision = {
            **missing_source_decision,
            "task_id": "dual-run-diff-boardwalk-movie-under-the-stars-schedule",
            "candidate_id": "dual-run-diff-boardwalk-movie-under-the-stars-schedule",
            "original_task": {
                **missing_source_decision["original_task"],
                "source_url": "manual://boardwalk/review",
                "source_metadata": {"canonical_url": "manual://boardwalk/review"},
                "dual_run_diff": {
                    "diff_key": "boardwalk:movie-under-the-stars:schedule",
                    "field": "schedule",
                },
            },
        }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            decisions_path = root / "review_decisions.json"
            output_path = root / "dual_run_review_keys.json"
            decisions_path.write_text(json.dumps([missing_source_decision, non_public_source_decision]))
        ...
        self.assertEqual(2, report["summary"]["skipped_missing_source_provenance_count"])
        self.assertEqual([], payload["reviewed_diff_keys"])
        self.assertEqual([], payload["reviewed_diff_reviews"])
    ...
    def test_review_schema_rejects_stale_dual_run_visual_review_artifacts(self) -> None:
        current_task = {
            "content_sha256": "sourcehash",
            "page_image_sha256": "pagehash-current",
            "field_crop_sha256": "schedulecrop-current",
            "dual_run_diff": {
                "diff_key": "boardwalk:poolside-activities:schedule",
                "field": "schedule",
            },
        ...
        stale_page_decision = {
            ...
            "reason": "Reviewed against stale page image.",
            "decided_at": "2026-06-28T12:00:00Z",
        ...
        stale_crop_decision = {
            **stale_page_decision,
            "page_image_sha256": "pagehash-current",
            "field_crop_sha256": "schedulecrop-old",
            "reason": "Reviewed against stale crop.",
            "decided_at": "2026-06-28T12:01:00Z",
        }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            decisions_path = root / "review_decisions.json"
            output_path = root / "dual_run_review_keys.json"
            decisions_path.write_text(json.dumps([stale_page_decision, stale_crop_decision]))
    ...
    def test_review_schema_rejects_unparseable_review_decided_at(self) -> None:
        task = {
            "task_id": "dual-run-status-boardwalk-poolside-activities-needs-review-to-auto-publishable",
            "task_type": "manual_review_required",
            "field": "status_transition",
        ...
        with self.assertRaisesRegex(ValueError, "invalid_decided_at"):
            build_review_decision(
                task,
                reviewer="josh",
                decision="approve",
    ...
    def test_review_schema_exports_hash_bound_new_record_approval(self) -> None:
        task = {
            ...
            "content_sha256": "sourcehash",
            "page_image_sha256": "pagehash",
            "field_crop": "crops/new-record.png",
            "field_crop_sha256": "cropnew",
            "calendar_group_key": "boardwalk",
    ...
    def test_review_schema_exports_hash_bound_missing_v2_record_approval(self) -> None:
        task = {
            "task_id": "dual-run-missing-v2-boardwalk-campfire-missing-v2-record",
            "task_type": "manual_review_required",
            "field": "missing_v2_record",
            "content_sha256": "oldsourcehash",
            "page_image_sha256": "oldpagehash",
            "field_crop": "crops/missing-v2.png",
            "field_crop_sha256": "cropmissing",
            "calendar_group_key": "boardwalk",
            "candidate_type": "dual_run_missing_v2_record",
            "source_metadata": {
                "canonical_url": "https://example.com/old-boardwalk.pdf",
                "source_kind": "official_pdf",
            },
            "dual_run_missing_v2_record": {
                "review_key": "boardwalk:campfire:missing_v2_record",
        ...
        decision = build_review_decision(
            ...
            reviewer="josh",
            decision="approve",
            reason="Reviewed the v2-only row and accepted it as out of v3 scope.",
            decided_at="2026-06-28T12:00:00Z",
        )
        ...
        self.assertEqual("approve", decision["decision"])
        self.assertEqual("oldsourcehash", decision["content_sha256"])
        self.assertEqual({}, decision["approved_fields"])
        self.assertEqual(1, report["summary"]["reviewed_missing_v2_record_key_count"])
        self.assertEqual(["boardwalk:campfire:missing_v2_record"], payload["reviewed_missing_v2_record_keys"])
        self.assertEqual(
            [
                {
                    "key": "boardwalk:campfire:missing_v2_record",
                    "task_id": "dual-run-missing-v2-boardwalk-campfire-missing-v2-record",
                    "content_sha256": "oldsourcehash",
                    "source_url": "https://example.com/old-boardwalk.pdf",
                    "source_metadata": {
                        "canonical_url": "https://example.com/old-boardwalk.pdf",
                        "source_kind": "official_pdf",
                    },
            ...
            payload["reviewed_missing_v2_record_reviews"],
        )

    def test_review_schema_requires_visual_artifacts_for_dual_run_record_approvals(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "missing_page_image_sha256"):
            build_review_decision(
                missing_artifact_task,
                reviewer="josh",
                decision="approve",
                reason="Reviewed the new V3 row.",
                decided_at="2026-06-28T12:00:00Z",
            )

    def test_review_schema_does_not_export_dual_run_record_reviews_missing_visual_artifacts(self) -> None:
        stale_decision = {
            "schema_version": "review_decision_v3_001",
            "task_id": "dual-run-new-boardwalk-poolside-activities-new-record",
            "task_type": "manual_review_required",
            "candidate_id": "dual-run-new-boardwalk-poolside-activities-new-record",
            "candidate_type": "dual_run_new_record",
            "calendar_group_key": "boardwalk",
            "content_sha256": "sourcehash",
            "reviewer": "josh",
        ...
        self.assertEqual(1, report["summary"]["skipped_invalid_review_decision_count"])
        self.assertEqual([], payload["reviewed_new_record_keys"])
        self.assertEqual([], payload["reviewed_new_record_reviews"])
    ...
    def test_review_schema_rejects_stale_source_drift_visual_review_artifacts(self) -> None:
        source_drift_report = {
            "report_kind": "v3_quarter_source_drift",
            "quarter": "fy26-q4",
            "status": "review_required",
                    ...
                    "new_title_details": [
                        {
                            "title": "Campfire",
                            "page_image_sha256": "pagehash-current",
                            "crop_storage_path": "crops/campfire-title.png",
                            "crop_sha256": "titlecrop-current",
                            "bbox_px": [20, 40, 180, 70],
                            "text": "Campfire",
                        }
                    ],
        ...
        stale_decision = build_review_decision(
            ...
            reviewer="josh",
            decision="approve",
            reason="Reviewed source drift against old page evidence.",
            decided_at="2026-06-28T12:00:00Z",
        )
        stale_decision["page_image_sha256"] = "pagehash-old"
        stale_decision["original_task"] = {
            **stale_decision["original_task"],
            "page_image_sha256": "pagehash-old",
        }
    ...
    def test_review_schema_script_can_export_reviewed_source_drift_report_directly(self) -> None:
        source_drift_report = {
            "status": "review_required",
            "calendar_group_key": "boardwalk",
            "new_hash": "newhash",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts/ingest/review_schema_v3.py"),
                    str(decisions_path),
                    "--export",
                    "reviewed-source-drift-report",
                    "--source-drift-report",
    ...
    def test_dual_run_report_surfaces_field_diffs_and_unreviewed_publish_blockers(self) -> None:
        ...
        self.assertEqual("v3_dual_run_report_001", report["schema_version"])
        self.assertRegex(report.get("generated_at", ""), r"^\d{4}-\d{2}-\d{2}T")
        ...
        self.assertEqual(["boardwalk:campfire:missing_v2_record"], report["missing_v2_review_keys"])
        self.assertEqual("boardwalk:campfire", report["missing_v2_records"][0]["row_key"])
        self.assertEqual("oldsourcehash", report["missing_v2_records"][0]["content_sha256"])
        self.assertEqual(
            str(v3_dual_run_report.PROCESSED_DIR / "pdf_page_images" / "oldsourcehash-page-001.png"),
            report["missing_v2_records"][0]["page_image_path"],
        )
        self.assertEqual([40, 50, 260, 80], report["missing_v2_records"][0]["field_bbox"])
        self.assertEqual(
            ["boardwalk:poolside-activities:schedule"],
        ...
        self.assertEqual(
            ["boardwalk:poolside-activities:schedule"],
            [diff["review_key"] for diff in report["field_diffs"]],
        )
        self.assertEqual("vision_v3_001", report["field_diffs"][0]["pipeline_version"])
        self.assertEqual("confighash", report["field_diffs"][0]["config_hash"])
        self.assertEqual("lineagehash", report["field_diffs"][0]["runtime_lineage"]["lineage_hash"])
    ...
    def test_dual_run_report_prefers_canonical_page_image_for_missing_v2_review(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            canonical_page = root / "page_images" / "oldsourcehash" / "page_001.png"
            canonical_page.parent.mkdir(parents=True)
            canonical_page.write_bytes(b"page")
            with patch.object(v3_dual_run_report, "PROCESSED_DIR", root):
        ...
        self.assertEqual(str(canonical_page), report["missing_v2_records"][0]["page_image_path"])

    def test_dual_run_report_requires_hash_bound_review_for_missing_v2_rows(self) -> None:
        v2_rows = [
            {
                "canonical_slug": "campfire",
                "title": "Campfire",
            ...
            reviewed_missing_v2_record_reviews=[
                {
                    ...
                    "page_image_sha256": "oldpagehash",
                    "field_crop_sha256": "oldtitlecrop",
                    "reviewer": "josh",
                    "decided_at": "2026-06-28T12:00:00Z",
                    "decision": "approve",
                    "reason": "Reviewed as acceptable v3 omission.",
                }
            ],
        )
        stale = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview={"rows": []},
            reviewed_missing_v2_record_reviews=[
                {
                    "key": "boardwalk:campfire:missing_v2_record",
                    "task_id": "dual-run-missing-v2-boardwalk-campfire-missing-v2-record",
                    "content_sha256": "stalehash",
                    "page_image_sha256": "oldpagehash",
                    "field_crop_sha256": "oldtitlecrop",
                    "reviewer": "josh",
                    "decided_at": "2026-06-28T12:00:00Z",
                    "decision": "approve",
                    "reason": "Reviewed stale record.",
                }
            ],
        )
        stale_visual = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview={"rows": []},
            reviewed_missing_v2_record_reviews=[
                {
                    ...
                    "decided_at": "2026-06-28T12:00:00Z",
                    "decision": "approve",
                    "reason": "Reviewed stale visual evidence.",
                }
            ],
        )
        bare_key = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview={"rows": []},
            reviewed_diff_keys=["boardwalk:campfire:missing_v2_record"],
        )

        self.assertEqual("clean", reviewed["status"])
        self.assertEqual([], reviewed["unreviewed_missing_v2_keys"])
        self.assertEqual(["boardwalk:campfire:missing_v2_record"], reviewed["reviewed_missing_v2_record_keys"])
        self.assertEqual(
            "https://example.com/old-boardwalk.pdf",
            reviewed["reviewed_missing_v2_record_reviews"][0]["source_url"],
        )
        self.assertEqual(
            "official_pdf",
            reviewed["reviewed_missing_v2_record_reviews"][0]["source_metadata"]["source_kind"],
        )
        self.assertNotIn("v3_missing_v2_rows", reviewed["publish_blockers"])
        self.assertEqual("blocked", stale["status"])
        self.assertEqual(["boardwalk:campfire"], stale["unreviewed_missing_v2_keys"])
        self.assertIn("v3_missing_v2_rows", stale["publish_blockers"])
        self.assertEqual("blocked", stale_visual["status"])
        self.assertEqual(["boardwalk:campfire"], stale_visual["unreviewed_missing_v2_keys"])
        self.assertEqual("blocked", bare_key["status"])
        self.assertEqual([], bare_key["reviewed_missing_v2_record_keys"])
        self.assertEqual(["boardwalk:campfire"], bare_key["unreviewed_missing_v2_keys"])
    ...
    def test_dual_run_report_blocks_unreviewed_new_v3_rows(self) -> None:
                    ...
                    "field_evidence": {
                        "title": {
                            "source": {
                                ...
                                "page_number": 1,
                                "page_image_sha256": "pagehash",
                                "page_image_path": "pages/title.png",
                                "crop_storage_path": "fields/title.png",
                                "crop_sha256": "titlecrop",
                                "bbox_px": [12, 24, 240, 60],
                            },
                            "engines": [
                                {"engine": "paddleocr_ppstructurev3", "text": "Poolside Activities"},
                                {"engine": "rapidocr", "text": "Poolside Activities"},
                            ],
                        },
                        "schedule": {
        ...
        self.assertEqual("titlecrop", blocked["new_v3_records"][0]["v3_field_evidence"]["title"]["source"]["crop_sha256"])
        self.assertEqual("pagehash", blocked["new_v3_records"][0]["v3_field_evidence"]["title"]["source"]["page_image_sha256"])
        self.assertEqual("vision_v3_001", blocked["new_v3_records"][0]["pipeline_version"])
        self.assertEqual("confighash", blocked["new_v3_records"][0]["config_hash"])
        self.assertEqual("lineagehash", blocked["new_v3_records"][0]["runtime_lineage"]["lineage_hash"])
        ...
        self.assertEqual(
            "https://example.com/boardwalk.pdf",
            reviewed["reviewed_new_record_reviews"][0]["source_url"],
        )
        self.assertEqual("official_pdf", reviewed["reviewed_new_record_reviews"][0]["source_metadata"]["source_kind"])
        self.assertEqual("blocked", bare_key["status"])
    ...
    def test_dual_run_report_requires_current_visual_artifacts_for_new_v3_row_review(self) -> None:
        v3_preview = {
            "rows": [
                {
                    "canonical_slug": "poolside-activities",
        ...
        stale_page = build_dual_run_report(
            v2_rows=[],
            v3_preview=v3_preview,
            reviewed_new_record_reviews=[
                {
                    **base_review,
                    "page_image_sha256": "pagehash-old",
                    "field_crop_sha256": "titlecrop-current",
                }
            ],
        )
        stale_crop = build_dual_run_report(
            v2_rows=[],
            v3_preview=v3_preview,
            reviewed_new_record_reviews=[
                {
                    **base_review,
                    "page_image_sha256": "pagehash-current",
                    "field_crop_sha256": "titlecrop-old",
                }
            ],
        )
        current = build_dual_run_report(
            v2_rows=[],
            v3_preview=v3_preview,
            reviewed_new_record_reviews=[
                {
        ...
        self.assertEqual("blocked", stale_page["status"])
        self.assertEqual([review_key], stale_page["new_v3_review_keys"])
        self.assertEqual(["boardwalk:poolside-activities"], stale_page["unreviewed_new_keys"])
        self.assertEqual("blocked", stale_crop["status"])
        self.assertEqual(["boardwalk:poolside-activities"], stale_crop["unreviewed_new_keys"])
        self.assertEqual("clean", current["status"])
        self.assertEqual([review_key], current["reviewed_new_record_keys"])
        self.assertEqual([], current["unreviewed_new_keys"])

    def test_dual_run_report_does_not_treat_legacy_unknown_price_as_conflict(self) -> None:
        v2_rows = [
            {
                "canonical_slug": "campfire",
                "title": "Campfire",
                    ...
                    "field_evidence": {
                        "fee": {
                            "source": {
                                ...
                                "page_number": 1,
                                "page_image_sha256": "pagehash",
                                "crop_storage_path": "fields/fee.png",
                                "crop_sha256": "feecrop",
                                "bbox_px": [20, 40, 180, 70],
                            },
                            "engines": [
                                {"engine": "paddleocr_ppstructurev3", "text": "no fee marker"},
                                {"engine": "rapidocr", "text": "no fee marker"},
                            ],
                        }
                    },
                    "validation_status": "auto_publishable",
        ...
        report = build_dual_run_report(v2_rows=v2_rows, v3_preview=v3_preview)

        self.assertEqual([], report["field_diffs"])
        self.assertNotIn("unreviewed_v2_v3_field_diffs", report["publish_blockers"])

    def test_dual_run_report_uses_fee_evidence_for_real_price_diffs(self) -> None:
        v2_rows = [
            {
                "canonical_slug": "campfire",
                "title": "Campfire",
        ...
        self.assertEqual("price", diff["field"])
        self.assertEqual("feecrop", diff["v3_field_evidence"]["source"]["crop_sha256"])
        self.assertEqual("pagehash", diff["v3_field_evidence"]["source"]["page_image_sha256"])
    ...
    def test_dual_run_report_requires_hash_bound_review_for_field_diff(self) -> None:
        ...
        self.assertEqual(
            "https://example.com/boardwalk.pdf",
            reviewed["reviewed_diff_reviews"][0]["source_url"],
        )
        self.assertEqual("official_pdf", reviewed["reviewed_diff_reviews"][0]["source_metadata"]["source_kind"])

    def test_dual_run_report_requires_valid_review_metadata_for_field_diff(self) -> None:
        v2_rows = [
            {
                "canonical_slug": "poolside-activities",
                "title": "Poolside Activities",
        ...
        placeholder_reviewer = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview=v3_preview,
            reviewed_diff_reviews=[
                {
        ...
        placeholder_reason = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview=v3_preview,
            reviewed_diff_reviews=[
                {
        ...
        self.assertEqual("blocked", placeholder_reviewer["status"])
        self.assertEqual(["boardwalk:poolside-activities:schedule"], placeholder_reviewer["unreviewed_diff_keys"])
        self.assertEqual([], placeholder_reviewer["reviewed_diff_keys"])
        self.assertEqual("blocked", placeholder_reason["status"])
        self.assertEqual(["boardwalk:poolside-activities:schedule"], placeholder_reason["unreviewed_diff_keys"])
        self.assertEqual([], placeholder_reason["reviewed_diff_keys"])
    ...
    def test_dual_run_report_blocks_needs_review_to_auto_publishable_without_new_evidence(self) -> None:
        ...
        self.assertEqual("vision_v3_001", blocked["status_transitions"][0]["pipeline_version"])
        self.assertEqual("confighash", blocked["status_transitions"][0]["config_hash"])
        self.assertEqual("lineagehash", blocked["status_transitions"][0]["runtime_lineage"]["lineage_hash"])
        ...
        self.assertEqual(
            "https://example.com/boardwalk.pdf",
            reviewed["reviewed_status_transition_reviews"][0]["source_url"],
        )
        self.assertEqual(
            "official_pdf",
            reviewed["reviewed_status_transition_reviews"][0]["source_metadata"]["source_kind"],
        )
        self.assertEqual("blocked", bare_key["status"])
        self.assertEqual(
            ["boardwalk:poolside-activities:needs_review_to_auto_publishable"],
    ...
    def test_dual_run_report_blocks_unprocessed_sources_without_source_error(self) -> None:
        ...
        source_statuses = [
            source_status_fixture(source_document_id="source-doc-1", status="processed"),
        ...
        clean = build_dual_run_report(
            ...
            v3_source_statuses=[
                *source_statuses,
                source_status_fixture(source_document_id="source-doc-2", status="processed"),
            ],
            review_tasks=[
    ...
    def test_dual_run_report_ignores_deterministic_field_normalization_diffs(self) -> None:
        report = build_dual_run_report(
            v2_rows=[
                ...
                {
                    "calendar_group_key": "riviera",
                    "canonical_slug": "cote-dazur-campfire",
                    "title": "Cote D’Azur Campfire",
                    "schedule": {"text": "Nightly from 6:30pm-7:30pm"},
                    "location": {"label": "Activity Lawn"},
                    "price": {"state": "free"},
                }
            ],
            v3_preview={
        ...
        repaired_prefix = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "saratoga-springs",
                    "canonical_slug": "glow-with-the-flow",
                    "title": "Glow with the Flow",
                    "schedule": {"text": "unday and Thursday from 7:30pm-8:30pm"},
                    "location": {"label": "High Rock Spring Pool Deck"},
                    "price": {"state": "free"},
                },
                {
                    ...
                    "canonical_slug": "mickey-tie-dye",
                    "title": "Mickey Tie-Dye",
                    "schedule": {"text": "— Sunday, Wednesday and Friday from 4:00pm-5:00pm"},
                    "location": {"label": "Near Wilderness Lodge Mercantile"},
                    "price": {"state": "fee"},
                },
            ],
            v3_preview={
                "rows": [
                    {
                        ...
                        "canonical_slug": "glow-with-the-flow",
                        "title": "glow with the flow",
                        "schedule": {"text": "Sunday and Thursday from 7:30pm-8:30pm"},
                        "location": {"label": "High Rock Spring Pool Deck"},
                        "price": {"state": "free"},
                        "field_evidence": {
                            field: {"source": {"content_sha256": "sourcehash"}}
        ...
        self.assertEqual([], report["field_diffs"])
        self.assertEqual([], repaired_prefix["field_diffs"])
        self.assertNotIn("unreviewed_v2_v3_field_diffs", report["publish_blockers"])
        self.assertNotIn("unreviewed_v2_v3_field_diffs", repaired_prefix["publish_blockers"])

    def test_dual_run_report_treats_legacy_location_time_schedule_as_split_fields(self) -> None:
        report = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "beach-yacht-club",
                    "canonical_slug": "movie-under-the-stars",
                    "title": "Movie Under the Stars",
                    "schedule": {"text": "Shipwreck Beach | 8:3OPM"},
                    "location": {"label": "Shipwreck Beach"},
                    "price": {"state": "free"},
                }
            ],
            v3_preview={
    ...
    def test_dual_run_report_treats_repaired_legacy_location_time_schedule_as_split_fields(self) -> None:
        report = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "all-star-movies",
                    "canonical_slug": "movie-under-the-stars",
                    "title": "Movie Under the Stars",
                    "schedule": {"text": "The Reel Spot | 8:30PM"},
                ...
                "rows": [
                    {
                        ...
                        "title": "Movie Under the Stars",
                        "schedule": {"text": "8:30PM"},
                        "location": {"label": "THE REEL SPOT I"},
                        "price": {"state": "free"},
                        "field_evidence": {
                            field: {"source": {"content_sha256": "sourcehash"}}
                            for field in ("title", "schedule", "location", "price")
    ...
    def test_dual_run_report_pairs_unique_orphan_rows_when_stable_fields_match(self) -> None:
        report = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "wilderness-lodge",
                    "canonical_slug": "buttons-and-bells-game-arcade",
                    "title": "Buttons And Bells Game Arcade",
                    "schedule": {"text": "Daily from 8:00am-11:00pm"},
                    "location": {"label": "Near Wilderness Lodge Mercantile"},
                    "price": {"state": "fee"},
                }
            ],
        ...
        self.assertEqual([], report["new_in_v3"])
        self.assertEqual([], report["missing_in_v3"])
        self.assertEqual(["wilderness-lodge:buttons-and-bells-game-arcade:title"], report["unreviewed_diff_keys"])
        [diff] = report["field_diffs"]
        self.assertEqual("Buttons And Bells Game Arcade", diff["v2_value"])
        self.assertEqual("game arcade ($)", diff["v3_value"])

    def test_dual_run_report_pairs_unique_orphan_rows_when_location_suffix_matches(self) -> None:
        report = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "port-orleans-french-quarter",
                    "canonical_slug": "south-quarter-games-arcade",
                    "title": "South Quarter Games Arcade",
                    "schedule": {"text": "Daily from 8:00am-11:00pm"},
                    "location": {"label": "South Quarter Games Arcade"},
                    "price": {"state": "fee"},
                }
            ],
            v3_preview={
                "rows": [
                    {
                        "calendar_group_key": "port-orleans-french-quarter",
                        "canonical_slug": "games-arcade",
                        "title": "games arcade ($)",
                        "schedule": {"text": "Daily from 8:00am-11:00pm"},
                        "location": {"label": "GAMES ARCADE"},
                        "price": {"state": "fee"},
                        "field_evidence": {
                            field: {"source": {"content_sha256": "sourcehash"}}
                            for field in ("title", "schedule", "location", "price")
        ...
        self.assertEqual(
            [
                "port-orleans-french-quarter:south-quarter-games-arcade:title",
                "port-orleans-french-quarter:south-quarter-games-arcade:location",
            ],
            report["unreviewed_diff_keys"],
        )

    def test_dual_run_report_keeps_legacy_location_time_schedule_diff_for_new_day_or_time(self) -> None:
        new_day = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "boardwalk",
                    "canonical_slug": "movie-under-the-stars",
                    "title": "Movie Under the Stars",
                    "schedule": {"text": "Village Green Lawn | 8:30PM"},
                    "location": {"label": "Village Green Lawn"},
                    "price": {"state": "free"},
                }
            ],
        ...
        different_time = build_dual_run_report(
            v2_rows=[
                {
                    "calendar_group_key": "beach-yacht-club",
                    "canonical_slug": "movie-under-the-stars",
                    "title": "Movie Under the Stars",
                    "schedule": {"text": "Shipwreck Beach | 8:30PM"},
                    "location": {"label": "Shipwreck Beach"},
                    "price": {"state": "free"},
                }
            ],
        ...
        self.assertEqual(["boardwalk:movie-under-the-stars:schedule"], new_day["unreviewed_diff_keys"])
        self.assertEqual(
            ["beach-yacht-club:movie-under-the-stars:schedule"],
            different_time["unreviewed_diff_keys"],
        )
    ...
    def test_dual_run_report_source_coverage_reports_duplicate_visual_sources(self) -> None:
        report = build_dual_run_report(
            v2_rows=[],
            v3_preview={"rows": []},
            expected_sources=[
                ...
                {
                    "source_document_id": "source-doc-c",
                    "content_sha256": "source-hash-2",
                    "calendar_group_key": "pop-century",
                },
            ],
            v3_source_statuses=[
        ...
        self.assertEqual("clean", report["status"])
        self.assertEqual(4, report["source_coverage"]["expected_source_row_count"])
        self.assertEqual(3, report["source_coverage"]["expected_source_count"])
        self.assertEqual(2, report["source_coverage"]["expected_source_hash_count"])
        self.assertEqual({"source-doc-a": 2}, report["source_coverage"]["duplicate_expected_source_ids"])
        self.assertEqual({"source-hash-1": 3}, report["source_coverage"]["duplicate_expected_source_hashes"])

    def test_dual_run_report_source_coverage_matches_status_by_hash_when_ids_differ(self) -> None:
        report = build_dual_run_report(
            v2_rows=[],
            v3_preview={"rows": []},
            expected_sources=[
                {
                    "source_document_id": "inventory-source-id",
                    "content_sha256": "source-hash",
                    "calendar_group_key": "all-star-sports",
                    "source_kind": "official_pdf",
                    "source_role": "supporting_price_image",
            ...
            v3_source_statuses=[
                source_status_fixture(
                    source_document_id="snapshot-source-id",
                    content_sha256="source-hash",
                    status="processed",
                )
            ],
    ...
    def test_dual_run_report_filters_expected_sources_to_quarter_visual_resort_documents(self) -> None:
        report = build_dual_run_report(
            ...
            expected_sources=[
                ...
                {
                    "source_document_id": "source-doc-no-edition",
                    "content_sha256": "hash-no-edition",
                    "currentness": "current",
                    "source_kind": "official_image",
                    "source_role": "supporting_price_image",
                    "source_type": "image",
        ...
        self.assertEqual("clean", report["status"])
        self.assertEqual(2, report["source_coverage"]["expected_source_count"])
        self.assertEqual(2, report["source_coverage"]["processed_source_count"])
        self.assertEqual([], report["source_coverage"]["unprocessed_source_ids"])

    def test_dual_run_report_scopes_v2_comparison_to_expected_visual_sources(self) -> None:
        v2_rows = [
            {
                "canonical_slug": "poolside-activities",
                "title": "Poolside Activities",
                "calendar_group_key": "boardwalk",
                "content_sha256": "visual-hash",
            },
            {
                "canonical_slug": "morning-crafts",
        ...
        expected_sources = [
            {
                "source_document_id": "source-doc-visual",
                "content_sha256": "visual-hash",
                "currentness": "current",
                "source_kind": "official_pdf",
                "source_role": "resort_pdf",
        ...
        scoped_clean = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview={
                "rows": [
                    {
        ...
        missing_visual = build_dual_run_report(
            v2_rows=v2_rows,
            v3_preview={"rows": []},
            expected_sources=expected_sources,
            v3_source_statuses=source_statuses,
        ...
        self.assertEqual(2, scoped_clean["v2_count"])
        self.assertEqual(1, scoped_clean["v2_compared_count"])
        self.assertEqual(1, scoped_clean["v2_out_of_scope_count"])
        self.assertEqual([], scoped_clean["missing_in_v3"])
        self.assertNotIn("v3_missing_v2_rows", scoped_clean["publish_blockers"])
        self.assertEqual(["poolside-activities"], missing_visual["missing_in_v3"])
        self.assertIn("v3_missing_v2_rows", missing_visual["publish_blockers"])
    ...
    def test_dual_run_report_surfaces_stale_source_status_schema_versions(self) -> None:
        report = build_dual_run_report(
            v2_rows=[],
            v3_preview={"rows": []},
            expected_sources=[
                {
                    "source_document_id": "source-doc-schema",
                    "content_sha256": "source-hash",
                    "calendar_group_key": "boardwalk",
                }
            ],
            v3_source_statuses=[
                {
                    "source_document_id": "source-doc-schema",
                    "content_sha256": "source-hash",
                    "schema_version": "v3_source_status_000",
                    "status": "processed",
                }
            ],
        )
        ...
        self.assertEqual("blocked", report["status"])
        self.assertEqual(["source-doc-schema"], report["source_coverage"]["stale_source_status_schema_ids"])
        self.assertEqual(["source-doc-schema"], report["source_coverage"]["unprocessed_source_ids"])
        self.assertIn("v3_stale_source_status_schema", report["publish_blockers"])
        self.assertIn("v3_unprocessed_sources", report["publish_blockers"])
    ...
    def test_source_status_v3_maps_expected_sources_to_processed_error_or_unprocessed(self) -> None:
        expected_sources = [
            {
                ...
                "source_kind": "official_image",
                "source_role": "resort_pdf",
                "canonical_url": "https://cdn.example.test/boardwalk.jpg",
                "fetched_url": "https://cdn.example.test/boardwalk.jpg?cache=true",
                "http_status": 200,
                "currentness": "current",
                "captured_at": "2026-06-28T12:00:00+00:00",
                "source_type": "image",
        ...
        self.assertEqual(
            ["v3_source_status_001", "v3_source_status_001", "v3_source_status_001"],
        ...
        self.assertEqual("official_image", statuses[0]["source_kind"])
        self.assertEqual("resort_pdf", statuses[0]["source_role"])
        self.assertEqual("https://cdn.example.test/boardwalk.jpg", statuses[0]["canonical_url"])
        self.assertEqual("https://cdn.example.test/boardwalk.jpg?cache=true", statuses[0]["fetched_url"])
        self.assertEqual(200, statuses[0]["http_status"])
        self.assertEqual("current", statuses[0]["currentness"])
        self.assertEqual("2026-06-28T12:00:00+00:00", statuses[0]["captured_at"])
    ...
    def test_source_status_v3_filters_expected_sources_and_snapshots_by_quarter(self) -> None:
        statuses = build_v3_source_statuses(
            ...
            snapshots=[
                ...
                {
                    "source_sha256": "hash-no-edition",
                    "status": "ocr_complete",
                    "snapshot_path": "snapshots/hash-no-edition.vision.json",
                },
            ],
            quarter="fy26-q4",
        )
        ...
        self.assertEqual(["source-doc-no-edition", "source-doc-q4"], sorted(status["source_document_id"] for status in statuses))
        self.assertEqual(["processed", "processed"], [status["status"] for status in statuses])

    def test_source_status_v3_can_scope_to_visual_resort_sources(self) -> None:
        statuses = build_v3_source_statuses(
            expected_sources=[
                {
                    "source_document_id": "source-doc-pdf",
                    "content_sha256": "hash-pdf",
                    "calendar_group_key": "boardwalk",
                    "source_kind": "official_pdf",
                    "source_role": "resort_pdf",
                    "source_type": "pdf",
                ...
                },
                {
                    ...
                    "calendar_group_key": "boardwalk",
                    "source_kind": "official_html",
                    "source_role": "resort_page",
                    "source_type": "html",
                },
                {
                    "source_document_id": "source-doc-manual",
                    "content_sha256": "hash-manual",
                    "calendar_group_key": "boardwalk",
                    "source_kind": "official_manual_extract",
                    "source_role": "manual_extract",
                },
            ],
            snapshots=[
                {"source_sha256": "hash-pdf", "status": "ocr_complete"},
                {"source_sha256": "hash-image", "status": "ocr_complete"},
                {"source_sha256": "hash-html", "status": "ocr_complete"},
                {"source_sha256": "hash-manual", "status": "ocr_complete"},
            ],
            v3_visual_only=True,
        )

        self.assertEqual(["source-doc-pdf", "source-doc-image"], [status["source_document_id"] for status in statuses])
        self.assertEqual(["processed", "processed"], [status["status"] for status in statuses])
    ...
    def test_dual_run_report_blocks_source_error_sources_without_review_tasks(self) -> None:
        ...
        source_statuses = [
            source_status_fixture(
                source_document_id="source-doc-error",
                content_sha256="hash-error",
                status="source_error",
            )
        ]
